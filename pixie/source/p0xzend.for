      SUBROUTINE P0XZEND(END,PADS_HIT,NLC,LC_HIT_POSITIONS,PADS_FHIT,
     &  NSC,SC_HIT_POSITIONS)
C----------------------------------------------------------------------
C-
C-   Purpose and Methods : Displays one LV0 array in X-Z view with
...
C-
C-   Outputs : display LV0 scintillator pad hits in X-Z
C-   Controls: none
C-
...
C
        DO IHIT=1,NSC(END)
          ...
          IF ( ABS(IX).GT.2 ) ZOFF=4.0
          IF ( END.EQ.1 ) ZOFF=ZOFF*(-1.0)
          CALL J3MOVE(L0Z(END)+ZOFF,XPOS,0.0)
          CALL J3STRG('+')
        ENDDO
C
        DO ICNTR=1,8
