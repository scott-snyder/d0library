...
C
     ...
     &         'Bank PX_MUODIS_RCP NOT FOUND','W')
      ...
      CALL PUGET_i('MUON TK QUAL',MUTKQUAL)
      CALL PUGET_i('MUON MAX MUD1',MAXMUD1)
      IF(MAXMUD1.LE.0)MAXMUD1=9999
      CALL PUGET_i('MUON MAX MUOH',MAXMUOH)
      IF(MAXMUOH.LE.0)MAXMUOH=9999
      CALL PUGET_i('MUON DRAW CELLS', DRCELLS)
      CALL PUGET_i('MUON DRAW HITS', DRHITS)
      CALL PUGET_i('MUON DRAW TRACKS',DRTRACKS)
      CALL PUGET_i('MUON HITS ON TKS',DRTKHITS)
      CALL PUGET_i('MUON DRIFT TIME',DTIMFLG)
