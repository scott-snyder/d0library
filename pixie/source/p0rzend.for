      SUBROUTINE P0RZEND(END,PADS_HIT,NLC,LC_HIT_POSITIONS,PADS_FHIT,
     &  NSC,SC_HIT_POSITIONS)
C----------------------------------------------------------------------
C-
C-   Purpose and Methods : Displays one LV0 array in R-Z view with
...
C-
C-   Outputs : display LV0 scintillator pad hits in R-Z
C-   Controls: none
C-
...
C
      ...
      REAL RPOS,XPOS,YPOS
      REAL L0Z(2)
      REAL RC,DR,XC,YC,DX,DY,XUL,YUL,XLR,YLR
      REAL OFFX,OFFY
      REAL ZOFF,RSIGN
      REAL FASTZ
      REAL SLOWZ,MIQUAL,FULLZ
      REAL SSLOWZ(10,10),MISQUAL,FULLSZ
...
C
      ...
      ENDIF
C
C  Mark the upper and lower maximum extension of good Level 0 hits.
C
      CALL J3MOVE(L0Z(END)+4.0, 45.8,0.0)
      CALL J3DRAW(L0Z(END)-4.0, 45.8,0.0)
      CALL J3MOVE(L0Z(END)+4.0,-45.8,0.0)
      CALL J3DRAW(L0Z(END)-4.0,-45.8,0.0)
C
...
C
        DO IHIT=1,NSC(END)
          XC=SC_HIT_POSITIONS(IHIT,END,1)
          YC=SC_HIT_POSITIONS(IHIT,END,2)
          IX=INT(SC_HIT_POSITIONS(IHIT,END,4))
          ...
          IF ( YC.LT.0.0 ) RSIGN=-1.0
          RPOS=RC*RSIGN
          CALL J3MOVE(L0Z(END)+ZOFF,RPOS,0.0)
          CALL J3STRG('+')
        ENDDO
C
        DO ICNTR=1,8
...
C
 ...
 1002   FORMAT(1X,'S',I1,1X)
      ...
      ENDIF
C
C      CALL L0_SLOWER_VERTEX(SSLOWZ,MISFLAG,MISQUAL,SINTER,GOODSSZ,
C     &                                              FULLSZ,CHAN_EFF)
C      IF ( GOODSSZ ) THEN
C        IF ( MISFLAG.LT.0 .OR. MISFLAG.GT.3 ) MISFLAG=9
C        WRITE(MSG,1003) MISFLAG
C 1003   FORMAT(1X,'T',I1,1X)
C        CALL J3MOVE(SSLOWZ,0.0,0.0)
C        CALL J3STRG(MSG)
C      ELSE
C        CALL J3MOVE(SSLOWZ,0.0,0.0)
C        CALL J3STRG('TB')
