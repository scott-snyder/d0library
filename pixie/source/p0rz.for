...
C----------------------------------------------------------------------
C-
C-   Purpose and Methods : Display the Level 0 in R-Z view.
C-
C-   Inputs  : none
C-   Outputs : display Level 0 R-Z view
C-   Controls: none
C-
...
C
     ...
     &  SC_HIT_POSITIONS)
C
C  Initialize a graphics segment and (if only L0) draw R-Z axes.
C
      CALL PUOPEN
      CALL JJUST(2,2)
      IF ( L0ONLY ) THEN
      ...
      ENDIF
C
C  Draw the Level 0 R-Z view, one end (N=1,S=2) at a time.
C
      DO END=1,2
        CALL P0RZEND(END,PADS_HIT,NLC,LC_HIT_POSITIONS,PADS_FHIT,NSC,
     &    SC_HIT_POSITIONS)
      ENDDO
C
