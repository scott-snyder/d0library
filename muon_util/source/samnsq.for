C+
      SUBROUTINE SAMNSQ (FCN, M, N, F, X, E)
C----------------------------------------------------------------------
C-
...
C-             COV - if 1.0, the approximated error matrix is printed
C-                   (only its lower triangular part).
C-                   Put 0.0 otherwise,
...
C----------------------------------------------------------------------
      ...
      REAL    W(NW)
      EXTERNAL FCN
      INTEGER IPRINT,NFUN,IFLAG,MAXFUN,MPLUSN,KST,NPLUS,KINV,KSTORE
      INTEGER NN,K,I,J,KK,II,IINV,ILESS,IGAMAX,INCINV,INCINP
      INTEGER IIP,JL,JJ,JJP,KL,ICONT,ISS,MC,IPP,ITC,IPS,IPC
      INTEGER IWC1,IWC,MR,JCOV,ICOV,IB,IA,IT,IS
      REAL COV,XSTEP,ESCALE,SUM,B,BB,FF,CHANGE,DM,FC,ACC,XC,XL,FMIN,FSEC
C
      IPRINT = 0
      NFUN = 1000
      COV = 0.0
...
C.....   38 FORMAT(//5X,'      FINAL VALUES OF FUNCTIONS AND VARIABLES')
   ...
   91 CONTINUE
C.....      WRITE(6,1000)
C..... 1000 FORMAT(///40X,'VARIANCE-COVARIANCE MATRIX'///)
      IB=IWC1
      DO 92 I=1,N
        IA=IB+1
        IB=IB+I
   92 CONTINUE
C.....      WRITE(6,1001)(W(J),J=IA,IB)
C..... 1001 FORMAT(10X,5G20.6//)
      RETURN
   36 ICONT=1
   34 ITC=ITC+1
      K=N
...
C.....   56 FORMAT(5X,I6,' CALLS OF FCN')
   ...
   86 CONTINUE
      ...
      END
C+
      SUBROUTINE VD01A (ITEST, X, F, MAXFUN, ABSACC,
     +                  RELACC, XSTEP)
C----------------------------------------------------------------------
C-
...
C-
C-   Created  01-MAR-1968   T. POMENTALE
C-   Updated  12-APR-1995   Andrei Mayorov  initialize and save variables 
C-
C----------------------------------------------------------------------
      data is,fc,xinc,dc,db,fb,fa,da,d /0,8*0./
      save is,fc,xinc,dc,db,fb,fa,da,d 
c
      GO TO (1,2,2),ITEST
    2 IS=6-ITEST
      ITEST=1
   ...
   25 IINC=2
      ...
      IF (FB-FC) 29,45,29
   29 CONTINUE
      IF (ABS(DA-DB) .LT. 1.0E-33) RETURN       ! Alexander Efimov
      IF (ABS(DA-DC) .LT. 1.0E-33) RETURN       ! Alexander Efimov
      D=(FA-FB)/(DA-DB)-(FA-FC)/(DA-DC)
      IF (D*(DB-DC)) 33,33,37
   37 CONTINUE
      IF (ABS(D) .LT. 1.0E-33) RETURN           ! Alexander Efimov
      D=0.5*(DB+DC-(FB-FC)/D)
