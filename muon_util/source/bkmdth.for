      INTEGER FUNCTION BKMDTH ( TREE, MODULE )
C----------------------------------------------------------------------
C-
C-   Purpose and Methods : routine to book a bank MDTH hanging from the 
...
C----------------------------------------------------------------------
      ...
      INTEGER       KMDTH               ! address of the created bank
      CHARACTER*60  MSGSTR              ! error message
C----------------------------------------------------------------------
      DATA          NMUON    /307/
C----------------------------------------------------------------------
      BKMDTH = 0                        ! assume failure
...
C
      IF ( KSTPX .NE. 0 ) THEN          ! TREE exists, go ahead
        ...
        IF ( KSUP .NE. 0 ) THEN         ! SMUO exists, go ahead
          CALL MZBOOK ( IDVSTP, KMDTH, KSUP, -IZMDTH, 'MDTH', NMUON,
     &      NMUON, 10, 2, 0)
          BKMDTH = KMDTH
        ELSE                            ! SMUO does not exist
          MSGSTR = ' BKMDTH: Bank SMUO does not exist under '//TREE
          CALL INTMSG (MSGSTR)
        ENDIF
      ELSE                              ! TREE does not exist
        MSGSTR = ' BKMDTH: Bank '//TREE//' does not exist '
        CALL INTMSG (MSGSTR)
      ENDIF
C
  999 RETURN
