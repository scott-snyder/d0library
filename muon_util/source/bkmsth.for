      INTEGER FUNCTION BKMSTH ( TREE, MODULE )
C----------------------------------------------------------------------
C-
C-   Purpose and Methods : routine to book a bank MSTH hanging from the 
...
C----------------------------------------------------------------------
      ...
      INTEGER       KMSTH               ! address of the created bank
      CHARACTER*60  MSGSTR              ! error message
C----------------------------------------------------------------------
      DATA          NMUON    /307/
C----------------------------------------------------------------------
      BKMSTH = 0                        ! assume failure
...
C
      IF ( KSTPX .NE. 0 ) THEN          ! TREE exists, go ahead
        ...
        IF ( KSUP .NE. 0 ) THEN         ! SMUO exists, go ahead
          CALL MZBOOK ( IDVSTP, KMSTH, KSUP, -IZMSTH, 'MSTH', NMUON,
     &      NMUON, 10, 2, 0)
          BKMSTH = KMSTH
        ELSE                            ! SMUO does not exist
          MSGSTR = ' BKMSTH: Bank SMUO does not exist under '//TREE
          CALL INTMSG (MSGSTR)
        ENDIF
      ELSE                              ! TREE does not exist
        MSGSTR = ' BKMSTH: Bank '//TREE//' does not exist '
        CALL INTMSG (MSGSTR)
      ENDIF
C
  999 RETURN
