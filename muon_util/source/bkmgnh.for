      INTEGER FUNCTION BKMGNH ( TREE, MODULE )
C----------------------------------------------------------------------
C-
C-   Purpose and Methods : routine to book a bank MGNH hanging from the 
...
C----------------------------------------------------------------------
      ...
      INTEGER       KMGNH               ! address of the created bank
      CHARACTER*60  MSGSTR              ! error message
C----------------------------------------------------------------------
      DATA          NMUON    /307/
C----------------------------------------------------------------------
      BKMGNH = 0                        ! assume failure
...
C
      IF ( KSTPX .NE. 0 ) THEN          ! TREE exists, go ahead
        ...
        IF ( KSUP .NE. 0 ) THEN         ! SMUO exists, go ahead
          CALL MZBOOK ( IDVSTP, KMGNH, KSUP, -IZMGNH, 'MGNH', NMUON,
     &      NMUON, 10, 2, 0)
          BKMGNH = KMGNH
        ELSE                            ! SMUO does not exist
          MSGSTR = ' BKMGNH: Bank SMUO does not exist under '//TREE
          CALL INTMSG (MSGSTR)
        ENDIF
      ELSE                              ! TREE does not exist
        MSGSTR = ' BKMGNH: Bank '//TREE//' does not exist '
        CALL INTMSG (MSGSTR)
      ENDIF
C
  999 RETURN
