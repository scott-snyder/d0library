      SUBROUTINE MU_WAM_CEN_C_L2(MCELL,I,MCELL_OUT,NCLUSTERS)
...
C     1      MCELL_OUT(I,1)=MCELL_OUT(I+2,3)
	...
	IF (MCELL_OUT(I+2,1).EQ.0.AND.MCELL_OUT(I+1,2).NE.0)
     1      MCELL_OUT(I+2,1)=MCELL_OUT(I+1,2)
...
C                       RIGHT SIDE / EVEN CELL
      IF(MCELL(I+1,1).NE.0.AND.MCELL(I+2,2).NE.0) THEN
	IF (MCELL_OUT(I+1,1).NE.0.AND.MCELL_OUT(I+2,2).EQ.0) 
     1      MCELL_OUT(I+2,2)=MCELL_OUT(I+1,1)
	IF (MCELL_OUT(I+1,1).EQ.0.AND.MCELL_OUT(I+2,2).NE.0)
     1      MCELL_OUT(I+1,1)=MCELL_OUT(I+2,2)
	IF (MCELL_OUT(I+1,1).EQ.0.AND.MCELL_OUT(I+2,2).EQ.0) THEN
	  MCELL_OUT(I+1,1) = J
          MCELL_OUT(I+2,2) = J
          FOUND = .TRUE.
	END IF
