C*********************************************************************
      ...
      SUBROUTINE PYRESD
C.....
C..   Modified to allow the decays of colored SUSY particles before
C..     showering.
C..   Last change:  27 June 95
C..   Upgraded to Pythia 5.7
...
C...Check if initial resonance has been moved (in resonance + jet).
      ...
      IF(IREF(1,JT).GT.0) THEN
        IF(K(IREF(1,JT),1).GT.10) THEN
          ...
          IF(KFA.EQ.6.OR.KFA.EQ.7.OR.KFA.EQ.8) THEN
            DO 110 I=IREF(1,JT)+1,N
            IF(K(I,1).LE.10.AND.K(I,2).EQ.K(IREF(1,JT),2)) IREF(1,JT)=I
  110       CONTINUE
          ELSE
            KDA=MOD(K(IREF(1,JT),4),MSTU(4))
CMRENNA            IF(KFA.GE.23.AND.KFA.LE.40.AND.KDA.GT.1) IREF(1,JT)=KDA
CMRENNA.....75 is the last susy particle
            IF(KFA.GE.23.AND.KFA.LE.75.AND.KDA.GT.1) IREF(1,JT)=KDA
          ENDIF
        ENDIF
      ENDIF
  120 CONTINUE
...
CMRENNA---
      ...
      IF(ID.EQ.0) GOTO 160
      KFA=IABS(K(ID,2))
CMRENNA....66 is the LSP....do not try to decay it!!
      IF(KFA.EQ.66) GOTO 160      
CMRENNA      IF((KFA.LT.23.OR.KFA.GT.40).AND.KFA.NE.6.AND.KFA.NE.7.AND.
CMRENNA.......................75 is the last susy particle
      IF((KFA.LT.23.OR.KFA.GT.75).AND.KFA.NE.6.AND.KFA.NE.7.AND.
...
CMRENNA      IF(IABS(KFL1(JT)).GE.23.AND.IABS(KFL1(JT)).LE.40) KDCY(JT)=3
      IF(IABS(KFL1(JT)).GE.23.AND.IABS(KFL1(JT)).LE.75) KDCY(JT)=3
CMRENNA+++
      KFL3(JT)=KFDP(IDC,3)*ISIGN(1,K(ID,2))
      IF(KFL3(JT).NE.0) THEN
       KFC3A=IABS(KFL3(JT))
       IF(KCHG(KFC3A,3).EQ.0) KFL3(JT)=IABS(KFL3(JT))
       IF(KFA.NE.65.AND.KFC1A.EQ.66) THEN
        IF(KFCA3.LT.10) THEN
         KDCY(JT)=1
        ELSE
         KDCY(JT)=2
...
CMRENNA+++
        P(N+3,5)=0.0 
        IF(KFL3(JT).NE.0) P(N+3,5)=PMAS(KFC3A,1)
CMRENNA....handle near degeneracy cases
        IF(KFA.GE.41.and.KFA.LE.75) THEN
	 if(p(n+3,5)+p(n+1,5)+p(n+2,5).gt.p(id,5)) then
          p(n+1,5)=p(id,5)-p(n+2,5)-.5
          if(p(n+1,5).lt.0.0) p(n+1,5)=0.0
         endif
        ENDIf
        IF(P(N+1,5)+P(N+2,5)+p(n+3,5)+PARJ(64).GT.P(ID,5)) THEN
         ...
         RETURN
        ENDIF
c$$$        IF(P(N+1,5)+P(N+2,5)+PARJ(64).GT.P(ID,5)) THEN
c$$$         CALL LUERRM(13,'(PYRESD:) daughter masses too large')
c$$$         MINT(51)=1
c$$$         RETURN
c$$$        ENDIF
      ELSEIF(IP.EQ.1) THEN
...
C...Special cases, done by hand, for techni-eta, t, leptoquark and q*.
      MSTU(10)=1
CMRENNA+++
      IF(KFA.GE.65.AND.KFA.LE.71.AND.KFL3(JT).NE.0) THEN
       MSTU(19)=1
       XM(1)=p(n+1,5)
       XM(2)=p(n+2,5)
       XM(3)=p(n+3,5)
...
C......NOW, FILL UP THE RECORD
       ...
       N=N+3
CMRENNA IF-> ELSEIF
      ELSEIF(KFA.EQ.38.OR.KFA.EQ.39.OR.
     $((MSTP(6).EQ.1.OR.MSTP(49).GE.1).AND.
     &(KFA.EQ.7.OR.KFA.EQ.8)).OR.KFA.EQ.6 .OR.
     $ (KFA.GE.41.and.KFA.LE.75) ) THEN
        MSTU(19)=1
        CALL LU2ENT(N+1,KFL1(JT),KFL2(JT),P(ID,5))
        ISID=4
        IF(K(ID,2).LT.0) ISID=5
CMRENNA        IF(KFA.EQ.38) THEN
        IF(KFA.EQ.38 .or. (KFA.eq.65.and.ABS(KFL1(JT)).GE.41
     $ .and.abs(kfl1(Jt)).LE.52)) THEN
          IF(KFC1A.EQ.21.AND.RLU(0).GT.0.5) ISID=9-ISID
          K(N-1,1)=3
          K(N,1)=3
CMRENNA+++
          IF(KFA.EQ.65) THEN
           IF( K(N-1,2).LT.0 ) THEN
            ISID=5
           ELSE
            ISID=4
           ENDIF
...
CMRENNA---
     ...
     &  THEN
        ...
        ELSEIF(KFA.EQ.39) THEN
          ...
          K(N-1,ISID)=MSTU(5)*ID
CMRENNA+++
        ELSEIF(KFA.GE.41.and.KFA.LE.75) THEN
         ICC1=KCHG(ABS(K(N-1,2)),2)
         ICC2=KCHG(ABS(K(N,2)),2)
         IF(ICC1.eq.0.and.ICC2.eq.0) THEN
          K(N-1,1)=1
          K(N,1)=1
         ELSEIF(ICC1*ICC2.ne.0) THEN
          K(N-1,1)=3
          K(N,1)=3
C.........3-3-93  SM, added h -> gg, sq -> q gluino, t2 -> t1 g
          IF(KFA.GE.72.and.KFA.LE.74.and.KFL1(JT).EQ.21) THEN
           IF(RLU(0).lt..5) ISID=9-ISID
           K(N-1,ISID)=MSTU(5)*N
           K(N-1,9-ISID)=MSTU(5)*N
           K(N,ISID)=MSTU(5)*(N-1)
           K(N,9-ISID)=MSTU(5)*(N-1)
          ELSEIF(KFA.GE.41.and.KFA.LE.52.and.KFL1(JT).eq.65) THEN
           K(ID,ISID)=K(ID,ISID)+(N-1)
           K(N-1,ISID)=MSTU(5)*ID
           K(N-1,9-ISID)=MSTU(5)*N
           K(N,ISID)=MSTU(5)*(N-1)
          ELSEIF(KFA.GE.41.and.KFA.LE.52.and.KFL2(JT).eq.21) THEN
           K(ID,ISID)=K(ID,ISID)+N
           K(N,ISID)=MSTU(5)*ID
           K(N,9-ISID)=MSTU(5)*(N-1)
           K(N-1,ISID)=MSTU(5)*(N)
...
CMRENNA---
      ...
      MSTU(10)=2
CMRENNA
CMRENNA  160 IF(KFA.GE.23.AND.KFA.LE.40.AND.KFL1(JT).EQ.0) NINH=NINH+1
  160 IF(KFA.GE.23.AND.KFA.LE.75.AND.KFL1(JT).EQ.0) NINH=NINH+1
  170 CONTINUE
...
C...Angular weight for f + f~' -> Z0 + W+/- -> 4 quarks/leptons.
        ...
        CAWZ=COUP(2,3)/SNGL(DT)-2.*(1.-XW)*COUP(1,2)*(SH-SQMW)*FACBW
        CBWZ=COUP(1,3)/SNGL(DU)+2.*(1.-XW)*COUP(1,2)*(SH-SQMW)*FACBW
...
CMRENNA+++
     ...
     &      DPMO(1)/DPMO(4),DPMO(2)/DPMO(4),DPMO(3)/DPMO(4))
         ...
         CALL LUDBRB(NSD(JT)+1,NSD(JT)+3,ACOS(CTHE(JT)),PHI(JT),
     &      DPMO(1)/DPMO(4),DPMO(2)/DPMO(4),DPMO(3)/DPMO(4))
        endif
CMRENNA---
CMRENNA        CALL LUDBRB(NSD(JT)+1,NSD(JT)+2,ACOS(CTHE(JT)),PHI(JT),
CMRENNA     &  DPMO(1)/DPMO(4),DPMO(2)/DPMO(4),DPMO(3)/DPMO(4))
      ELSE
C...Z + q + q~ : angles already known, in rest frame of system.
  ...
  430   CONTINUE
     ...
     &  DBEZQQ(2),DBEZQQ(3))
      ...
      KFA=IABS(K(ID,2))
CMRENNA
CMRENNA      IF(KFA.EQ.38.OR.KFA.EQ.39.OR.((MSTP(6).EQ.1.OR.MSTP(49).GE.1).AND.
CMRENNA     &(KFA.EQ.7.OR.KFA.EQ.8)).OR.(MSTP(48).GE.1.AND.KFA.EQ.6)) THEN
      IF(KFA.EQ.38.OR.KFA.EQ.39.OR.((MSTP(6).EQ.1.OR.MSTP(49).GE.1).AND.
     &(KFA.EQ.7.OR.KFA.EQ.8)).OR.(MSTP(48).GE.1.AND.KFA.EQ.6) .OR.
     $ ((KFA.GE.41.AND.KFE.LE.52).OR.KFA.EQ.65) ) THEN
...
CMRENNA+++
      ...
      DO 450 I=NSD(JT)+1,IHI
CMRENNA---
CMRENNA      DO 450 I=NSD(JT)+1,NSD(JT)+2
      I1=MINT(83)+MINT(4)+1
      K(I,3)=I1
...
CMRENNA      IF(KNSDA1.EQ.6.OR.KNSDA2.EQ.6.OR.KNSDA1.EQ.7.OR.KNSDA2.EQ.7.OR.
CMRENNA     &KNSDA1.EQ.8.OR.KNSDA2.EQ.8) THEN
CMRENNA....handle case of stop -> top + gluino
      IF((KNSDA1.EQ.6.AND.KNSDA2.NE.65).OR.
     $   (KNSDA2.EQ.6.AND.KNSDA1.NE.65).OR.
     $    KNSDA1.EQ.7.OR.KNSDA2.EQ.7.OR.
