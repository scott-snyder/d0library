...
C-
C-           Updated to account for supersymmetric particles from spythia
C-
C-
C-   Inputs  :
...
C-   Updated  12-NOV-1995   Adam Lyon  - handles susy particles from spythia
C-
C----------------------------------------------------------------------
      IMPLICIT NONE
      INTEGER IQUARK(6),LEPTON(6),BOSON(20),L,JSPIN,QRKCRG(6),QPROD
      INTEGER QMESON, GAUGINO(10)
      ...
      DATA GAUGINO/29,30,40,50,60,39,49,82,84,86/
      INTEGER IABSIN,INPUT,OUTPUT,NEXT,I5,I4,I3,I2,I1,ICHRG,IQ(3),hand
      INTEGER spar
C----------------------------------------------------------------------
      IABSIN=IABS(INPUT)
      ICHRG=INPUT/IABSIN
...
C*****Is it a supersymmetric particle (from spythia)?
      ELSEIF ( IABSIN .GE. 41 .AND. IABSIN .LE. 74 ) THEN
        
C*****  is it right or left handed?
        hand = 0       ! 0 = lefty, 1 = righty
        IF ( REAL(REAL(iabsin)/2.) .EQ. (iabsin/2))
     &       hand = 1

        spar = ( iabsin / 2 ) - hand - 19

C*****  if its a squark...
        IF ( iabsin .LE. 52 )
     &     output = ( iquark(spar) + 20 + 20*hand ) * ichrg

C*****  if its a slepton...
        IF ( iabsin .GE. 53 .AND. iabsin .LE. 64 ) THEN
          spar = spar - 6
          output = ( lepton(spar) + 20 + 20*hand ) * ichrg
        ENDIF

C*****  if its a gaugino (gluino, chargino, neutralino, higgsino)...
        IF ( iabsin .GE. 65 ) THEN
          spar = iabsin - 64
          output = gaugino(spar) * ichrg
        ENDIF
