      SUBROUTINE DUHIT(ITRA,X,Y,Z,LAYER,SUBLAY,DESTEP)
C----------------------------------------------------------------------
C-
C-   Purpose and Methods : Save Energy for CC track in working array
C-
...
C
        ILYR = LAYER
      ELSE
        CALL UCINDX(LAYER,SUBLAY,X,Y,Z,IETA,IPHI,ILYR)
      ENDIF
C
C ****  STORE THE DATA
C
