...
C
      IF ( FIRST ) THEN
        ...
        CALL EZGET('MOVE_TRACK_TO_SHOWER',MOVE_TRACK_TO_SHOWER,IER)
        CALL EZGET('DECAY_TYPE', DECAY_TYPE, IER)
        IF(FORCE_SHLB)THEN
          DECAY_TYPE = 2
        ENDIF
        CALL EZGET('NFORCE', MAX_FORCE, IER)
...
C
          ENDIF
C
          CALL UCOPY(RKEY,RKEY_SRT(1,NSRT),NKEY)
          CYC_SRT(NSRT) = CYC
