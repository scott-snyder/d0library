...
C-    ISECV is the sector number (0 - 31)
C-   Outputs :
...
C-  P. Grudberg 16-NOV-1989  fix up end numbering, clean up
C-----------------------------------------------------------------------
      IMPLICIT NONE
      INCLUDE 'D0$INC:ZEBSTP.INC/LIST'
C
      INTEGER LVZST, GZVZST, NWZLYR, I
...
C
      ...
      IPAD   = INT(ZIPAD)
      NZSTRP = MOD( ISECV*NSTRPS(LAYER) / NSECS(LAYER)
     &              - (-1)**LAYER*IPAD + NSTRPS(LAYER),  
     &                NSTRPS(LAYER))
...
C
      ...
      ELSE IF ( LAYER .EQ. 3 ) THEN
        IEND = MOD( (NZSTRP+185)/96, 2 )
      ELSE IF ( LAYER .EQ. 4 ) THEN
        IEND = MOD( (NZSTRP+5)/96, 2 )
      ELSE 
