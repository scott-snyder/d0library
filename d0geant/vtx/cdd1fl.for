      SUBROUTINE CDD1FL(VERSION,LNSKP,IFL)
C----------------------------------------------------------------------
C-
C-   Purpose and Methods : Fill Header or Trailer Blocks for the
C-                         specfied Version in CDD1 bank.
C-
C-   Inputs  : VERSION = Input Version Number
C-             LNSKP   = Number of words to skip relative to LCDD1 before
C-             filling bank.
C-   Outputs : none
...
C
      INTEGER VERSION,IFL,LNSKP,LCDD1,ISETVN
...
C
      IQ(LCDD1) = ISETVN( IQ(LCDD1), VERSION )  ! Put Version number into
C                                               ! Status word
...
C
      IF (VERSION.EQ.1) THEN
        IQ(LCDD1+LNSKP+1) = 1
        IQ(LCDD1+LNSKP+2) = WRDVAL
      ELSE IF (VERSION.EQ.2) THEN
        IQ(LCDD1+LNSKP+1) = 5           ! Header Longword count
        IQ(LCDD1+LNSKP+2) = WRDVAL      ! SYNC Word
C
        IF (SVTX(5).EQ.0) THEN
          CWORD(1) = 0
        ELSE
        ...
        IQ(LCDD1+LNSKP+3) = WRDVAL      ! Controller Word
C
        IQ(LCDD1+LNSKP+4) = 0           ! Spare
        CALL MVBITS(VERSION,0,29,WRDVAL,0)
        CALL MVBITS(1,0,3,WRDVAL,29)
        IQ(LCDD1+LNSKP+5) = WRDVAL      ! Version Number
        IQ(LCDD1+LNSKP+6) = 24          ! Header Byte count
      ENDIF
      GO TO 999
C
  100 CONTINUE
C
      IF (VERSION.EQ.1) THEN
        IQ(LCDD1+LNSKP+1) = 0
        IQ(LCDD1+LNSKP+2) = LNSKP
      ELSE IF (VERSION.EQ.2) THEN
        IQ(LCDD1+LNSKP+1) = LNSKP
