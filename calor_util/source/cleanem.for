...
C
        ...
        IF (IER.EQ.0) THEN
C
          IF (IER.EQ.0) CALL EZGETA_iarr('COARSE_CHISQUARED',0,0,0,N
     &         ,IER)
          IF (IER.EQ.0) CALL EZGETA('COARSE_CHISQUARED',1,N,1,
     &      COARSE_CHISQUARED,IER)
          IF (IER.EQ.0) CALL EZGETA_iarr('FINE_CHISQUARED',0,0,0,N,IER)
     ...
     &      CORECLUS_CUT,IER)
          ...
          IF (IER.EQ.0) CALL EZGETA_iarr('ISOLATION_CONES',0,0,0,N,IER)
          IF (IER.EQ.0) CALL EZGETA('ISOLATION_CONES',1,N,1,
     &      ISOL_CONE_SIZE,IER)
          IF (IER.EQ.0) CALL EZGETA_iarr('ISOLATION_CONE_ENERGY_CUT',0,0
     &         ,0,N,IER)
          IF (IER.EQ.0) CALL EZGETA('ISOLATION_CONE_ENERGY_CUT',1,N,1,
     &      FISOL_ECUT,IER)
          IF (IER.EQ.0) CALL EZGETA_iarr('ISOLATION_CONE_ET_CUT',0,0,0,N
     &         ,IER)
          IF (IER.EQ.0) CALL EZGETA('ISOLATION_CONE_ET_CUT',1,N,1,
     &      FISOL_ETCUT,IER)
          IF (IER.EQ.0) CALL EZGETA_iarr('ISOLATION_CONE_TOT_ENERGY_CUT'
     &         ,0,0,0,N,IER)
          IF (IER.EQ.0) CALL EZGETA('ISOLATION_CONE_TOT_ENERGY_CUT',1,N,
     &      1, ISOL_ECUT,IER)
          IF (IER.EQ.0) CALL EZGETA_arr('ISOLATION_CONE_TOT_ET_CUT',0,0
     &         ,0,N,IER)
          IF (IER.EQ.0) CALL EZGETA('ISOLATION_CONE_TOT_ET_CUT',1,N,1,
     &      ISOL_ETCUT,IER)
          IF (IER.EQ.0) CALL EZGET('CC_CRACK_WIDTH',CC_CRACK_WIDTH,IER)
          IF (IER.EQ.0) CALL EZGET_i('MINIMUM_NUMBER_OF_CELLS'
     &         ,NCELLS_MIN,IER)
          IF (IER.EQ.0) CALL EZGET('CELL_ENERGY_THRESHOLD',CELL_EMIN,
     &      IER)
          IF (IER.EQ.0) CALL EZGET_l('MATCH_LEVEL2_ELECTRON'
     &         ,LEVEL2_MATCH,IER)
          ...
          IF (IER.EQ.0) CALL EZGET_i('NZTRAKS_CUT',NZTRAKS_CUT,IER)
          IF (IER.EQ.0) CALL EZGETA_iarr('CDC_MIP_CUT',0,0,0,N,IER)
          IF (IER.EQ.0) CALL EZGETA('CDC_MIP_CUT',1,N,1,CDC_MIP_CUT,IER)
          IF (IER.EQ.0) CALL EZGETA_iarr('FDC_MIP_CUT',0,0,0,N,IER)
          IF (IER.EQ.0) CALL EZGETA('FDC_MIP_CUT',1,N,1,FDC_MIP_CUT,IER)
          IF (IER.EQ.0) CALL EZGETA_iarr('VTX_MIP_CUT',0,0,0,N,IER)
          IF (IER.EQ.0) CALL EZGETA('VTX_MIP_CUT',1,N,1,VTX_MIP_CUT,IER)
          IF (IER.EQ.0) CALL EZGET_l('CC_ALIGN_CORR',CC_MISALIGN,IER)
          IF (IER.EQ.0) CALL EZGETA_iarr('CC_ALIGN_OFF',0,0,0,N,IER)
          IF (IER.EQ.0) CALL EZGETA('CC_ALIGN_OFF',1,N,1,CC_ALIGN_OFF,
     &      IER)
          IF (IER.EQ.0) CALL EZGETA_iarr('DISTANCE_CC_CUT',0,0,0,N,IER)
          IF (IER.EQ.0) CALL EZGETA('DISTANCE_CC_CUT',1,N,1,
     &      DIST_TRKCUT_CC,IER)
          IF (IER.EQ.0) CALL EZGETA_iarr('DISTANCE_EC_CUT',0,0,0,N,IER)
     ...
     &      IER)
          IF (IER.EQ.0) CALL EZGETA_iarr('ERR_DISTANCE_EC',0,0,0,N,IER)
          IF (IER.EQ.0) CALL EZGETA('ERR_DISTANCE_EC',1,N,1,ERR_DIST_EC,
     &      IER)
          IF (IER.EQ.0) CALL EZGETA_iarr('OFFSET_DISTANCE_CC',0,0,0,N
     &         ,IER)
          IF (IER.EQ.0) CALL EZGETA('OFFSET_DISTANCE_CC',1,N,1,
     &      OFFSET_DIST_CC,IER)
          IF (IER.EQ.0) CALL EZGET_l('TURNOFF_EC_OFFSET',
     &      TURNOFF_EC_OFFSET,IER)
          IF (IER.EQ.0) CALL EZGETA_iarr('OFFSET_DISTANCE_EC',0,0,0,N
     &         ,IER)
     ...
     &      TRD_EFFICIENCY_CUT,IER)
          IF (IER.EQ.0) CALL EZGET_l('DO_TRD_ANALYSIS',
     &      DO_TRD_ANALYSIS,IER)
          IF (IER.EQ.0) CALL EZGETA_iarr('IMPACT_PARAMETER_XY',0,0,0,N
     &         ,IER)
     ...
     &      MISSING_ET_FISOL_CUT,IER)
          IF (IER.EQ.0) CALL EZGET_l('DO_CC_CLUS_ZCOR',
     &      DO_CC_CLUS_ZCOR, IER)
          IF (IER.EQ.0) CALL EZGET_l('DO_CDC_ZCOR',DO_CDC_ZCOR, IER)
     ...
     &      ERRFACT_CDC_ZCOR, IER)
          ...
          ENDIF
          IF (IER.EQ.0) CALL EZGET_i('USERMASK',USERMASK,IER)
          DO I = 1,2
