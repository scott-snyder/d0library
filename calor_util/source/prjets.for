      SUBROUTINE PRJETS ( PRUNIT, LJETS, NJETS, CFL, IFL )
C----------------------------------------------------------------------
C-
...
C-   Modified 20-NOV-1993 R. Astur "Flint fix"
C----------------------------------------------------------------------
      IMPLICIT NONE
C
...
C
        ...
        TEMPLATE(3)=0.7
        ALGO       = 'CONE_JET'
        CALL SET_CAPH(ALGO,TEMPLATE,IER)
...
1000    FORMAT(/' ** PRJETS ** Illegal value of CFL = ',a/)
        GOTO 999
      ENDIF
  2   CONTINUE
...
C           OUTPUT JTSH BANK IF IFL = 0 OR 2
        IF (IFL.EQ.0 .OR. IFL.GE.2 ) THEN
          LJTSH = LQ(LJETS1-IZJTSH)
          IF(LJTSH.NE.0) CALL PRJTSH(PRUNIT,LJTSH,NJTSH,CFL,IFL)
        ENDIF
      ELSE
        EMFRAC = Q(LJETS1+14)
...
C
 ...
 2000 FORMAT(/' ** PRJETS ** called for LINEAR without valid bank '
     &        ,'pointer, LJETS =',I10/)
      GOTO 999
C
...
C
 ...
 2100 FORMAT(/'  ** PRJETS ** called for ONE without bank pointer and '
