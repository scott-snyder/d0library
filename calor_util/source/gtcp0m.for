      SUBROUTINE GTCP0M(CRATE,ADR,VAL)
C----------------------------------------------------------------------
C-
C-   Purpose and Methods : Returns values of higher moments (skewness and
C-                            kurtosis) from calib - level-0 pedestals
C-
...
C----------------------------------------------------------------------
   ...
   10   format(' DUMMY VERSION OF GTCP0M CALLED for crate ',I3,
     &    ' adr ',I8)
        call intmsg(msg)
        lfirst = .false.
      endif
...
C          CHAN = ????
C
C          VAL(1) = C(LCP0M+NHEAD+2*CHAN+1)
C          VAL(2) = C(LCP0M+NHEAD+2*CHAN+2)
