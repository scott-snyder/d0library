      SUBROUTINE CLINPL(VTX,DIR,NCLMAX,NCELL,IETAC,IPHIC,LAYERC,PATHC,
     &  ARGSOK)
C----------------------------------------------------------------------
C-
...
C-             PATHC(NCLMAX)  List of path lengths in hit cells
...
C
  ...
  900   CONTINUE
     ...
     &      LAYER.EQ.LAYERC(NCELL)) THEN
              PATHC(NCELL)=PATHC(NCELL)-(S(2)-S(1))
              GO TO 950
            ENDIF
          ENDIF
