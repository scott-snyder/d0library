...
C
      IF (LNEW) THEN
        ...
        CALL EZGET('NETWORK_STRUCTURE',M10, IER)     ! Nodes/layer
        IF(IER.EQ.0) THEN
          DO M = 1, MSTJN(1)
            MSTJN(9+M)=M10(M)
          END DO
...
C
      CALL EZGET('PATTERNS_PER_UPDATE',M,IER) ! # patterns/Update
      IF(IER.EQ.0) MSTJN(2)=M
      CALL EZGET('SIGMOID_FUNCTION_TYPE',M,IER)
      IF(IER.EQ.0) MSTJN(3)=M
      CALL EZGET('ERROR_MEASURE_TYPE',M, IER)     ! Error measure
      IF(IER.EQ.0) MSTJN(4)=M
      CALL EZGET('UPDATING_PROCEDURE',M, IER)
      IF(IER.EQ.0) MSTJN(5)=M
      CALL EZGET('UPDATES_PER_EPOCH', M, IER)
      IF(IER.EQ.0) MSTJN(9)=M
      CALL EZGET('PRUNING',M, IER)
      IF(IER.EQ.0) MSTJN(21)=M
      CALL EZGET('SATURATION_MEASURE',M, IER)
      IF(IER.EQ.0) MSTJN(22)=M
      CALL EZGET('RECEPTIVE_FIELD_GEOMETRY_IN',M10, IER)
      IF(IER.EQ.0) CALL UCOPY(M10(1),MSTJN(23),2)
      CALL EZGET('RECEPTIVE_FIELD_GEOMETRY', M10, IER)
      IF(IER.EQ.0) CALL UCOPY(M10(1),MSTJN(25),2)
      CALL EZGET('RECEPTIVE_FIELD_HIDDEN', M, IER)
      IF(IER.EQ.0) MSTJN(27)=M
      CALL EZGET('BIT_PRECISION',M10, IER)
      IF(IER.EQ.0) CALL UCOPY(M10(1),MSTJN(28),3)
      CALL EZGET('WARNING_PROCEDURE',M, IER)
      IF(IER.EQ.0) MSTJN(31)=M
      CALL EZGET('WARNING_MAX',M, IER)
      IF(IER.EQ.0) MSTJN(32)=M
      CALL EZGET('LINE_SEARCH_MAX_ITERATIONS',M, IER)
      IF(IER.EQ.0) MSTJN(35)=M
      CALL EZGET('LINE_SEARCH_MAX_RESTARTS', M, IER)
