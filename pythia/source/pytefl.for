...
C
      INTEGER I,IER,J,DTR,NVERTX,IWPOS,IWNEG,ID,IV,NPART,LVP,NJT,NQS
      INTEGER RUNNUM,NEVENT
      INTEGER IZ,PNTR
      INTEGER NPJET,LPJHD,GZPJHD
      REAL XYZ(3,NVD),MOM(5),THETA,PHI,ETA
C
...
C----------------------------------------------------------------------
      IF(FIRST) THEN              ! only for first event
        ...
        CALL EZGET('RUN_NUMBER',RUNNUM,IER)
        CALL EZRSET
      ENDIF
C
C ****  zero some things
...
C
      ...
      Q(LISAE+14)=PARI(14)     ! s
      Q(LISAE+15)=PARI(15)     ! t
      Q(LISAE+16)=PARI(16)     ! u
