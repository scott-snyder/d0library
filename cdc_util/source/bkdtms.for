      SUBROUTINE BKDTMS(LSUP,ND,LBANK,IFL)
C----------------------------------------------------------------------
C-
C-   Purpose and Methods : Create Central Drift Chamber Times bank 
C-
C-   Inputs  : LSUP = Address of the supporting bank
C-             ND = length of the created bank
C-   Output  : LBANK = address of the created bank
C-   Control : IFL = 0, Book DTMS with FTMH as supporting bank
C-             IFL = 1, Book DTMS as next bank in linear structure
C-
...
C
      ...
      ENDIF
C
      CALL MZFORM('DTMS','30I -F',NIO)  ! Data Type
      IF (IFL.EQ.0) THEN
        CALL MZBOOK(IDVSTP,LBANK,LSUP,-IZDTMS,'DTMS',NL,NS,ND,NIO,0)
      ELSE
        CALL MZBOOK(IDVSTP,LBANK,LSUP,0,'DTMS',NL,NS,ND,NIO,0)
      ENDIF
C
C ****  Get date and time 
C
