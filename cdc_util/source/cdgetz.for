...
C-   Updated  14-SEP-1994   Stefano Lami Avoid calling nonlinearity corrections
C-                                       for MC data
C-   Updated   2-JAN-1995   Norman A. Graf   Added FIXZ option to correct
C-                                           for CDC dilation
C-   Updated  24-MAY-1995   Norman A. Graf   Removed fixed DELVEL for MC
C-                                           Need to get DL velocities from 
C-                                           STP to agree with D0GEANT.
C----------------------------------------------------------------------
      IMPLICIT NONE
C
      ...
      INTEGER KPS, NHS, IZS, IPS, KPSEC, KPDCBD, IOFCBD
      ...
      REAL    ZCOR, ERRZ
      REAL    CDLTOL, DLTZR1, DLTZR2, DLERR1, DLERR2
      INTEGER IER
      LOGICAL FIRST, EZERROR, DLERRS
      LOGICAL OK, MCDATA
C
...
C
      IF (FIRST) THEN
        ...
        IF ( EZERROR(IER) ) THEN
          CALL ERRMSG('DTRAKS','CDGETZ',
     &      'Unable to find bank DTRAKS_RCP','W')
          GOTO 999
        ENDIF
        CALL EZGET('CDLTOL',CDLTOL,ERR)
        CALL EZGET('DLTZR1',DLTZR1,ERR)
        CALL EZGET('DLTZR2',DLTZR2,ERR)
        CALL EZGET_l('DLERRS',DLERRS,ERR)
        CALL EZGET('DLERR1',DLERR1,ERR)
        CALL EZGET('DLERR2',DLERR2,ERR)
C
...
C
      ...
      KPDCBD = LC( KPDTMD - 1 )
      IOFCBD = KPDCBD + SECTOR*IC(KPDCBD+5)*IC(KPDCBD+4) +5
C
...
C- Pointer to DL nonlinearity correction coefficients
        PTDCBD= IOFCBD + (NUMDL-1)*IC (KPDCBD+4)
...
C
          DO 30 IZM = 1, NHM
            ...
            IF(.NOT. MCDATA) ZM = ZM - (DLC2+DLC3*ZM)*ZM**2
            NBMINU = NBMINU + 1
            ZMINU( NBMINU ) = ZM
...
C
          DO 40 IZP = 1, NHP
            ...
            IF(.NOT. MCDATA) ZP = ZP - (DLC2+DLC3*ZP)*ZP**2
            NBPLUS = NBPLUS + 1
            ZPLUS( NBPLUS ) = ZP
