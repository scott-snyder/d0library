      SUBROUTINE GTDTRK_LINK(LOC,QTRAK)
C-----------------------------------------------------------------------
C
...
C  Daria Zieminska
C  Dec 1989:
C  Updated  28-NOV-1994   NORMAN A. GRAF  Added additional DTRK words
C
...
C
      IF (LOC.EQ.0) THEN           ! track deleted
        CALL VZERO(QTRAK,27)
        GO TO 1000
      END IF
      CALL UCOPY(Q(LOC+1),QTRAK,27)
 1000 RETURN
      END
