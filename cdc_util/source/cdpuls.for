...
C----------------------------------------------------------------------
     ...
     &    'Unable to find bank DTRAKS_RCP','W')
        ...
        CALL EZGET_l('BILFLG',BILFLG,ERR)
        CALL EZGET('BILIRT',BILIRT,ERR)
        CALL EZGET('BILIPT',BILIPT,ERR)
        CALL EZGET_i('MAXCNT',MAXCNT,ERR)
        CALL EZGET_l('SBTRCT',SBTRCT,ERR)
        CALL EZGET_i('CDSURV',CDSURV,ERR)
        CALL EZGET_i('PULTH1(1)',PULTH1(1),ERR)
        CALL EZGET_i('PULTH2(1)',PULTH2(1),ERR)
        CALL EZGET_i('PULTH3(1)',PULTH3(1),ERR)
        CALL EZGET_i('PULMAX(1)',PULMAX(1),ERR)
        CALL EZGET('PULWEI(1)',PULWEI(1),ERR)
        CALL EZGET('TERRSW',TERRSW,ERR)
        CALL EZGET('TERRDL',TERRDL,ERR)
        CALL EZGET_l('TBLFLG',TBLFLG,ERR)
...
C
      IF (CDSURV .NE. NCALL) THEN
        NCALL = CDSURV
        CALL VZERO_i(NEVPED,(MXFADC+1)*32*4)
