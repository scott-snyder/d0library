      SUBROUTINE BKDGNS(LSUP,ND,LBANK,IFL)
C----------------------------------------------------------------------
C-
C-   Purpose and Methods : Create Central Drift Chamber Gains bank 
C-
C-   Inputs  : LSUP = Address of the supporting bank
C-             ND = length of the created bank
C-   Output  : LBANK = address of the created bank
C-   Control : IFL = 0, Book DGNS with FGNH as supporting bank
C-             IFL = 1, Book DGNS as next bank in linear structure
C-
...
C
      ...
      ENDIF
C
      CALL MZFORM('DGNS','30I -F',NIO)
      IF (IFL.EQ.0) THEN
        CALL MZBOOK(IDVSTP,LBANK,LSUP,-IZDGNS,'DGNS',NL,NS,ND,NIO,0)
      ELSE
        CALL MZBOOK(IDVSTP,LBANK,LSUP,0,'DGNS',NL,NS,ND,NIO,0)
      ENDIF
C
C ****  Get date and time 
C
