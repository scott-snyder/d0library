      integer function ez_get_integer_elt (param, index_in,
     &                                     callers_name)
C----------------------------------------------------------------------
C-
...
c
     ...
     &         type .eq. VTDBL) then
        ...
        if (ier .eq. 0) then
          ival = value (sval(1:slen), ii, jj, type)
          if (type .le. 0) then
            call errmsg ('invalid string for integer', callers_name,
     &                   param, 'e')
            ival = 0
          endif
        endif
