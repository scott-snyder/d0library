      SUBROUTINE FITPT3 (LT,IK)
C----------------------------------------------------------------------
C-
C-   Purpose and Methods : Fill the bank TPT3: Table of "probabilities"
C-                         for pions and electrons using the TRUNCATED 
C-                         TOTAL ENERGY with the condition that 3 chambers 
C-                         are hit
C-                         Has been obtained integrating the total energy
C-                         summed over 3 chambers for pions and electrons
C                          Corresponds to PRETRONQ310 and PRETRONQ320
...
C----------------------------------------------------------------------
      WRITE(CAR1,'(I1)')IK
C      PRINT*,' IK',IK,' CAR1 ',CAR1
      DFILE = 'PRETRONQ3'//CAR1//'0'
...
C      PRINT*,CAR26
        ...
        IC(LT+11)= IET   !   Nb. of steps 
         C(LT+12)= or    !   Origin
         C(LT+13)= pas   !   Step
        READ(JUNIT,'(10I7)')wsi,(IWS(I),I=1,iet),
     &        wss
C        PRINT*,'DNA FITPT3  LECTURE'
C        PRINT'(10I7)',wsi,(IWS(I),I=1,iet), wss
        CALL VFLOAT(IWS,WS,IET)
        S1=WSI+WSS
C       PRINT*,' WSI,WSS,S1',WSI,WSS,S1
        S2=VSUM(WS,IET)
        S=S1+S2
C       PRINT*,' DANS FITPT3 WS'
C        PRINT'(10F8.1)',(WS(I),I=1,IET)
C        PRINT*,'DANS FITPT3 S1,S2,S ',S1,S2,S
        S2=WSI
        S1=WSI
        J=(2-ID)*IET+13+LT
        DO 40 I=1,IET
          ...
          WS(I)=1.-S2/S
          C(J+I)=1.-S2/S
C          PRINT*,I,' VNORM',VNORM,'S1,S2,S',S1,S2,S,' WS',WS(I)
          S2=S1
  40    CONTINUE
C        PRINT*,' ID',ID,'J',J
...
C        K=0
   88 CONTINUE
      CLOSE (JUNIT)
C      PRINT*,' NB. DE POINTS',K-LT
C      PRINT'(10F7.3)',(C(13+I+LT),I=1,2*iet)
      GO TO 999
