      SUBROUTINE ECIH_COARSE_ABS_GEOM(STEP,GAP,COPY,Z)
C----------------------------------------------------------------------
C-
C-   Purpose and Methods : Extract volume dimensions for ECIH Coarse
...
C-      
C-   Inputs  : STEP     Step number, to be encoded into volume label
...
C  Data statements
      ...
      DATA IENTRY / 0 /
C----------------------------------------------------------------------
C  On first entry extract Coarse absorber plate dimension information
C----------------------------------------------------------------------
      IF ( IENTRY .EQ. 0 ) THEN
        IENTRY = 1
C----------------------------------------------------------------------
...
C----------------------------------------------------------------------
        CALL EZGET ( 'ECIH_COARSE_ABS_VOLUME_NAME', NAME, IER )
        CALL UCTOH ( 'TUBE', SHAPE, 4, 4 )
        CALL EZGET ( 'STAINLESS_STEEL_CODE', MATERIAL_CODE, IER )
        CALL EZGET ( 'ECIH_MODULE_VOLUME_NAME', MOTHER, IER )
        CALL UCTOH ( 'POS', POS, 4, 3 )
      ENDIF
C----------------------------------------------------------------------
C  Encode the STEP and GAP number into the volume label
C----------------------------------------------------------------------
      WRITE(VOLUME_LABEL,1001) STEP,GAP
C----------------------------------------------------------------------
...
C----------------------------------------------------------------------
C  Write Coarse absorber plate volume
C----------------------------------------------------------------------
      CALL WRITE_VOLUME
C----------------------------------------------------------------------
...
C----------------------------------------------------------------------
      Z = Z + THICKNESS
  999 RETURN
 1001 FORMAT('ECIH_COARSE_ABS_',2I1,'_VOLUME')
      END
