...
C----------------------------------------------------------------------
      ...
      CALL EZGET_i('ECMH_NUMBER_STEPS',NSTEPS,IER)
      ...
      CALL EZGET_i('ECMH_MATERIALS_CODE',MATERIAL_START_CODE,IER)
      CALL EZGET_iarr('ECMH_LAST_GAP_PER_STEP',LAST_GAP_PER_STEP,IER)
      CALL EZGET_i('ECMH_HOMO_MODULE_VOLUME_NAME',VOLUME_MOTHER,IER)
C
C----------------------------------------------------------------------
...
C     WRITE (*,*) 'ECMH Homogenized Mother Material'
     ...
     &           OUTSID_VOL(3)+OUTSID_VOL(4)
      ...
      DO 200 I=1,4
        ...
        N = N + 1
        IF (I.EQ.1) CALL EZGET_i('STAINLESS_STEEL_CODE',
     &              COMPONENT_CODE(I),IER)
        IF (I.EQ.2) CALL EZGET_i('URANIUM_CODE',COMPONENT_CODE(I),IER)
        IF (I.EQ.3) CALL EZGET_i('LIQUID_ARGON_CODE',COMPONENT_CODE(I),
     &              IER)
        IF (I.EQ.4) CALL EZGET_i('G10_CODE',COMPONENT_CODE(I),IER)
...
C-    MINUS the resist-coat cutback.
C----------------------------------------------------------------------
      CALL EZGET_i('ECMH_HOMO_VOLUME_NAME',VOLUME_NAME,IER)
      DO ISTEP=1,NSTEPS
        IF (ISTEP.GE.9) THEN
          SECT='ECMCH'
        ELSE
