      SUBROUTINE CLAYMG_CC
C----------------------------------------------------------------------
C-
C-   Purpose and Methods : Creating the Layer Bank for the Central
...
C
      ...
      LQCMSG = LZFIND(IDVSTP, LC(LQCREG-IZCMSG), ICCMG, IGIDEN)
      CALL MZFORM('CLAY','6I9F2I-F',IOCLAY)
      MCLAY(5) = IOCLAY
C
      CALL MZLIFT(IDVSTP, LQCLAY, LQCMSG, -IZCLAY, MCLAY, 0)
      IC(LQCLAY+ILIDEN) = ICCMG        ! layer ID
      ...
      CALL FLRSRC(LQCLAY,'NONE','CC_SOUTH_MASSLESS_GAP_VOLUME','NONE')
      CALL ECPAD(LQCLAY,'CCMG_DIVISIONS+Z','UMG+')
C
      LC(LQCLAY-IZLSHA) = LC(LQCMSG-IXCSHA)      ! link to CSHA -- same
C                                       ! as from CMSG
      LQCMAT = LZFIND(IDVSTP,LC(LCGEH-IZCMAT),IC(LQCLAY+ILMATE),1)
      LQCLNK =LZFIND(IDVSTP,LC(LQCREG-IZCLNK),IC(LQCMSG+IGIDEN),IGIDEN)
