      SUBROUTINE CC_SURVEY_CYLINDER
C----------------------------------------------------------------------
C-
C-   Purpose and Methods : THIS SUBROUTINE PRODUCES SURVEY TRANSFORMATION
C-                         FOR THE THREE CC CYLINDERS CORRESPONDING TO
C-                         THE EM, FH, CH RINGS.  RESULTS ARE STORED IN
C-                         'CLIN' BANK LINK SUPPORTED BY THE 'CLGI'
C-                         BANK LINK CORRESPONDING TO THE APPROPRIATE
C-                         CYLINDER.
C-
C-   Inputs  :
C-   Outputs :
...
C
        DO 50 I = 1, 3          ! zero arrays for each cylinder
          do j=1, 3
            XNXN(I,j) = 0.
            XNXS(I,j) = 0.
          enddo
...
C
          NMDL = NMDL + 1               ! count modules on a cylinder
...
C
          DO 400 J = 1, 3
            XNOM(J) = XNOM(J)/NMEAS   ! mean nominal position of cylinder
            XSURV(J) = XSURV(J)/NMEAS ! mean survey position of cylinder
            XCNTR(J) = XCNTR(J)/NMDL  ! mean central position of cylinder
  400     CONTINUE
C
          DO 425 J = 1, 3
          DO 425 K = 1, 3
...
C         CALL MATIN2(XSINV, 3, 3, 3, 0, IX, IERR, DETERM)  ! CERNLIB
          ...
          IF( IERR.NE.0) THEN
            CALL ERRMSG('CC_SURVEY_CYLINDER','XSINV',
...
C         CALL MATIN1(RINV, 3, 3, 3, 0, IX, IERR, DETERM)    ! it is
          ...
          IF( IERR .NE. 0) THEN
            CALL ERRMSG('CC_SURVEY_CYLINDER','RINV',
     +        'ERROR INVERTING MATRIX','W')
          END IF
C
C ...     book 'CLIN' bank and fill header
C
          LQCLGI = LC(LCSYL-IRCLGI)
          CALL BKCLIN(LQCLIN)           ! lift CLIN for survey results
          CALL UCOPY(C(LCSRV+1), C(LQCLIN+1), 10)
          IC(LQCLIN+IGMDID) = IC(LQCLGI+IGIDEN)
          LC(LQCLGI-IXCLIN) = LQCLIN
...
C
          do i=1, 3
            do j=1, 3
              C(LQCLIN+IGR11+(i-1)*3+j-1) = RINV(j,i)
            enddo
          enddo
C
