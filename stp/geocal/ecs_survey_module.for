...
C
      ...
      END IF
C
      LCSRV = LZFIND(IDVSURV, LSURV, 3, ISREGN) ! search for ECS flag
...
C         CALL MATIN2(XSINV, 3, 3, 3, 0, IX, IERR, DETERM)  ! CERNLIB
          ...
          IF( IERR.NE.0) THEN
            CALL ERRMSG('ECS_SURVEY_MODULE','XSINV',
...
C         CALL MATIN1(RINV, 3, 3, 3, 0, IX, IERR, DETERM)    ! it is
          ...
          IF( IERR .NE. 0) THEN
            CALL ERRMSG('ECS_SURVEY_MODULE','RINV',
     +        'ERROR INVERTING MATRIX','W')
          END IF
C
...
C
          ...
          IC(LQCLIN) = IOR(IC(LQCLIN),1)   ! set bits to indicate SOUTH
                                           ! End Cal
          CALL UCOPY(C(LCSRV+1), C(LQCLIN+1), 10)
          IC(LQCLIN+IGMDID) = IC(LQCLGA+IGIDEN)    ! minus sign implies SOUTH
                                                   ! modules
          IC(LQCLIN+IGFTYP) = 1                    ! indicates THIS LS analysis
          LC(LQCLGA-IXCLIN) = LQCLIN
          LQCLGI = LC(LQCLGA-IXCLGI)
     ...
     &          (MOD(IC(LQCLGA+IGIDEN),IELINC) .EQ. 0)) THEN ! found EM module
                LC(LQCLGA-IXCLIN) = LQCLIN         ! all EM modules share
