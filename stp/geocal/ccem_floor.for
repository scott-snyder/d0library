      SUBROUTINE CCEM_FLOOR(FLOOR)
C----------------------------------------------------------------------
C-
C-   Purpose and Methods : CCEM Floor volume and materials
C-
C-   CCEM floor volume
...
C-      mixture.
C-
C-   Inputs  : FLOOR    CCEM Floor number
C-   Outputs : none
C-   Controls: none
C-
...
C----------------------------------------------------------------------
C  Determine CCEM volume dimensions:
...
C----------------------------------------------------------------------
      CCEM_FLOOR_INNER_WIDTH = WIDA + (CCEM_FLOOR_INNER_RADIUS-RADA) / 
     &  (RADB-RADA) * (WIDB-WIDA)
      CCEM_FLOOR_OUTER_WIDTH = WIDA + (CCEM_FLOOR_OUTER_RADIUS-RADA) / 
     &  (RADB-RADA) * (WIDB-WIDA)
      CCEM_FLOOR_INNER_LENGTH = LENA + (CCEM_FLOOR_INNER_RADIUS-RADA) / 
     &  (RADB-RADA) * (LENB-LENA)
      CCEM_FLOOR_OUTER_LENGTH = LENA + (CCEM_FLOOR_OUTER_RADIUS-RADA) / 
     &  (RADB-RADA) * (LENB-LENA)
C----------------------------------------------------------------------
C  Compute volume of floor
C----------------------------------------------------------------------
      CCEM_FLOOR_VOLUME = 0.25 * (CCEM_FLOOR_INNER_WIDTH +
     &                            CCEM_FLOOR_OUTER_WIDTH) *
     &                           (CCEM_FLOOR_INNER_LENGTH +
     &                            CCEM_FLOOR_OUTER_LENGTH) *
     &                           (CCEM_FLOOR_OUTER_RADIUS -
     &                            CCEM_FLOOR_INNER_RADIUS)
C----------------------------------------------------------------------
...
C----------------------------------------------------------------------
     ...
     &                   CCEM_FLOOR_OUTER_RADIUS)
C----------------------------------------------------------------------
C  Determine CCEM materials:
C       Determine the material content of the CCEM volume, plus
C       the material content from the CCEM elements which are
...
C----------------------------------------------------------------------
      DO CODE=1,4
        CCEM_FLOOR_VOLUME_INSIDE(CODE) = 0
        CCEM_FLOOR_VOLUME_EXCESS(CODE) = 0
      ENDDO
C----------------------------------------------------------------------
C  Loop over elements within this CCEM floor
C----------------------------------------------------------------------
      DO ICCEM=FIRST_ELEMENT,LAST_ELEMENT
C----------------------------------------------------------------------
...
C----------------------------------------------------------------------
        WIDA = CCEM_FLOOR_INNER_WIDTH +
     &    (RVAL(3) - CCEM_FLOOR_INNER_RADIUS) /
     &    (CCEM_FLOOR_OUTER_RADIUS - CCEM_FLOOR_INNER_RADIUS) *
     &    (CCEM_FLOOR_OUTER_WIDTH  - CCEM_FLOOR_INNER_WIDTH)
        WIDB = CCEM_FLOOR_INNER_WIDTH +
     &    (RVAL(3) + RVAL(4) - CCEM_FLOOR_INNER_RADIUS) /
     &    (CCEM_FLOOR_OUTER_RADIUS - CCEM_FLOOR_INNER_RADIUS) *
     &    (CCEM_FLOOR_OUTER_WIDTH  - CCEM_FLOOR_INNER_WIDTH)
        LENA = CCEM_FLOOR_INNER_LENGTH + 
     &    (RVAL(3) - CCEM_FLOOR_INNER_RADIUS) /
     &    (CCEM_FLOOR_OUTER_RADIUS - CCEM_FLOOR_INNER_RADIUS) *
     &    (CCEM_FLOOR_OUTER_LENGTH - CCEM_FLOOR_INNER_LENGTH)
        LENB = CCEM_FLOOR_INNER_LENGTH +
     &    (RVAL(3) + RVAL(4) - CCEM_FLOOR_INNER_RADIUS) /
     &    (CCEM_FLOOR_OUTER_RADIUS - CCEM_FLOOR_INNER_RADIUS) *
     &    (CCEM_FLOOR_OUTER_LENGTH - CCEM_FLOOR_INNER_LENGTH)
...
C----------------------------------------------------------------------
        IF ( VOL_ELEMENT .GT. VOL_ENCLOSED ) THEN
          CCEM_FLOOR_VOLUME_INSIDE(CODE) = 
     &          CCEM_FLOOR_VOLUME_INSIDE(CODE) + VOL_ENCLOSED
          CCEM_FLOOR_VOLUME_EXCESS(CODE) = 
     &          CCEM_FLOOR_VOLUME_EXCESS(CODE) + VOL_ELEMENT -
     &          VOL_ENCLOSED
        ELSE
          CCEM_FLOOR_VOLUME_INSIDE(CODE) = 
     &          CCEM_FLOOR_VOLUME_INSIDE(CODE) + VOL_ELEMENT
        ENDIF
      ENDDO
C----------------------------------------------------------------------
C  Get label, name, and material code to be used for this CCEM floor
C----------------------------------------------------------------------
...
C----------------------------------------------------------------------
      CALL STORE_MATERIAL
C----------------------------------------------------------------------
C  Set the GEANT SRCP volume parameters for the floor volume
C----------------------------------------------------------------------
      ...
      NUMBER_PARAMS     = 4
      PARAM(1)          = 0.5 * CM_PER_INCH * CCEM_FLOOR_INNER_WIDTH
      PARAM(2)          = 0.5 * CM_PER_INCH * CCEM_FLOOR_OUTER_WIDTH
      PARAM(3)          = 0.5 * CM_PER_INCH * CCEM_FLOOR_INNER_LENGTH
      PARAM(4)          = 0.5 * CM_PER_INCH *
     &          (CCEM_FLOOR_OUTER_RADIUS - CCEM_FLOOR_INNER_RADIUS)
C----------------------------------------------------------------------
C  Write the Floor volume GEANT SRCP parameter description
C----------------------------------------------------------------------
      CALL WRITE_VOLUME
      RETURN
 1001 FORMAT('CCEM_FLOOR',I1,'_FIRST_ELEMENT')
 1002 FORMAT('CCEM_FLOOR',I1,'_FIRST_SIGNAL')
 1003 FORMAT('CCEM_FLOOR',I1,'_LAST_SIGNAL')
 1004 FORMAT('CCEM_FLOOR',I1,'_LAST_ELEMENT')
 1005 FORMAT('CCEM_ELEMENT_',I3.3)
 1006 FORMAT('CCEM_FLOOR',I1,'_MATERIAL')
 1007 FORMAT('CCEM_FLOOR',I1,'_VOLUME')
      END
