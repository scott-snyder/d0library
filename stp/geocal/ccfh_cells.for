      SUBROUTINE CCFH_CELLS(FLOOR)
C----------------------------------------------------------------------
C-
C-   Purpose and Methods : CCFH Cell phi and eta division info for
C-                         a Floor
C-
C-      Set the phi and eta boundary information for the CCFH floors
C-
...
C----------------------------------------------------------------------
      ANGLE_FLOOR = 2. * ATAN( 0.5  * (CCFH_FLOOR_INNER_WIDTH  +
     &                                  CCFH_FLOOR_OUTER_WIDTH ) /
     &                                 (CCFH_FLOOR_INNER_RADIUS +
     &                                  CCFH_FLOOR_OUTER_RADIUS ))/RAD
C----------------------------------------------------------------------
...
C----------------------------------------------------------------------
      ...
      ENDDO
C----------------------------------------------------------------------
C  Get also the phi offset information for the CCFH modules
C----------------------------------------------------------------------
      CALL EZGET('CCFH_ANGULAR_OFFSET',ANGULAR_OFFSET,IER)
C----------------------------------------------------------------------
...
C----------------------------------------------------------------------
     ...
     &                                     NUMBER_ETA_BOUNDARIES)
      WRITE(OUT_VOL,2008)
      RETURN
 1001 FORMAT('CCFH_FLOOR',I1,'_PHI_DIVISIONS')
 1002 FORMAT('CCFH_FLOOR',I1,'_ETA_DIVISIONS')
 1003 FORMAT('CCFH_FLOOR',I1,'_ETA_BOUNDARIES')
 2001 FORMAT('\ARRAY CCFH_FLOOR',I1,'_CELLS')
