      SUBROUTINE CCEM_CELLS(FLOOR)
C----------------------------------------------------------------------
C-
C-   Purpose and Methods : CCEM Cell phi and eta division info for
C-                         a Floor
C-
C-      Set the phi and eta boundary information for the CCEM floors
C-
C-   Inputs  : FLOOR    CCEM Floor number
C-   Outputs : none
C-   Controls: none
C-
C-   Created  14-JAN-1989   Stuart Fuess
...
C----------------------------------------------------------------------
      ANGLE_FLOOR = 2. * ATAN( 0.5  * (CCEM_FLOOR_INNER_WIDTH  +
     &                                  CCEM_FLOOR_OUTER_WIDTH ) /
     &                                 (CCEM_FLOOR_INNER_RADIUS +
     &                                  CCEM_FLOOR_OUTER_RADIUS ))/RAD
C----------------------------------------------------------------------
...
C----------------------------------------------------------------------
      ...
      ENDDO
C----------------------------------------------------------------------
C  Get also the phi offset information for the CCEM modules
C----------------------------------------------------------------------
      CALL EZGET('CCEM_ANGULAR_OFFSET',ANGULAR_OFFSET,IER)
C----------------------------------------------------------------------
...
C----------------------------------------------------------------------
     ...
     &                                     NUMBER_ETA_BOUNDARIES)
      WRITE(OUT_VOL,2008)
      RETURN
 1001 FORMAT('CCEM_FLOOR',I1,'_PHI_DIVISIONS')
 1002 FORMAT('CCEM_FLOOR',I1,'_ETA_DIVISIONS')
 1003 FORMAT('CCEM_FLOOR',I1,'_ETA_BOUNDARIES')
 2001 FORMAT('\ARRAY CCEM_FLOOR',I1,'_CELLS')
