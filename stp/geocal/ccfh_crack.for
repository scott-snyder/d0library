      SUBROUTINE CCFH_CRACK(MODE)
C----------------------------------------------------------------------
C-
C-   Purpose and Methods : CCFH Crack material determination
C-
C-      Use the excess material from each CCFH floor volume
...
C----------------------------------------------------------------------
      IF ( MODE .EQ. 'ZERO' ) THEN
        DO CODE=1,4
          CCFH_CRACK_VOL(CODE) = 0.
        ENDDO
        CCFH_NON_CRACK_VOL = 0.
C----------------------------------------------------------------------
C  If MODE = 'FLOOR' then accumulate floor contributions
C----------------------------------------------------------------------
      ELSE IF ( MODE .EQ. 'FLOOR' ) THEN
        DO CODE=1,4
          CCFH_CRACK_VOL(CODE) = CCFH_CRACK_VOL(CODE) +
     &                           CCFH_FLOOR_VOLUME_EXCESS(CODE)
        ENDDO
        CCFH_NON_CRACK_VOL = CCFH_NON_CRACK_VOL + CCFH_FLOOR_VOLUME
C----------------------------------------------------------------------
C  If MODE = 'ENDPLATE' then subtract endplate volumes
C----------------------------------------------------------------------
      ELSE IF ( MODE .EQ. 'ENDPLATE' ) THEN
        CCFH_NON_CRACK_VOL = CCFH_NON_CRACK_VOL +
     &                       2. * CCFH_ENDPLATE_VOLUME
C----------------------------------------------------------------------
C  If MODE = 'FRONTPLATE' then subtract Front plate volume
C----------------------------------------------------------------------
      ELSE IF ( MODE .EQ. 'FRONTPLATE' ) THEN
        CCFH_NON_CRACK_VOL = CCFH_NON_CRACK_VOL + CCFH_FRONTPLATE_VOLUME
C----------------------------------------------------------------------
C  If MODE = 'BACKPLATE' then subtract Back plate volume
C----------------------------------------------------------------------
      ELSE IF ( MODE .EQ. 'BACKPLATE' ) THEN
        CCFH_NON_CRACK_VOL = CCFH_NON_CRACK_VOL + CCFH_BACKPLATE_VOLUME
C----------------------------------------------------------------------
C  If MODE = 'SKIN' then accumulate module skin contributions
C----------------------------------------------------------------------
      ELSE IF ( MODE .EQ. 'SKIN' ) THEN
        CALL EZGET('CCFH_SIDE_SKIN_WIDTH',WIDTH,IER)
        CALL EZGET('CCFH_SIDE_SKIN_LENGTH',LENGTH,IER)
        CALL EZGET('CCFH_SIDE_SKIN_THICKNESS',THICKNESS,IER)
        CCFH_CRACK_VOL(2) = CCFH_CRACK_VOL(2) +
...
C----------------------------------------------------------------------
        TOTAL_CRACK_VOL = CCFH_MODULE_VOLUME - CCFH_NON_CRACK_VOL
        LIQUID_ARGON_VOL = TOTAL_CRACK_VOL
        DO CODE=1,4
          LIQUID_ARGON_VOL = LIQUID_ARGON_VOL - CCFH_CRACK_VOL(CODE)
        ENDDO
C----------------------------------------------------------------------
C  Get the label, name, and material number to be used for the crack
C----------------------------------------------------------------------
        MATERIAL_LABEL = 'CCFH_CRACK_MATERIAL'
        CALL EZGET_iarr('CCFH_CRACK_MATERIAL_NAME',MATERIAL_NAME,IER)
        CALL EZGET_i('CCFH_CRACK_MATERIAL_CODE',CCFH_CRACK_MATERIAL,IER)
        MATERIAL_CODE = CCFH_CRACK_MATERIAL
C----------------------------------------------------------------------
