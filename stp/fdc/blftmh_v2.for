...
C-   Updated  13-AUG-1992   Robert E. Avery  Make FTMH into
...
C----------------------------------------------------------------------
      ...
      INTEGER NPARWR, NPARDL, NUMTDL, NUMPDL, NIO
...
C
      ...
      LFTMH = LZLAST(0,LSFDC-IZFTMH)
      IF ( LFTMH.GT.0 ) THEN
        CALL MZBOOK(IDVSTP,LFTMH,LFTMH,0,'FTMH',3,3,5,NIO,-1)
      ELSE
        CALL MZBOOK(IDVSTP,LFTMH,LSFDC,-IZFTMH,'FTMH',3,3,5,NIO,-1)
      ENDIF
      IC(LFTMH-5)=2
      IC(LFTMH+1)=LOWRUN                 ! Lowest valid run number
      IC(LFTMH+2)=HIGRUN                 ! Highest valid run number
      LKFTMH = LFTMH
C
      DO 10 HALF = 0, 1
