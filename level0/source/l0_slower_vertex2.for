      SUBROUTINE L0_SLOWER_VERTEX2(SLOWER_Z,MI_FLAG,MI_QUALITY,
     &        INTERACTION,GOODZ,FULL_Z,CHAN_EFF,
     &        N_TOT,S_TOT,N_TIMES,S_TIMES)
C----------------------------------------------------------------------
C-
...
C
     ...
     &                              'LEVEL0_RCP not found.','W')
        ELSE
          ...
          CALL EZGET_i('CUT2_A',CUT_A,ERR)
          CALL EZGET_i('CUT2_B',CUT_B,ERR)
          CALL EZGET_i('CUT2_C',CUT_C,ERR)
          CALL EZGET_i('CUT2_D',CUT_D,ERR)
          CALL EZGET('CUT2_Z1',CUT_Z1,ERR)
          CALL EZGET('CUT2_Z2',CUT_Z2,ERR)
          CALL EZRSET
        ENDIF
C
        FIRST = .FALSE.
...
C
      DO I=1,N_TOT
        N_TIMAP(I)=I
C        N_ORIGTIMES(I)=N_TIMES(I)
      ENDDO
      DO I=1,S_TOT
        S_TIMAP(I)=I
C        S_ORIGTIMES(I)=S_TIMES(I)
      ENDDO
      CALL SRTINT(N_TIMES,N_TOT,N_TIMAP)
      CALL SRTINT(S_TIMES,S_TOT,S_TIMAP)
      DO I=1,N_TOT
        N_PTIMES(I)=N_TIMES(I)
      ENDDO
      DO I=1,S_TOT
        S_PTIMES(I)=S_TIMES(I)
      ENDDO
      N_PTOT = N_TOT
      S_PTOT = S_TOT
C
...
C
      ...
      IF ( NVER.LT.1 ) GOTO 402
C
      N_NGRP=0
      S_NGRP=0
      IF ( DO_NORTH ) THEN  
        IF ( DO_SOUTH ) GOTO 402
        N_CENTIME = (ZVER(1)/0.75) + AVG_T_S
        N_TOT = 0
        N_OTOT = 0
        DO I=1,N_PTOT
          IF ( ABS(N_CENTIME-N_PTIMES(I)).GT.CUT_D ) THEN
            N_TOT = N_TOT + 1
            N_TIMES(N_PTOT)=N_PTIMES(I)
          ELSE
            N_OTOT = N_OTOT + 1
            N_OTIMES(N_OTOT)=N_PTIMES(I)
          ENDIF
        ENDDO
        CALL SRTINT(N_TIMES,N_TOT,N_TIMAP)
C
...
C
     ...
     &                                        (N_MEAN(IGRP)**2.))
        ...
        DO I=1,N_OTOT
          SUM  = SUM + N_OTIMES(I)
          SUM2 = SUM2 + (N_OTIMES(I)**2.)
          N_NUMGRP(N_NGRP) = N_NUMGRP(N_NGRP) + 1
        ENDDO
        IF ( N_NUMGRP(N_NGRP).GT.0 ) THEN
          N_MEAN(N_NGRP)   = SUM / N_NUMGRP(N_NGRP)
          N_SIGMA(N_NGRP)  = SQRT(SUM2/N_NUMGRP(N_NGRP) -
     &                                      (N_MEAN(N_NGRP)**2.))
        ENDIF
c      ENDIF
        S_NGRP=1
        S_NUMGRP(S_NGRP)=0
C
      ELSEIF ( DO_SOUTH ) THEN  
        S_CENTIME = AVG_T_N - (ZVER(1)/0.75)
        S_TOT = 0
        DO I=1,S_PTOT
          IF ( ABS(S_CENTIME-S_PTIMES(I)).GT.CUT_D ) THEN
            S_TOT = S_TOT + 1
            S_TIMES(S_TOT)=S_PTIMES(I)
          ELSE
            S_OTOT = S_OTOT + 1
            S_OTIMES(S_OTOT)=S_PTIMES(I)
          ENDIF
        ENDDO
        CALL SRTINT(S_TIMES,S_TOT,S_TIMAP)
C
...
C
     ...
     &                                        (S_MEAN(IGRP)**2.))
        ...
        DO I=1,S_OTOT
          SUM  = SUM + S_OTIMES(I)
          SUM2 = SUM2 + (S_OTIMES(I)**2.)
          S_NUMGRP(S_NGRP) = S_NUMGRP(S_NGRP) + 1
        ENDDO
        IF ( S_NUMGRP(S_NGRP).GT.0 ) THEN
          S_MEAN(S_NGRP)   = SUM / S_NUMGRP(S_NGRP)
          S_SIGMA(S_NGRP)  = SQRT(SUM2/S_NUMGRP(S_NGRP) -
     &                                      (S_MEAN(S_NGRP)**2.))
        ENDIF
        N_NGRP=1
        N_NUMGRP(N_NGRP)=0
...
C
      IF ( N_NGRP.GT.6 ) N_NGRP=6
      IF ( S_NGRP.GT.6 ) S_NGRP=6
...
C
      ...
      ENDDO
C
      IF ( N_NUMGRP(N_NGRP-1).GT.0.AND.S_NUMGRP(S_NGRP-1).EQ.0 ) THEN
        SLOWER_Z(N_NGRP-1,S_NGRP-1) = 0.75 * (N_MEAN(N_NGRP-1)-AVG_T_S)

      ENDIF
      IF ( N_NUMGRP(N_NGRP-1).EQ.0.AND.S_NUMGRP(S_NGRP-1).GT.0 ) THEN
        SLOWER_Z(N_NGRP-1,S_NGRP-1) = 0.75 * (AVG_T_N-S_MEAN(S_NGRP-1))
