...
C
        ...
        CALL EZGETA_iarr('START_TIME',1,2,1,START_TIME,IER)
 ...
 1001     FORMAT('Number of bad runs in RCP=',I5,' Maximum=',I5)
        ...
        ELSEIF(IER.EQ.0) THEN 
          CALL EZGETA_iarr('VETO_RUNS',1,NBAD_RUNS,1,BAD_RUNS,IER)
          NBAD_RUNS=NBAD_RUNS
        ENDIF
 ...
 1002     FORMAT('Number of selected runs in RCP=',I5,' Maximum=',I5)
        ...
        ELSEIF(IER.EQ.0) THEN 
          CALL EZGETA_iaee('SELECT_RUNS',1,NSEL_RUNS,1,GOOD_RUNS,IER)
        ENDIF
 ...
 1003     FORMAT('Number of selected events in RCP=',I5,' Max=',I5)
        ...
        ELSEIF(IER.EQ.0) THEN 
          CALL EZGETA_iarr('SELECT_EVENTS',1,NSEL_EVTS,1,GOOD_EVTS,IER)
        ENDIF
C
C  File-name based RCP parameters (Can't get MDS version yet...)
C
        CALL EZGETA_iarr('RECO_VERSION',1,2,1,FILE_RECO_VER,IER)
        CALL EZGETA_iarr('OMNI_VERSION',1,2,1,FILE_OMNI_VER,IER)
...
C
        CALL EZGET_l('file_ERROR_IS_FATAL',FILE_ERR_FATAL,IER)
