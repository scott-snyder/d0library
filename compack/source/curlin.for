...
C----------------------------------------------------------------------
      ...
      IF(.NOT.ASTFLG) THEN
C&IF VAXVMS
        CALL README(COMAND,PF1,GETDEV(),'Select: '//COMIN(1:LEN(COMIN))
     &    //' > ')
C&ENDIF
C&IF SIUNIX,IBMAIX,ULTRIX,ALFOSF
C&        CTEMP = 'Select: '//COMIN(1:LEN(COMIN))//' > '
C&        CALL README(COMAND,PF1,GETDEV(),CTEMP)
C&ENDIF
      ELSE
        CALL README(COMAND,PF1,GETDEV(),' ')
      ENDIF
      IF(PF1.EQ.0) THEN
