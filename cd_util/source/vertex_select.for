...
C
     ...
     &                'Unable to find bank VERTEX_FIX_RCP','F')
        ENDIF
        CALL EZGET_l('DO_SWAP_VERTICES',DO_SWAP_VERTICES,IER)
        IF (IER.EQ.0) CALL EZGET_l('USE_EM',USE_EM,IER)
        IF (IER.EQ.0) CALL EZGET_l('USE_MU',USE_MU,IER)
        IF (IER.EQ.0) CALL EZGET_l('USE_JET',USE_JET,IER)
...
C
     ...
     &                'Unable to find bank VERTEX_RCP','F')
        ...
        IF (IER.EQ.0) CALL EZGET_rarr('BEAM_POS',BEAM_POS,IER)
        IF (IER.EQ.0) CALL EZGET_rarr('BEAM_ERR',BEAM_ERR,IER)
        IF (IER.EQ.0) CALL EZGET_rarr('BEAM_SLOP',BEAM_SLOP,IER)
        IF (IER.EQ.0) CALL EZGET_rarr('BMSL_ERR',BMSL_ERR,IER)
        IF (IER.EQ.0) CALL EZGET_rarr('IP_SIGMA',IP_SIGMA, IER)
