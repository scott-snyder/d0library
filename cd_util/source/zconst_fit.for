...
C
      IF (FIRST) THEN
        ...
        CALL EZGET_l('DO_VTX_ITER',DO_VTX_ITER,IER)
        IF (IER.EQ.0) CALL EZGET('MAX_VTX_CHISQ_DIFF',MAX_DIFF,IER)
        IF (IER.EQ.0) CALL EZGET('BXYLMT',BXYLMT,IER)
        IF (IER.EQ.0) CALL EZGET_i('MAX_VTX_ITER',MAX_ITER,IER)
        IF (IER.EQ.0) CALL EZGET_i('MIN_VTX_TRACKS',MIN_TRACK,IER)
        IF (IER.EQ.0) CALL EZGET('MAX_VTX_XYCHISQ',MAX_CHISQ,IER)
        IF (IER.EQ.0) CALL EZGET_i('MIN_VTX_DLHIT',MIN_DLHIT,IER)
        ...
        DO ITRK = 1,NTRACK
          ...
          INDEX(ITRK) = 0             ! Reset bit
           IF (CHISQ.LT.PCHISQ) THEN
            ...
            EZV = TEZV
            IREJ = ITRK
            CALL UCOPY_i(TPID(1),CPID(1),NTRACK-1)
...
C
        NTRACK = NTRACK - 1
        CALL UCOPY_i(CPID(1),PID(1),NTRACK)
