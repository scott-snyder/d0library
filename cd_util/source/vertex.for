...
C------------------------------------------------------------------
      ...
      LOGICAL FIRST, VERTEX_ACCURATE,FIRST_CDC, VERTEX_MULTI
      integer method
      LOGICAL CDCON, FDCON
      LOGICAL MCDATA
...
C
     ...
     &    'Unable to find bank VERTEX_RCP','W')
          GOTO 999
        ENDIF
        CALL EZGET_i('VERTEX_TYPE',VERTEX_TYPE,IER)
        CALL EZGET_i('METHOD',METHOD,IER)
        CALL EZGET_l('FIRST_CDC',FIRST_CDC,IER)
        CALL EZGET_l('FIRST_FDC',FIRST_FDC,IER)
