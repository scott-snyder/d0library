      SUBROUTINE CDC_VERTEX(ZVERTX,ZERROR,WEIGHT,NTRK)
C----------------------------------------------------------------------
C-
...
C-   Updated  19-JUL-1995   Srini Rajagopalan  Adopted from ZVERTX_MULTI for
C-                          redoing vertex fit in vertex_fix package.
C-
C----------------------------------------------------------------------
      IMPLICIT NONE
      ...
      INTEGER I, ERR, IER, NEWHID, WEIGHT(3), IPATH
...
C
        ...
        CALL EZGET('CUTWEI',CUTWEI,ERR)
        CALL EZGET('MIN_ENTRY',MIN_ENTRY,ERR)
