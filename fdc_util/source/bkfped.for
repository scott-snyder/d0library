      SUBROUTINE BKFPED(LSUP,ND,LBANK,IFL)
C----------------------------------------------------------------------
C-
C-   Purpose and Methods : Create Forward Drift Chamber Pedestal bank
C-
C-   Inputs  : LSUP = Address of the supporting bank
C-             ND = length of the created bank
C-   Output  : LBANK = address of the created bank
C-   Control : IFL = 0 Book FPED with FPDH as supporting bank
C-             IFL = 1 Book FPED supported by previous bank (Linear str.)
C-
...
C
      ...
      ENDIF
C
      CALL MZFORM('FPED','30I -F',NIO)  ! Data Type
      IF (IFL.EQ.0) THEN
        CALL MZBOOK(IDVSTP,LBANK,LSUP,-IZFPED,'FPED',NL,NS,ND,NIO,0)
      ELSE
        CALL MZBOOK(IDVSTP,LBANK,LSUP,0,'FPED',NL,NS,ND,NIO,0)
      ENDIF
C
C ****  Get date and time 
C
