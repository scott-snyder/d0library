C VAX/DEC CMS REPLACEMENT HISTORY, Element LDFDCT_COS.FOR
C *1     4-NOV-1993 10:59:44 AVERY "FDC changes for v12 RECO"
C VAX/DEC CMS REPLACEMENT HISTORY, Element LDFDCT_COS.FOR
      SUBROUTINE LDFDCT_COS(QTRAK,QHSEC,HALF,LADDER)
C-----------------------------------------------------------------------
C
...
C-       Special version for cosmic ray format. 
C-       Extra words for t0 and associated parameters. 
C
C  Input:  QTRAK(1:32)   contains information on the fitted track
C                        candidate
...
C           QTRAK(25)    spare
C           QTRAK(26)    t0 from fit (ns)                      
C           QTRAK(27)    error on par(5), t0                   
C           QTRAK(28)    COV(1,5)                                       
C           QTRAK(29)    COV(2,5)                                       
C           QTRAK(30)    COV(3,5)                                       
C           QTRAK(31)    COV(4,5)                                       
...
C
C-   Created  xx-DEC-1988   Daria Zieminska  (LDFDCT).
...
C-   Updated   1-NOV-1993   Robert E. Avery  Cosmic ray version, (LDFDCT_COS).
C
C-----------------------------------------------------------------------
      IMPLICIT NONE
C
...
C
      ...
      INTEGER USED,USED2
C
      REAL QTRAK(32),QHSEC(3,34),FHSEC,FLADDER(0:2)
C
...
C
      TRKNUM=IQ(LKFTRH+2)
      IQ(LKFDCT-5)=TRKNUM
      CALL UCOPY(QTRAK,Q(LKFDCT+1),32)
      FHSEC=QTRAK(2)
C
