      SUBROUTINE PRFSG2( PRUNIT, KTSEG, NTSEG, CFL, IFL )
C----------------------------------------------------------------------
C-
C-   Purpose and Methods : Print bank FSG2
C-
...
C----------------------------------------------------------------------------
      ...
      IF(LSEG.LE.0) THEN
        WRITE(PRUNIT,*) ' FSG2 - NO SEGMENTS FOUND'
        GOTO 999
      ENDIF
  ...
  100 CONTINUE
C
  101 FORMAT(/,' FSG2  HALF UNIT SECT  NHITS  PHI   DR1     Z1',
     X'     SLOPE  INTER  CHISQ/DOF  IONIZ/HIT ')
  102 FORMAT(4I5,1X,I5,2X,2F6.2,2X,F7.2,2F7.2,2X,F7.3,5X,F7.3)
  103 FORMAT(/,' FSG2  - ',I4,'  SEGMENTS FOUND')
  104 FORMAT(/,' FSG2  HALF UNIT SECT  NHITS  PHI   DR1     Z1',
     X'     SLOPE  INTER  CHISQ/DOF  IONIZ/HIT',
