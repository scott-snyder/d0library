      FUNCTION FLFSEC(ZVTX,PHIMINI,PHIMAXI,THEMINI,THEMAXI)
C--------------------------------------------------------------------------
C
...
C-   Updated  17-AUG-1994   Susan K. Blessing   Add entry point to check
C-    the ON array for a particular sector.  Remove ON from call.  Change
C-    to a logical function so that the entry point can be a logical.
C--------------------------------------------------------------------------
      IMPLICIT NONE
C
...
C
  ...
  100 CONTINUE
      GO TO 999
C-------------------------------------------------------------------------
      ENTRY FSECTOR_IN_ROAD(H,U,Q,S)
C
      FSECTOR_IN_ROAD = .FALSE.
      IF (ON(H,U,Q,S)) THEN
        FSECTOR_IN_ROAD = .TRUE.
      END IF
C
      GO TO 999
C-------------------------------------------------------------------------
