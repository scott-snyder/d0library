...
C
      IF (INIT.EQ.0) THEN
        ...
        CALL EZGET_rarr('PULWEI_CUT',PULWEI_CUT,IER)
        CALL EZGET_iarr('PULMAX',PULMAX,IER)
        CALL EZGET_i('TIMCHA',TIMCHA,IER)
        CALL EZGET_i('SHIFTT',SHIFTT,IER)
        CALL EZGET_l('CLOSE_PEAK',CLOSE_PEAK,IER)
        CALL EZGET_i('RESET_THETA',RESET_THETA,IER)
        CALL EZGET_i('RESET_DL',RESET_DL,IER)
        CALL EZGET_i('RESET_PHI',RESET_PHI,IER)
        CALL EZGET_i('WIDE_THETA',WIDE_THETA,IER)
        CALL EZGET_i('WIDE_DL',WIDE_DL,IER)
        CALL EZGET_i('WIDE_PHI',WIDE_PHI,IER)
        CALL EZGET_l('DO_RISE',DO_RISE,IER)
        CALL EZGET_i('RUNTYPE',RUNTYPE,IER)
        CALL EZGET_i('PULSE_SHAPE_LEVEL',PSHLEV,IER)
        CALL EZGET_l('REDO_AREA',REDO_AREA,IER)
        CALL EZGET('VELOCT',VELOCT,IER)
        CALL EZGET('VELOCP',VELOCP,IER)
        CALL EZGET_i('HITS_PER_WIRE',HITS_PER_WIRE,IER)
        CALL EZGET_rarr('MIN_AVE_DIFF',MIN_AVE_DIFF,IER)
        CALL EZGET_l('GN_INI',GN_INI,IER)
        CALL EZGET_l('TM_INI',TM_INI,IER)
        CALL EZGET_l('FIXED_MAXBIN',FIXED_MAXBIN,IER)
C
