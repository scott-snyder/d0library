      SUBROUTINE FGNINI_OFFLINE(CRUN,DB_OPENED,IOK)
C----------------------------------------------------------------------
C-
C-   Purpose and Methods : Initialzation routine for offline gains
C-                         in logical format.
C-                         Called by FDBINI.
C-
...
C
      ...
      IF (FIRST) THEN
        ...
        CALL EZRSET
        CALL INTMSG(' Reading FDC Offline Gains banks.')
        FIRST = .FALSE.
      ENDIF
C
C Check to see if gain calibration has changed since last run. 
...
C
      ...
      IF ( (LDATA.LE.0) .OR. (LKEY.LE.0) ) THEN
        CALL ERRDB(' FGNINI_OFFLINE - Error in finding requested data ')
        IOK = .FALSE.
        GO TO 999
      ENDIF
...
C
      ...
      IF ( LFGNH.NE.0 ) THEN
        CALL MZDROP(IXSTP,LFGNH ,' ')
      ENDIF
C
      CALL ZSHUNT(IXSTP,LDATA,LSFDC,-IZFGNH,0)
      LFGNH = LC(LSFDC-IZFGNH)
C----------------------------------------------------------------------
  999 RETURN
      END
