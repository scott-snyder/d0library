...
C
      ...
      integer iqfsec(3,34)
      equivalence (qfsec, iqfsec)
      REAL CHI,QTRAK(26),XPOS,YPOS,ZPOS,BESDIF,ANS
c      real resid
...
C
      IF (FIRST) THEN    ! Book histograms
        ...
        CALL EZGET_i('MXHALF',MXHALF,ERR)
        CALL EZGET_i('MXUNIT',MXUNIT,ERR)
        CALL EZGET_i('MXQUAD',MXQUAD,ERR)
        CALL EZGET_i('MXSECT',MXSECT,ERR)
        CALL EZGET_i('MXSECP',MXSECP,ERR)
...
C
        ...
        DO 202 IHIT=1,IQTRAK(2)
          CALL UCOPY_i(iQFSEC(1,IHIT),IADD,1)
          CALL UCOPY_i(iQFSEC(2,IHIT),IPNTR,1)
c          CALL UCOPY(QFSEC(3,IHIT),RESID,1)
          PREV_W=W
