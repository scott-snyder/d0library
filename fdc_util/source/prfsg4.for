      SUBROUTINE PRFSG4( PRUNIT, KTSEG, NTSEG, CFL, IFL )
C----------------------------------------------------------------------
C-
C-   Purpose and Methods : Print bank FSG4
C-
...
C        CALL EZRSET
      ...
      IF(LSEG.LE.0) THEN
        WRITE(PRUNIT,*) ' FSG4 - NO SEGMENTS FOUND'
        GOTO 999
      ENDIF
  ...
  100 CONTINUE
C
  101 FORMAT(/,' FSG4  HALF UNIT QUAD SECT  NHITS   PHI ',
     & '  THETA  SLOPE  INTER  CHISQ/DOF  IONIZ/HIT')
  102 FORMAT(5I5,2X,I4,2X,F6.2,F6.2,F7.2,1X,F7.2,2X,F7.3,5X,F7.3)
  103 FORMAT(/,' FSG4  - ',I4,'  SEGMENTS FOUND')
  104 FORMAT(/,' FSG4  HALF UNIT QUAD SECT  NHITS  PHI ',
     & '  THETA  SLOPE  INTER  CHISQ/DOF  IONIZ/HIT 
