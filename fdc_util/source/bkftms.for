      SUBROUTINE BKFTMS(LSUP,ND,LBANK,IFL)
C----------------------------------------------------------------------
C-
C-   Purpose and Methods : Create Forward Drift Chamber Times bank 
C-
C-   Inputs  : LSUP = Address of the supporting bank
C-             ND = length of the created bank
C-   Output  : LBANK = address of the created bank
C-   Control : IFL = 0, Book FTMS with FTMH as supporting bank
C-             IFL = 1, Book FTMS as next bank in linear structure
C-
...
C
      ...
      ENDIF
C
      CALL MZFORM('FTMS','30I -F',NIO)  ! Data Type
      IF (IFL.EQ.0) THEN
        CALL MZBOOK(IDVSTP,LBANK,LSUP,-IZFTMS,'FTMS',NL,NS,ND,NIO,0)
      ELSE
        CALL MZBOOK(IDVSTP,LBANK,LSUP,0,'FTMS',NL,NS,ND,NIO,0)
      ENDIF
C
C ****  Get date and time 
C
