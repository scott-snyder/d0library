...
C
     ...
     &      ELE_TYPE,IER)
          IF (IER.EQ.0) CALL EZGETA_i('ELE_MASK',0,0,0,NELE_TYPE,IER)
          IF (IER.EQ.0) CALL EZGETA_iarr('ELE_MASK',1,NELE_TYPE,1
     &         ,ELE_MASK,IER)
          IF (IER.EQ.0) CALL EZGETA_i('ELE_MIN_ET',0,0,0,NELE_TYPE,IER)
          IF (IER.EQ.0) CALL EZGETA('ELE_MIN_ET',1,NELE_TYPE,1,
     &      ELE_MIN_ET,IER)
          IF (IER.EQ.0) CALL EZGETA_i('ELE_MAX_ETA',0,0,0,NELE_TYPE,IER)
          IF (IER.EQ.0) CALL EZGETA('ELE_MAX_ETA',1,NELE_TYPE,1,
     &      ELE_MAX_ETA,IER)
          IF (IER.EQ.0) CALL EZGETA_i('ELIKE_MASK_CC',
     &      0,0,0,NELE_TYPE,IER)
          IF (IER.EQ.0) CALL EZGETA_iarr('ELIKE_MASK_CC',
     &      1,NELE_TYPE,1,ELIKE_MASK_CC,IER)
          IF (IER.EQ.0) CALL EZGETA_i('ELE_MAX_LIKELIHOOD_CC',
     &      0,0,0,NELE_TYPE,IER)
          IF (IER.EQ.0) CALL EZGETA('ELE_MAX_LIKELIHOOD_CC',
     &      1,NELE_TYPE,1,ELE_MAX_LKLY_CC,IER)
          IF (IER.EQ.0) CALL EZGETA_i('ELIKE_MASK_EC',
     &      0,0,0,NELE_TYPE,IER)
          IF (IER.EQ.0) CALL EZGETA_iarr('ELIKE_MASK_EC',
     &      1,NELE_TYPE,1,ELIKE_MASK_EC,IER)
          IF (IER.EQ.0) CALL EZGETA_i('ELE_MAX_LIKELIHOOD_EC',
     &      0,0,0,NELE_TYPE,IER)
     ...
     &      PHOT_TYPE,IER)
          IF (IER.EQ.0) CALL EZGETA_i('PHOT_MASK',0,0,0,NPHOT_TYPE,IER)
          IF (IER.EQ.0) CALL EZGETA_iarr('PHOT_MASK',1,NPHOT_TYPE,1,
     &      PHOT_MASK,IER)
          IF (IER.EQ.0) CALL EZGETA_i('PHOT_MIN_ET',0,0,0,NPHOT_TYPE
     &         ,IER)
          IF (IER.EQ.0) CALL EZGETA('PHOT_MIN_ET',1,NPHOT_TYPE,1,
     &      PHOT_MIN_ET,IER)
          IF (IER.EQ.0) CALL EZGETA_i('PHOT_MAX_ETA',0,0,0,NPHOT_TYPE
     &         ,IER)
          IF (IER.EQ.0) CALL EZGETA('PHOT_MAX_ETA',1,NPHOT_TYPE,1,
     &      PHOT_MAX_ETA,IER)
          IF (IER.EQ.0) CALL EZGETA_i('PHOT_TRKSIG',0,0,0,NPHOT_TYPE
     &         ,IER)
          IF (IER.EQ.0) CALL EZGETA('PHOT_TRKSIG',1,NPHOT_TYPE,1,
     &      PHOT_TRKSIG,IER)
          IF (IER.EQ.0) CALL EZ_GET_CHARS('MUON_NAMES',NMUON_TYPE,
     &      MUON_TYPE,IER)
          IF (IER.EQ.0) CALL EZGETA_i('MUON_MASK',0,0,0,NMUON_TYPE,IER)
          IF (IER.EQ.0) CALL EZGETA_iarr('MUON_MASK',1,NMUON_TYPE,1,
     &      MUON_MASK,IER)
          IF (IER.EQ.0) CALL EZGETA_i('MUON_MIN_PT',0,0,0,NMUON_TYPE
     &         ,IER)
          IF (IER.EQ.0) CALL EZGETA('MUON_MIN_PT',1,NMUON_TYPE,1,
     &      MUON_MIN_PT,IER)
          IF (IER.EQ.0) CALL EZGETA_i('MUON_MAX_RGN',0,0,0,NMUON_TYPE
     &         ,IER)
          IF (IER.EQ.0) CALL EZGETA_iarr('MUON_MAX_RGN',1,NMUON_TYPE,1,
     &      MUON_MAX_RGN,IER)
          IF (IER.EQ.0) CALL EZGET('USE_MUON_MCNAMES',USE_MUON_MCNAMES,
     &      IER)
          IF (IER.EQ.0) CALL EZ_GET_CHARS('MC_MUON_NAMES',NMC_MUON_TYPE,
     &      MC_MUON_TYPE,IER)
          IF (IER.EQ.0) CALL EZGETA_i('MC_MUON_MASK',0,0,0,
     &      NMC_MUON_TYPE,IER)
          IF (IER.EQ.0) CALL EZGETA_iarr('MC_MUON_MASK',1,NMC_MUON_TYPE
     &         ,1,MC_MUON_MASK,IER)
          IF (IER.EQ.0) CALL EZGETA_i('MC_MUON_MIN_PT',0,0,0,
     &      NMC_MUON_TYPE,IER)
          IF (IER.EQ.0) CALL EZGETA('MC_MUON_MIN_PT',1
     &         ,NMC_MUON_TYPE,1,MC_MUON_MIN_PT,IER)
          IF (IER.EQ.0) CALL EZGETA_i('MC_MUON_MAX_RGN',0,0,0,
     &      NMC_MUON_TYPE,IER)
          IF (IER.EQ.0) CALL EZGETA_iarr('MC_MUON_MAX_RGN',1
     &         ,NMC_MUON_TYPE,1,MC_MUON_MAX_RGN,IER)
          IF (IER.EQ.0) CALL EZ_GET_CHARS('JET_NAMES',NJET_TYPE,
     &      JET_TYPE,IER)
          IF (IER.EQ.0) CALL EZGETA_i('JET_ALGORITHM',0,0,0,NJET_TYPE
     &         ,IER)
          IF (IER.EQ.0) CALL EZGETA_iarr('JET_ALGORITHM',1,NJET_TYPE,1,
     &      JET_ALGORITHM,IER)
          IF (IER.EQ.0) CALL EZGETA_i('JET_MIN_ET',0,0,0,NJET_TYPE,IER)
          IF (IER.EQ.0) CALL EZGETA('JET_MIN_ET',1,NJET_TYPE,1,
     &      JET_MIN_ET,IER)
          IF (IER.EQ.0) CALL EZGETA_i('JET_MAX_ETA',0,0,0,NJET_TYPE,IER)
          IF (IER.EQ.0) CALL EZGETA('JET_MAX_ETA',1,NJET_TYPE,1,
     &      JET_MAX_ETA,IER)
          IF (IER.EQ.0) CALL EZGETA_i('JET_MIN_DELTA_R',0,0,0,NJET_TYPE,
     &      IER)
