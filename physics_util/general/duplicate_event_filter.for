...
C-
        ...
        IF(IER.NE.0)THEN
          ...
          CALL EZPICK_NOMSG('DUPLICATE_EVENT_FILTER_RCP', IER)
        ENDIF
        IF(IER.EQ.0)CALL EZGET_l('DO_DUPLICATE_EVENT_FILTER',
     &    DO_DUPLICATE_EVENT_FILTER, IER)
        IF(IER.EQ.0.AND.DO_DUPLICATE_EVENT_FILTER)THEN
          IF(IER.EQ.0)CALL EZGET_l('FILTER_MONTECARLO',
     &         FILTER_MONTECARLO,IER)
          IF(IER.EQ.0)CALL EZGET_i('PRIMARY_ALLOCATION', ALLOC, IER)
          IF(IER.EQ.0)CALL EZGET_i('RECORD_LENGTH', LRECL, IER)
          IF(IER.EQ.0)CALL EZGET_l('UPDATE_RZFILE', UPDATE_RZFILE, IER)
