...
C
     ...
     >      'RCP not found','F')
C
        CALL EZGETA_i('FIRST_RUN',0,NTYPES_max,0,NTYPES,IERR)
        CALL EZGETA_iarr('FIRST_RUN',1,NTYPES,1,FIRST_RUN,IERR)
        NMAX_FAILED=MAX(NTYPES,NTYPES_MAX)
        IF(IERR.EQ.0)CALL EZGETA_iarr('LAST_RUN',1,NTYPES,1,LAST_RUN
     &       ,IERR)
        NMAX_FAILED=MAX(NTYPES,NTYPES_MAX)
