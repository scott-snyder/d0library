C DEC/CMS REPLACEMENT HISTORY, Element GETLUM.FOR
C *1    28-MAR-1995 20:18:59 GRUDBERG "instant lum"
C DEC/CMS REPLACEMENT HISTORY, Element GETLUM.FOR
      SUBROUTINE GETLUM(VAXTIME,LUM,AGE,ERR)
C----------------------------------------------------------------------
C-
...
C-                   -3 = requested time is too late (after run1c)
C-                   -4 = requested time is between run1a and run1b
C-                   -5 = requested time is between run1b and run1c
C-
...
C-   Updated  16-JAN-1996   Peter Grudberg  Add run 1c 
C-
C----------------------------------------------------------------------
      IMPLICIT NONE
...
C
     ...
     &    'D0$PHYSICS_UTIL$DATA:LUM_RUN1C.DAT' /
      DATA IUSER / 666 /
      DATA FIRST / .TRUE. /
C----------------------------------------------------------------------
      IF ( FIRST ) THEN
...
C
     ...
     &         DBL3TIME .LE. LAST_TIME(2) ) THEN
        I = 2                                 ! Run 1b
      ELSEIF ( DBL3TIME .GT. LAST_TIME(2) .AND. 
     &         DBL3TIME .LT. FIRST_TIME(3) ) THEN
        ERR = - 5
        CALL ERRMSG('No lum value', 'GETLUM',
     &    'Input time between run 1b and 1c', 'W')
        GO TO 999
      ELSEIF ( DBL3TIME .GE. FIRST_TIME(3) .AND. 
     &         DBL3TIME .LE. LAST_TIME(3) ) THEN
        I = 3                                 ! Run 1c
      ELSEIF ( DBL3TIME .GT. LAST_TIME(3) ) THEN
        ERR = - 3
        CALL ERRMSG('No lum value', 'GETLUM',
     &    'Input time too late (after run 1c)', 'W')
        GO TO 999
      ENDIF
C
