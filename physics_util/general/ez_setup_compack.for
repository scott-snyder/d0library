...
C
      CALL EZGET_l('ACTIVE',ACTIVE,IER)
C
      IF ( IER .EQ. 0 ) THEN
        IF ( .NOT. ACTIVE ) THEN
...
C
      IF ( FIRST ) THEN
        CALL EZGET_l('MENDEF',CALL_MENDEF,IIER)
...
C
      CALL EZGET_i('SETSTA',NUMBER_LINES,IIER)
      IF ( .NOT. EZERROR(IIER) ) THEN
        CALL SETSTA (NUMBER_LINES)
      ENDIF
C
      CALL EZGET_i('SETLIN',NUMBER_LINES,IIER)
      IF ( .NOT. EZERROR(IIER) ) THEN
        CALL SET_SPLIT_SCREEN (NUMBER_LINES)
      ENDIF
C
      CALL EZGET_i('DEFSPA',NUMBER_LINES,IIER)
      IF ( .NOT. EZERROR(IIER) ) THEN
        CALL DEFSPA (NUMBER_LINES)
      ENDIF
C
      CALL EZGET_i('DEFCOL',NUMBER_LINES,IIER)
      IF ( .NOT. EZERROR(IIER) ) THEN
