...
C
      IF(FIRST) THEN
        IER = 0
C
C *** Muon id - from SS_MU_SEL_RCP -
C
        CALL INRCP('SS_MU_SEL_RCP',IER)
        IF(IER.NE.0)
     1    CALL ERRMSG('SS_MU_SEL_RCP not found',
     2      'SS_MU_SEL',' ','W')
        CALL EZPICK('SS_MU_SEL_RCP')
        CALL ERRMSG('Reading SS_MU_SEL_RCP',
     1      'SS_MU_SEL',' ','W')
C
C *** Muon id cuts
C
...
C
        ...
        IF (IER.NE.0) CALL ERRMSG('Error in SS_MU_SEL_RCP',
     &    'SS_MU_SEL',' ','F')
C
        CALL EZRSET
        FIRST=.FALSE.
