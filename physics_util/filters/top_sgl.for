...
C----------------------------------------------------------------------
      IF(FIRST) THEN
        FIRST=.FALSE.
        CALL INRCP('TOP_SGL_RCP',IER)
        IF(IER.EQ.0) THEN
          CALL EZPICK('TOP_SGL_RCP')
        ELSE
          CALL ERRMSG('TOP_SGL','TOP_SGL_RCP',
     &      'ERROR READING TOP_SGL RCP FILE','F')
        ENDIF
      ENDIF
C
C ***   intializations
