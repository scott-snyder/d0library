       SUBROUTINE MULQ_MUON_SELECT(LPMUO,STATUS,OK)
C----------------------------------------------------------------------
C-
...
C
      IF(FIRST) THEN
        IER = 0
C
C *** Muon id - from MULQ_MUON_SELECT_RCP -
C
        CALL INRCP('MULQ_MUON_SELECT_RCP',IER)
        IF(IER.NE.0)
     1    CALL ERRMSG('MULQ_MUON_SELECT_RCP not found',
     2      'MULQ_MUON_SELECT',' ','W')
        CALL EZPICK('MULQ_MUON_SELECT_RCP')
        CALL ERRMSG('Reading MULQ_MUON_SELECT_RCP',
     1      'MULQ_MUON_SELECT',' ','W')
C
C *** Muon id cuts
C
...
C
        ...
        CALL EZRSET
        IF (IER.NE.0) CALL ERRMSG('Error in MULQ_MUON_SELECT_RCP',
     &    'MULQ_MUON_SELECT',' ','F')
        FIRST=.FALSE.
      ENDIF
C
