      SUBROUTINE MUON_SELECT_NP(LPMUO)
C----------------------------------------------------------------------
C-
...
C
      IF(FIRST) THEN
        IER = 0
C
C *** Muon id - from MUON_SELECT_NP_RCP -
C
        CALL INRCP('MUON_SELECT_NP_RCP',IER)

        IF(IER.NE.0) THEN
          CALL ERRMSG('MUON_SELECT_NP_RCP not found',
     &       'MUON_SELECT_NP', ' ','F')
        ENDIF

        CALL EZPICK('MUON_SELECT_NP_RCP')
C
C *** Muon id cuts
C
        CALL EZGET_l('DO_MUCTAG_CUT',DO_MUCTAG_CUT,IER)

        IF(DO_MUCTAG_CUT) THEN
          IF(IER.EQ.0)
     &      CALL EZGET_i('IFW2_BIT7_CUT',IFW2_BIT7_CUT,IER)
          IF(IER.EQ.0)
     &      CALL EZGET_i('IFW2_BIT8_CUT',IFW2_BIT8_CUT,IER)
        ENDIF

        IF(IER.EQ.0)
     &    CALL EZGET_l('DO_OCTANT_CUT',DO_OCTANT_CUT,IER)
        IF(DO_OCTANT_CUT) THEN
          IF(IER.EQ.0)
     &      CALL EZGET_i('IFW2_BIT9_CUT',IFW2_BIT9_CUT,IER)
        ENDIF
C
        IF(IER.EQ.0)
     &    CALL EZGET_l('DO_IFW4_CUT',DO_MUIFW4_CUT,IER)
        IF(DO_MUIFW4_CUT) THEN
          IF(IER.EQ.0)
     &      CALL EZGET_i('IFW4_CUT',IFW4_CUT,IER)
        ENDIF
C
        IF(IER.EQ.0)
     &    CALL EZGET_l('DO_IFW1_CUT',DO_MUIFW1_CUT,IER)
        IF(DO_MUIFW1_CUT) THEN
          IF(IER.EQ.0)
     &      CALL EZGET_i('IFW1_NOMISS_A',IFW1_CUT_A,IER)
          IF(IER.EQ.0)
     &      CALL EZGET_i('IFW1_NOMISS_B',IFW1_CUT_B,IER)
          IF(IER.EQ.0)
     &      CALL EZGET_i('IFW1_NOMISS_C',IFW1_CUT_C,IER)
        ENDIF
C
        IF(IER.EQ.0)
     &    CALL EZGET_l('DO_CDCUT',DO_MUCD_CUT,IER)
        IF(DO_MUCD_CUT) THEN
          IF(IER.EQ.0)
     1      CALL EZGET('MAX_ZTRAK_DTHETA',
     1      MU_MAX_ZTRAK_DTHETA,IER)
          IF(IER.EQ.0)
     1      CALL EZGET('MAX_ZTRAK_DPHI',
     1      MU_MAX_ZTRAK_DPHI,IER)
        ENDIF
C
        IF(IER.EQ.0)
     &    CALL EZGET_l('DO_IMPACT_CUT',DO_IMPACT_CUT,IER)
        IF(DO_IMPACT_CUT) THEN
          IF(IER.EQ.0)
     &      CALL EZGET('MAX_IMPACT',IMPACT_MU_MAX,IER)
        ENDIF
C
        IF(IER.EQ.0)
     &    CALL EZGET_l('DO_CALDEP_CUT',DO_CALDEP_CUT,IER)
        IF(DO_CALDEP_CUT) THEN
          IF(IER.EQ.0)
     &      CALL EZGET('MIN_ENERGY_02',MIN_ENERGY_02,IER)
        ENDIF
        IF(IER.EQ.0)
     &    CALL EZGET_l('DO_BDL_CUT',DO_BDL_CUT,IER)
        IF(DO_BDL_CUT) THEN
          IF(IER.EQ.0)
     &      CALL EZGET('MIN_BDL',MIN_INTEG_BDL,IER)
        ENDIF
        CALL EZGET_l('DO_PT_CUT',DO_PT_CUT,IER)
        IF (DO_PT_CUT) CALL EZGET('PT_CUT',PT_CUT,IER)
        IF (DO_PT_CUT) CALL EZGET('PT_CUT2',PT_CUT2,IER)
        CALL EZGET_l('DO_ETA_CUT',DO_ETA_CUT,IER)
        IF (DO_ETA_CUT) CALL EZGET('ETA_CUT',ETA_CUT,IER)
        IF (IER.NE.0) CALL ERRMSG('Error in MUON_SELECT_NP_RCP',
     &    'MUON_SELECT',' ','F')
C
        CALL EZRSET
        FIRST=.FALSE.
