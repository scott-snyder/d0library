      LOGICAL FUNCTION NP_WRIGHT_TIGHT ()
C----------------------------------------------------------------------
C-
...
C-   Inputs  : none
C-   Outputs : none
C-   Controls: control parameters in NP_WRIGHT_TIGHT_RCP
C-
C-
...
C----------------------------------------------------------------------
      LOGICAL  FIRST, OK
      LOGICAL  NP_WRIGHT_TIGHT_EOJ,NP_WRIGHT_TIGHT_STA,
     &  NP_WRIGHT_TIGHT_SUM
      DATA     FIRST /.TRUE./
...
C----------------------------------------------------------------------
      IF ( FIRST ) THEN
        ...
        CALL INRCP ('NP_WRIGHT_TIGHT_RCP', IER)
C
        IF ( IER .NE. 0 ) THEN
          CALL ERRMSG ('No NP_WRIGHT_TIGHT_RCP', 'NP_WRIGHT_TIGHT',
     &      'Could not find NP_WRIGHT_TIGHT_RCP', 'F')
        ENDIF
        CALL EZPICK ('NP_WRIGHT_TIGHT_RCP')
...
C
      NP_WRIGHT_TIGHT = .FALSE.                 ! default action -- reject event
C
      ...
      NP_WRIGHT_TIGHT = .TRUE.
 999  RETURN
C
      ENTRY NP_WRIGHT_TIGHT_EOJ(RSUM)
C
      NP_WRIGHT_TIGHT_EOJ = .TRUE.
      DO I = 1, 20
        RSUM(I) = RSUMMARY(I)
      ENDDO
      RETURN
      ENTRY NP_WRIGHT_TIGHT_STA()
      MSG=' '
        WRITE(MSG,120)RSUMMARY(1),RSUMMARY(2),RSUMMARY(3),RSUMMARY(3)
     &    /RSUMMARY(1)*100.
        CALL INTMSG(MSG)
      RETURN
      ENTRY NP_WRIGHT_TIGHT_SUM()
      MSG=' '
        WRITE(MSG,120)RSUMMARY(1),RSUMMARY(2),RSUMMARY(3),RSUMMARY(3)
     &    /RSUMMARY(1)*100.
        CALL INTMSG(MSG)
