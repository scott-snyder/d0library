...
C-   Inputs  : none
C-   Outputs :
C-   Controls: TOP_TOP_MUMU RCP file
...
C
      LOGICAL COSREJ1,XOCT1,PASSED,FIRST,TOP_TOP_MUMU_EOJ
      LOGICAL COSREJ2,XOCT2,PASSED1,PASSED2,PASSED3
      ...
      REAL DIMU_DPHI,DIMU_DETAMAX,MUCTAG_TOT
      ...
      REAL DPHI,DTHETA,CHISQ,BDL,BDL_CUT
...
C
C   Get parameters from TOP_TOP_MUMU.RCP
        CALL INRCP('TOP_TOP_MUMU_RCP',IER)
        IF (IER.NE.0) THEN
          CALL ERRMSG('TOP_TOP_MUMU','TOP_TOP_MUMU_RCP',
     &        'ERROR READING TOP_TOP_MUMU RCP FILE','W')
          STOP 'TOP_TOP_MUMU_RCP not found'
        ELSE
          CALL EZPICK('TOP_TOP_MUMU_RCP')
C parameters for first (tight) muon
          CALL EZGET('ETA_MUZ1', ETACUT1, IER)
          CALL EZGET('QUAD_MUZ1', QUADCUT1, IER)
          ...
          CALL EZGET('CDMTCH_EF_MUZ1', CDMTCH_EF, IER)
...
C parameters for second (loose) muon
          ...
          CALL EZGET('QUAD_MUZ2', QUADCUT2, IER)
          ...
          IF (IER.NE.0) THEN
            CALL ERRMSG('TOP_TOP_MUMU','TOP_TOP_MUMU_RCP',
     &        'ERROR GETTING TOP_TOP_MUMU RCP VALUES','W')
          ENDIF
          CALL EZRSET
...
c          IF(NTIGHT_HIGHPT.GE.2) THEN
c            DIMU_DPHI=ABS(PHIMU_SAVE(1)-PHIMU_SAVE(2))
c            IF(DIMU_DPHI.GT.PI) DIMU_DPHI=TWOPI-DIMU_DPHI
c            DIMU_DETAMAX=ABS(ETAMU_SAVE(1)+ETAMU_SAVE(2))
c            IF(DIMU_DPHI.LE.2.70.OR.DIMU_DETAMAX.GT.0.25) PASSED1=.TRUE.
c            MUCTAG_TOT=MUCTAG_SAVE(1)+MUCTAG_SAVE(2)
c            IF(MUCTAG_TOT.EQ.0) PASSED1=.TRUE.
c          ENDIF
          passed1 = .true.
        ENDIF
        IF(NGOOD_LOOSE.GE.2 .AND. NJETS.GT.0 ) PASSED2 = .TRUE.
        IF(NGOOD_LOOSE.GE.2 ) THEN
          IF(FILTER_MUJET) PASSED3 = .TRUE.
        ENDIF
        IF(PASSED1.OR.PASSED2.OR.PASSED3) PASSED = .TRUE.
...
C
C----------------------------------------------------------------------
      ENTRY TOP_TOP_MUMU_EOJ(MU1_SUMRY)
C----------------------------------------------------------------------
C-
