...
C
      ...
      INTEGER NGOODMUON,NGOODELEC,NGOODPHO
      INTEGER JBIT,COSMIC
      INTEGER IFW4
C
      REAL P_GMUO(5,20)
...
C----------------------------------------------------------------------
      ...
      NP_LSS_TRILEP_LOOSE=.FALSE.
C
      IF ( FIRST ) THEN
        FIRST=.FALSE.
C
        CALL INRCP ('NP_LSS_LOOSE_2_RCP', IER)
        IF (IER.EQ.0) THEN
          CALL EZPICK ('NP_LSS_LOOSE_2_RCP')
          ...
          CALL EZRSET
        ELSE
          CALL ERRMSG ('No NP_LSS_LOOSE_2_RCP', 'NP_LSS_TRILEP_LOOSE',
     &        'Could not find NP_LSS_LOOSE_2_RCP', 'F')
        ENDIF                           ! if ier .eq. 0
C
...
C...  PHOTONS
      ...
      DO WHILE(LPPHO.NE.0)
        ...
        IF(Q(LPPHO+17).NE.0) THEN
          X_ISO=(Q(LPPHO+16)-Q(LPPHO+17))/Q(LPPHO+17)
        ELSE
          X_ISO=-1.
        ENDIF
C 
        ...
        IF(PTXX.GT.G_PT_CUT.AND.
     &     X_ISO.LE.EM_ISO_CUT.AND.
     &     HCHSQ.LE.EM_HCHSQ_CUT) THEN
