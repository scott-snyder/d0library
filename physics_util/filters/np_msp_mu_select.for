      SUBROUTINE NP_MSP_MU_SELECT(LPMUO)
C----------------------------------------------------------------------
C-
...
C------------------------------------------------------------------------
      IF(FIRST) THEN
        IER = 0
C
C *** Muon id - from NP_MSP_MU_SELECT_RCP -
C
        CALL INRCP('NP_MSP_MU_SELECT_RCP',IER)
        IF(IER.NE.0)
     1    CALL ERRMSG('NP_MSP_MU_SELECT_RCP not found',
     2      'NP_MSP_MU_SELECT',' ','W')
        CALL EZPICK('NP_MSP_MU_SELECT_RCP')
        CALL ERRMSG('Reading NP_MSP_MU_SELECT_RCP',
     1      'NP_MSP_MU_SELECT',' ','W')
C
C *** Muon id cuts
C
        CALL EZGET_l('MU_DO_MUCTAG_CUT',DO_MUCTAG_CUT,IER)
        IF(DO_MUCTAG_CUT) THEN
          IF(IER.EQ.0) CALL EZGET_i('MU_IFW2_BIT7_CUT',IFW2_BIT7_CUT
     &         ,IER)
          IF(IER.EQ.0) CALL EZGET_i('MU_IFW2_BIT8_CUT',IFW2_BIT8_CUT
     &         ,IER)
        ENDIF
        IF(IER.EQ.0) CALL EZGET_l('MU_DO_OCTANT_CUT',DO_OCTANT_CUT,IER)
        IF(DO_OCTANT_CUT) THEN
          IF(IER.EQ.0) CALL EZGET_i('MU_IFW2_BIT9_CUT',IFW2_BIT9_CUT
     &         ,IER)
        ENDIF
C
...
C
        ...
        ENDIF
        CALL EZGET_l('MU_DO_PT_CUT',DO_PT_CUT,IER)
        IF (DO_PT_CUT) CALL EZGET('MU_PT_CUT',PT_CUT,IER)
        CALL EZGET_l('MU_DO_ETA_CUT',DO_ETA_CUT,IER)
        IF (DO_ETA_CUT) CALL EZGET('MU_ETA_CUT',ETA_CUT,IER)
        IF (IER.NE.0) CALL ERRMSG('Error in NP_MSP_MU_SELECT_RCP',
     &    'NP_MSP_MU_SELECT',' ','F')
C
        CALL EZRSET
        FIRST=.FALSE.
