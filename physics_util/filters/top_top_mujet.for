      LOGICAL FUNCTION TOP_TOP_MUJET
C----------------------------------------------------------------------
C-
...
C-   Inputs  : none
C-   Outputs :
C-   Controls: TOP_TOP_MUJET RCP file
...
C-    above threshold when forming the isolation variable.
C-
C----------------------------------------------------------------------
      IMPLICIT NONE
...
C----------------------------------------------------------------------
      IF (FIRST) THEN
C
C   Get parameters from TOP_TOP_MUJET.RCP
        CALL INRCP('TOP_TOP_MUJET_RCP',IER)
        IF (IER.NE.0) THEN
          CALL ERRMSG('TOP_TOP_MUJET','TOP_TOP_MUJET_RCP',
     &        'ERROR READING TOP_TOP_MUJET RCP FILE','W')
          STOP 'TOP_TOP_MUJET_RCP not found'
        ELSE
...
C done with defining all the selection cuts.
          IF (IER.NE.0) THEN
            CALL ERRMSG('TOP_TOP_MUJET','TOP_TOP_MUJET_RCP',
     &        'ERROR GETTING TOP_TOP_MUJET RCP VALUES','W')
          ENDIF
          CALL EZRSET
...
c now all the selections for signal/QCD/cosmic are done.
  ...
  900 IF (PASSED_FAKE .OR. PASSED_MU )THEN
      ...
      ENDIF
      CALL RESET_CAPH
      TOP_TOP_MUJET = PASSED_ALL
C
  999 RETURN
C----------------------------------------------------------------------
      ENTRY TOP_TOP_MUJET_EOJ(MUJ_SUMRY)
C----------------------------------------------------------------------
C-
