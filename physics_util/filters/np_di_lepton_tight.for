...
C
      INTEGER IER,IFW4,JBIT,IUPLIM,I,J,IUNIT,IERR,LMUOT
      INTEGER NMUON,LPMUO,GZPMUO,NELEC,LPELC,GZPELC
      INTEGER LPNUT2,GZPNUT,LHMTE,LHMTP,MU_IFW4_CUT
      INTEGER NGOODMUON,NGOODELEC,OCTANT_CROSS,CD_TRACKS,NCOSMIC
C
      PARAMETER (IUPLIM = 20)
C
      LOGICAL FIRST,DO_OCTANT_CUT,PASS_OCT_CUT,NP_DI_LEPTON_TIGHT,EZERR
C
      REAL X_ISO,PTEM,ANG,EXMISS,EYMISS,MU_CORR_MET,DEL_PHI
      REAL E_PT_CUT,MU_PT_CUT,G_PT_CUT,EM_ISO_CUT,EM_HCHSQ_CUT,HCHSQ
...
C
      DATA FIRST/.TRUE./
C----------------------------------------------------------------------
      NP_DI_LEPTON_TIGHT=.FALSE.
C
      IF ( FIRST ) THEN
        FIRST=.FALSE.
        CALL INRCP ('NP_DI_LEPTON_TIGHT_RCP', IER)
        IF (IER.EQ.0)  CALL EZPICK('NP_DI_LEPTON_TIGHT_RCP')
        IF (.NOT.EZERR(IER)) THEN
          ...
          CALL EZGET_i ('MU_IFW4_CUT',    MU_IFW4_CUT,    IER)
          ...
          CALL EZRSET
        ELSE
          CALL ERRMSG ('No NP_DI_LEPTON_TIGHT_RCP', 
     &'NP_DI_LEPTON_TIGHT',
     &        'Could not find NP_DI_LEPTON_TIGHT_RCP', 'F')
        ENDIF                          ! if ier .eq.
      ENDIF                            ! if first
C
C     GET MISSING ET
...
C
     ...
     &        *GOODMU_P(4,J))
            IF (ANG.LT.-1.) ANG = -1.
            IF (ANG.GT. 1.) ANG =  1.
            IF(ACOS(ANG).GE.MU_COL_CUT) NCOSMIC = NCOSMIC + 1
          ENDDO
        ENDDO
      ENDIF
...
C
     ...
     &            .OR.((NGOODMUON.GE.1).AND.(NGOODELEC.GE.1)
