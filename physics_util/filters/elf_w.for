      LOGICAL FUNCTION ELF_W() 
C----------------------------------------------------------------------
C-
C-   Purpose and Methods : Filter W events from ELF stream according to 
C-                         the following criteria 
C-     - PELC  with pt > ELEC_MIN_ET 
C-     - PPHO  with pt > PHOT_MIN_ET 
C-     - PNUT2 with pt > ETMISS_MIN 
C-
C-   Controls: ELF_W_RCP 
...
C
      IF(FIRST) THEN
        FIRST=.FALSE.
        CALL INRCP('ELF_W_RCP',IER)       
        IF(IER.EQ.0) THEN
          CALL EZPICK('ELF_W_RCP')  
          CALL EZGET('ELEC_MIN_ET',ELETMIN,IER)
          CALL EZGET('PHOT_MIN_ET',PHETMIN,IER)
          CALL EZGET('ETMISS_MIN', ETMISSMIN,IER)
          IF (IER.NE.0) THEN
            CALL ERRMSG('ELF_W','ELF_W_RCP',
     &        'ERROR GETTING ELF_W RCP VALUES','W')
          ENDIF
          CALL EZRSET
          CALL VZERO(W_STAT,20)
        ELSE
          CALL ERRMSG('ELF_W','ELF_W_RCP',
     &      'ERROR READING ELF_W RCP FILE','F')
        ENDIF
      ENDIF
C
C------------------------------------------------------------------------------
