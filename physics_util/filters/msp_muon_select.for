       SUBROUTINE MSP_MUON_SELECT(LPMUO,STATUS,OK)
C----------------------------------------------------------------------
C-
...
C
      IF(FIRST) THEN
        IER = 0
C
C *** Muon id - from MSP_CLEANMU_RCP -
C
        CALL INRCP('MSP_CLEANMU_RCP',IER)
        IF(IER.NE.0)
     1    CALL ERRMSG('MSP_CLEANMU_RCP not found',
     2      'MSP_MUON_SELECT',' ','W')
        CALL EZPICK('MSP_CLEANMU_RCP')
        CALL ERRMSG('Reading MSP_CLEANMU_RCP',
     1      'MSP_MUON_SELECT',' ','S')
C
C *** Muon id cuts
C
...
C
        ...
        IF(IER.EQ.0)
     &    CALL EZGETA('MAX_IFW4_LOOSE',1,N,1,MAX_IFW4_LOOSE,IER)
C
        IF(IER.EQ.0)
...
C
        IF(IER.EQ.0) CALL EZGETA('MIN_MTC_HFRAC',0,0,0,N,IER)
        IF(IER.EQ.0) CALL EZGETA('MIN_MTC_HFRAC',1,N,1,
     1    MIN_MTC_HFRAC,IER)
C
        IF(IER.EQ.0) CALL EZGETA('MIN_MTC_FRAC',0,0,0,N,IER)
...
C
        CALL EZRSET
        IF (IER.NE.0) CALL ERRMSG('Error in MSP_CLEANMU_RCP',
     &    'MSP_MUON_SELECT',' ','F')
        FIRST=.FALSE.
      ENDIF
C
