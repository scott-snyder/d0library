...
C
      LMTRH=GZMTRH()
      IF (LMTRH.GT.0) THEN
        CALL SPLIT_BITMASK(iQ(LMTRH+2),OUT)
        MUSTP1_ = OUT(1)
        MUSTP1  = OUT(2)
        CALL SPLIT_BITMASK(iQ(LMTRH+3),OUT)
        MUSTP2_ = OUT(1)
        MUSTP2  = OUT(2)
        CALL SPLIT_BITMASK(iQ(LMTRH+4),OUT)
        MUSTP3_ = OUT(1)
        MUSTP3  = OUT(2)
        CALL SPLIT_BITMASK(iQ(LMTRH+5),OUT)
        MUSTP4_ = OUT(1)
        MUSTP4  = OUT(2)
        CALL SPLIT_BITMASK(iQ(LMTRH+8),OUT)
        L15M1_  = OUT(1)
        L15M1   = OUT(2)
        CALL SPLIT_BITMASK(iQ(LMTRH+9),OUT)
        L15M2_  = OUT(1)
        L15M2   = OUT(2)
        CALL SPLIT_BITMASK(iQ(LMTRH+10),OUT)
        L15M3_  = OUT(1)
        L15M3   = OUT(2)
      ENDIF
C
