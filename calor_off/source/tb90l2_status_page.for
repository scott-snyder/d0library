...
C
 ...
 2240   FORMAT(5X,'NO BINF RCP BANK')
      ...
      ELSE
        ...
        CALL EZGET_i('LOG_BOOK_NUMBER',        LOG_BOOK,  IER)
...
C######CORRECTIONS TO CALOR HIST RCP READ OUT
          ...
          CALL EZGET_rarr('TB90L2EM_RANGE',EM_RANGE,IER)
          CALL EZGET_rarr('TB90L2FH_RANGE',FH_RANGE,IER)
 ...
 1600       CONTINUE
          ELSE
            CALL EZGET_rarr('TB90L2CH_RANGE',CH_RANGE,IER)
            CALL EZGET_rarr('TB90L2MH_RANGE',MH_RANGE,IER)
            CALL EZGET_rarr('TB90L2OH_RANGE',OH_RANGE,IER)
            CALL EZGET_rarr('TB90L2ICD_RANGE',ICD_RANGE,IER)
          ENDIF
...
C######CHANGE CALOR_UNPACK TO CAHITS
  ...
  296     FORMAT(5X,' NO CAHITS_RCP BANK')
        ELSE
          ...
          CALL EZGET_l('DO_ZERO_SUPRESS',ZSUP,IER)
          ...
          CALL EZGET_l('DO_ADC_TO_GEV',ADCGEV,IER)
          CALL EZGET_rarr('SAMPLING_FRACTIONS',SFRAC_CORR,IER)
          CALL EZGET_l('DO_ADDR_CHECK',ADDR_CHECK,IER)
...
C#####NEW FANOUT CURRENTS
 ...
 1030     FORMAT(5X,'NO FANOUT CURRENT BANK')
        ELSE
          CALL EZGET_rarr('FANOUT_CURR',FAN_CURR,IER)
          WRITE(NUNIT,1035)
...
C######ARGON TEMPERATURE PRINTOUT
  ...
  810     FORMAT(5X,'NO TB_LAR_TEM_RCP')
        ELSE
          CALL EZGET_i('TB_MONTH_TEMP',MONTH_TEMP,IER)
          CALL EZGET_i('TB_DAY_TEMP',DAY_TEMP,IER)
          CALL EZGET_i('TB_HOUR_TEMP',HOUR_TEMP,IER)
...
C        IF(IER.NE.0) DO_CONNECTED = .TRUE.
        ...
        CALL EZGET_l('SUM_F_CURRENT',DO_F_CURRENT,IER)
        CALL EZGET_l('SUM_DBM_SPAGE',DO_DBM_SPAGE,IER)
        CALL EZGET_l('SUM_LAR_TEMP',DO_LAR_TEMP,IER)
        CALL EZGET_l('SUM_LAR_PUR',DO_LAR_PUR,IER)
        CALL EZGET_l('SUM_ERR',DO_ERR,IER)
        CALL EZRSET
      END IF
C******************************************************************
