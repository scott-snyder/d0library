...
C
      IF(FIRST)THEN                     ! LOCAL INIT
        ...
        CALL EZGET_l('RERUN_CAPHEL',RERUN,IER)
        ...
        CALL EZGET_l('SELECTIVE_EC_TRACKING',SELECTIVE_EC_TRACKING,IER)
        CALL EZGET_l('SELECTIVE_CC_TRACKING',SELECTIVE_CC_TRACKING,IER)
        CALL EZGET_l('SELECTIVE_VERTLOOP',SELECTIVE_VERTLOOP,IER)
        ...
        CALL EZGET_l('APPLY_CHISQ_CUTOFF',CHISQ_CUTOFF,IER)
        CALL EZGET('CHISQ_ECTRK_THR',CHISQ_ECTRK_THR,IER)
        CALL EZGET('CHISQ_CCTRK_THR',CHISQ_CCTRK_THR,IER)
        CALL EZGET_l('DO_PPHO_IN_TRD',DO_PPHO_IN_TRD,IER)
        IF (IER.NE.0) THEN
     ...
     &        'problem reading CAPHEL RCP parameters','F')
        ...
        CALL EZGET_l('ALL_TRACKS',ALL_TRACKS,IER)
        CALL EZRSET
