...
C
        CALL EZGETA_iarr('ETA_PHI_CELL',0,0,0,NCELLS,IER)
...
C
        CALL EZGET_iarr('ETA_PHI_CELL',ETA_PHI_CELL,IER)
C
        BWID = 0.0
     ...
     &    'NUMBER OF MOMENTUM CELLS EXCEEDS DIMENSION ALLOWED ','W')
        CALL EZGET_rarr('PARTICLE_MOMENTUM',MOMSPEC,IER)
...
C
   ...
   11   CONTINUE
C
        CALL EZGET_i('PARTICLE_ID',PARTICLE_ID,IER)
CCC        CALL EZGET('PARTICLE_MOM',PARTICLE_MOM,IER)
        CALL EZGET_rarr('PRIMARY_VERTEX',VERTEX,IER)
