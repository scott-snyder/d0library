...
C
          ...
          DO K = 1 , NSTREAM
            ...
            CALL EZGETA('STREAM_DESCRIP',ISTART+2,ISTART+6,1,LVAL,
     &        STATUS)
  ...
  103         FORMAT(A3,'_L1BITS')
              ...
              CALL EZGET(ARRAY_NAME,L1BITS,STATUS)
              DO I = 1,NBITS
                J = L1BITS(I)
                L1_BIT_REQ(J+1,K) = 1 
              ENDDO
  ...
  104         FORMAT(A3,'_L2BITS')
              CALL EZGETA(ARRAY_NAME,0,0,0,NBITS,STATUS)
              CALL EZGET(ARRAY_NAME,L2BITS,STATUS)
              DO I = 1,NBITS
                J = L2BITS(I)
                L2_BIT_REQ(J+1,K) = 1 
              ENDDO
...
C
        DO K = 1 , NSTREAM
          ...
          CALL EZGETC('STREAM_DESCRIP',ISTART,5,STREAM_NAME(K),STATUS)
          CALL EVGET_STREAM(STREAM_NAME(K),ISTREAM(K))
          CALL EZGETA('STREAM_DESCRIP',ISTART+2,ISTART+6,1,LVAL,STATUS)
          STRIP_ON_L1(K) = LVAL(1)
          STRIP_ON_L2(K) = LVAL(2)
          STRIP_ON_BOTH(K) = LVAL(3)
          USER_ONLY(K) = LVAL(4)
