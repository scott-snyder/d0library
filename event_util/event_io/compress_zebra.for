...
C-
        ...
        IF(IER.NE.0)THEN
          ...
          CALL EZPICK_NOMSG('COMPRESS_ZEBRA_RCP', IER)
        ENDIF
        IF(IER.EQ.0)CALL EZGET_l('DO_COMPRESS_ZEBRA', DO_COMPRESS_ZEBRA,
     &    IER)
        IF(IER.EQ.0)CALL EZGET_i('COMP_ALG', COMP_ALG, IER)
        ...
        IF(IER.EQ.0)CALL EZGET_i('RECORD_SIZE', RECORD_SIZE, IER)
        IF(IER.EQ.0)CALL EZGET_i('MIN_BUFFER_SIZE', MIN_BUFFER_SIZE,
     &       IER)
        IF(IER.EQ.0)CALL EZGET_i('MAX_BUFFER_SIZE', MAX_BUFFER_SIZE,
     &       IER)
        IF(IER.EQ.0)CALL EZGET_l('STA_MODE', STA_MODE, IER)
        IF(IER.EQ.0)CALL EZGET_l('DST_MODE', DST_MODE, IER)
        IF(IER.EQ.0)CALL EZGET_l('ROUND_FLOATING', ROUND, IER)
        IF(IER.EQ.0)CALL EZGET_i('FLOATING_BITS', FLOATING_BITS, IER)
        IF(IER.EQ.0)CALL EZGET_i('ZIP_PACK_LEVEL', ZIP_PACK_LEVEL, IER)
        IF(IER.EQ.0)CALL EZGET_I('LZSS_INDEX_BITS', LZSS_INDEX_BITS,
     &       IER)
        IF(IER.EQ.0)CALL EZGET_i('LZSS_LENGTH_BITS', LZSS_LENGTH_BITS, 
     &    IER)
        IF(IER.EQ.0)CALL EZGET_i('LZW_DICTIONARY_BITS', 
