      SUBROUTINE VALUES (STR,NUMBER,TYPE,NUMS)
C----------------------------------------------------------------------
C-
...
C-
C-   Note : Uses the function VALUE.
C-
C-   Created  14-MAR-1988   Harrison B. Prosper
C-   Modified 14-NOV-1988   NUMS now last argument
C----------------------------------------------------------------------
      IMPLICIT NONE
      LOGICAL       ACTIVE
      INTEGER       NUMS
...
C
        X = VALUE (STR(II:NS),I,J,CODE)
        ACTIVE = CODE .NE. 0
        IF ( ACTIVE ) THEN
          II = II + J
          IF ( STR(II:II) .EQ. '*' ) THEN
            Y = VALUE (STR(II:NS),I,J,CODE)
            II = II + J
            K = X
            DO 110 I = 1,K  
              N = N + 1
