      LOGICAL FUNCTION OUTPUT_STREAM_EDIT (RCP_BANK)
C----------------------------------------------------------------------
C-
...
C            WRITE(MSG,120) (K,BANK(K),IZLINK(K),K=1,NDEP)
            CALL ERRMSG('ISARCP','OUTPUT_STREAM_EDIT',MSG,'I')
...
C
          ...
          IF (INDEX(WORD(1),'WORD').GT.0) THEN
            READ (WORD(2),*) IWORD
          ELSE
            CALL ERRMSG('OUTPUT_STREAM','EDIT',
     &        ' NO OTHER COMPARISON SCHEME YET','W')
          END IF
C
C ****  EQ NE GT LT GE LT ?
C
...
C
      IF (EDIT) THEN
        ...
        IF (LBANK.EQ.0) THEN
            CALL ERRMSG('OUTPUT_STREAM','EDIT',
     &        ' BANK MISSING ','W')
           OUTPUT_STREAM_EDIT = CUT
           GOTO 999
        ELSE IF (NINTE.GT.0) THEN
          TEST = ITEST_BANK(LBANK,IWORD,COMPA,J,NINTE)
        ELSE
          TEST = RTEST_BANK(LBANK,IWORD,COMPA,R,NREAL)
        END IF
        IF (CUT) THEN
          OUTPUT_STREAM_EDIT = .NOT. TEST
        ELSE
          OUTPUT_STREAM_EDIT =  TEST
        END IF
      END IF
