      SUBROUTINE LSQ_SET_VAL(MATRIX_NAME,IROW,ICOL,INDEX,VAL,IER)
C----------------------------------------------------------------------
C-
C-   Purpose and Methods : SET VALUE OF A MTRIX ELEMENT
C-
...
C-   Updated   7-APR-1995   Alan M. Jonckheere  Change DSET -> DDSET
...
C
      ...
      NCOLS = IC(LINK+2)
C
      IND = (ICOL-1)*NROWS + IROW 
      IND = 2*(IND-1) + 3   !DOUBLE PRECISION + 1ST TWO WORDS
      CALL DDSET(LINK+IND,VAL)
C
  999 RETURN
      END
