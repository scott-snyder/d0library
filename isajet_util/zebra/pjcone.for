...
C-   Updated  29-JAN-1991   Boaz Klima, Chip Stewart
C-      Et instead of Pt in word 2
C-   Updated  16-SEP-1991   Marc Paterno   Check for more than NPMAX partons in
C-       ISAQ to cure access violation
C-   Updated  13-NOV-1992   Brent May, Andy Milder
C-      Remove forcing version 2  (version filled in BKPJHD)
C-   Updated  15-APR-1993   Chip Stewart  - protected ALOG  with ABS
C----------------------------------------------------------------------
      IMPLICIT NONE
...
C
      ...
      IF ( NP .GT. NPMAX ) THEN
        CALL ERRMSG ( 'PJETS', 'PJCONE',
     &    'Too many final state partons in ISAQ, using first 100', 'W' )
        GOTO 35
...
C
      IF ( INFI.NE.0 ) THEN
        ...
        P_ETA(NP) = -ALOG ( ABS(TAN(P_THE(NP)/2.)) + EPS )
      ELSE
        NP = NP - 1
      ENDIF
   30 LISAQ = LQ( LISAQ )
...
C ... ALREADY CONNECTED, SINGLE PARTON JET, OR HAS OTHERS TO CONNECT TO
        IF ( JCNN(JOP1,1).GE.0 ) THEN
          ...
          PJ_P(NPJ)   = P_P(JOP1)
          PJ_PT(NPJ) = P_PT(JOP1)
          PJ_ET(NPJ) = P_E(JOP1)*SIN(P_THE(JOP1))
          ...
          DO 210 JJ = 1 , JCNN(JOP1,1)
            ...
            PJ_ET(NPJ)  = PJ_ET(NPJ)  +
     &        P_E(JCNN(JOP1,JJ+1))*SIN(P_THE(JCNN(JOP1,JJ+1)))
  ...
  210     CONTINUE
          ...
          PJ_ETA(NPJ) = -ALOG ( ABS(TAN(PJ_THE(NPJ)/2.)) + EPS )
