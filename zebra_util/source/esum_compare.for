...
C
        ...
        CALL EZGET('PT_SPEC',PT_SPEC,IER_SUB)
        CALL EZGET('PT_MIN',PT_MIN,IER_SUB)
        CALL EZGET('PT_MAX',PT_MAX,IER_SUB)
        CALL EZGET('LOC',LOC,IER_SUB)
        CALL EZGET('RES',RES,IER_SUB)
        CALL EZGET('REF_MATCH',REF_MATCH,IER_SUB)
        CALL EZGET('EVENT_LIST',EVENT_LIST,IER_SUB)
...
C
                ...
                IF (LOC) THEN
                  ...
                  IDH = IDHN+37
                  CALL HFILL (IDH,ET,ETA_DET,PHI,1.)
                END IF
              ENDIF
            ENDIF
  300       CONTINUE
