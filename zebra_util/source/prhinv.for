      SUBROUTINE PRHINV ( PRUNIT, LHINV, NHINV, CFL, IFL )
C----------------------------------------------------------------------
C-
...
C-             LHINV  [I] : Pointer to the one bank ( CFL = 'ONE' ) or to the
...
C
      INTEGER PRUNIT, LHINV, NHINV, IFL
      CHARACTER*(*) CFL
C----------------------------------------------------------------------
C
C  ***  Print the content of the bank pointed by LHINV
C
      IF ( INVIS_DIM.GT.0 ) THEN
        CALL MXPRND(PRUNIT,'DUMP OF HINV',C(LHINV+1),INVIS_DIM,
     &  INVIS_DIM,INVIS_DIM,INVIS_DIM,7,'(D10.3)')
      ELSE
      ENDIF
  999 RETURN
      END
