      SUBROUTINE L1EXTRACT_SPTRG_FIRED_SCALERS(L1_BLOCK, 
     &                                         SPTRG_FIRED_SCALERS)
C----------------------------------------------------------------------
C-
C-   Purpose and Methods : Extract the Trigger Count scalers from the Level 1
...
C-           |---------------------------------------------------------------|
C-           | YES :   L1EXTRACT_SPTRG_FIRED_SCALERS ( IQ(LTRGR_LEVEL1),... )|
C-           |---------------------------------------------------------------|
C-           | NO  :   L1_BLOCK = IQ( LTRGR_LEVEL1 )                         |
C-           |         L1EXTRACT_SPTRG_FIRED_SCALERS ( L1_BLOCK, ... )       |
C-           +---------------------------------------------------------------+
C- 
C-   Outputs : SPTRG_FIRED_SCALERS(2, 0:31) [I]
C-
C-                The Trigger Count scaler for each Specific Trigger. 
C-                The 40-bit scalers are stored as follows:
C-                SPTRG_FIRED_SCALERS(1, ST) holds the low  24 bits
C-                SPTRG_FIRED_SCALERS(2, ST) holds the high 16 bits
...
C
      INTEGER L1_BLOCK(0:*)
      INTEGER SPTRG_FIRED_SCALERS(2, 0:31)
C
      INTEGER TRG_NUM
C
