      SUBROUTINE SHOWERLIB_OPEN(IUNIT,FILNAM,SRECL,NEW,KEYED,READONLY,
     &                           IERR)
C----------------------------------------------------------------------
C-
C-   Purpose and Methods : Open showerlibrary proper.
C-
...
C-             IF (KEYED. AND. NEW)THEN one is writing a showerlibrary
...
C
  ...
  998 FORMAT('OPENED SHOWERLIBRARY',
     &  ' NEW = ',L,' KEYED = ',L,' READONLY = ',L,1X,A)
      CALL ERRMSG('SHOWERLIBRARY','SHOWERLIB_OPEN',
     &  MSG,'W')
  999 RETURN
 9960 CALL ERRMSG('SHOWERLIBRARY','SHOWERLIB_OPEN',
     &  'ERROR OPENING SHOWERLIBRARY','F')
      IERR = IOSTAT_OPEN
      RETURN
      END
