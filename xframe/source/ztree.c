...
*/
	...
	daughter =
    		XtCreateManagedWidget(temp, xmPushButtonWidgetClass,
    				zebra_tree, wargs, n);
    XtAddCallback(daughter, XmNactivateCallback, bankbutton, address);
    XmStringFree(str);
    return(daughter);
}


void undotree()   /* called from D0X returning from navigator */
{
...
*/
	str = XtName(thewidget); 	strcpy(bank,str);	bank[4] = 0;
/*
...
*/
		...
		XmTextSetString(bank_text,bank);
	}
}
