...
static XtResource resources[] = {
 {XtNhorizontalSpace,XtCSpace,XtRDimension,sizeof(Dimension),
   XtOffset(XsTreeWidget, tree.h_min_space), XtRString,"40" },
 {XtNverticalSpace,XtCSpace, XtRDimension,sizeof (Dimension),
   XtOffset(XsTreeWidget, tree.v_min_space), XtRString,"10"  },
     ...
     long         level;
{
 ...
 set_current_position(tw->tree.horizontal, level,
                      CFMAX(current_hpos, w->core.width));
 /*
...
/*    tree_const->tree.y = (top + bottom)/2 - (w->core.height/ 2);*/
  /*
   * Set the node's position to the top its sub_nodes (ARB)
   */
    tree_const->tree.y = top  - (w->core.height/ 2);
  /*
