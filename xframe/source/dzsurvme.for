...
C----------------------------------------------------------------------
C-
C-   Purpose and Methods : cern/zebra dzsurv only into character array
C-
C-   Inputs  : 
C-   Outputs : 
C-   Controls: 
...
c
     ...
     +               IBASE,LBASE,NDW,JDFD,JD,JTYP
      ...
      DATA  NWDES  / 4 /,inull/0/,inl/538976266/

ccccccc      call getnlnull(inl,inull)
      nline = 0
      CQSTAK = CHROUT//'/'
      IQUEST(1) = 0
...
cccc          CALL ZPAGE(IQPRNT,7)
          CQMAP(1)        = ' DZSURV --- '
          CQMAP(1)(13:99) = CHTEXT
          WRITE(CQMAP(2),'('' ST= '',2A4,''  LSTART= '',I8)')
     X      NQPNAM(KQT+1),NQPNAM(KQT+2),LSTART
          nline = nline + 1
          outchr(nline) = cnl//cqmap(1)
          nline = nline + 1
          outchr(nline) = cnl//cqmap(2)
ccc      CALL DZTEXT(0,CDUMMQ,2)
      ENDIF

cccc      IF (CHTEXT.NE.'-') THEN
cccc          CALL ZPAGE(IQPRNT,5)
          nline = nline + 1
          outchr(nline) = cnl//
     &      '   NBK    IDENTIFIER(S)'//cnl
          nline = nline + 1
          outchr(nline) = cnl//' '
cccc          CQMAP(1) = ' '
cccc          CQMAP(2) = '   NWCUM     NW   WBK  NBK    IDENTIFIER(S)'
cccc          CQMAP(3) = ' '
cccc
cccc          CALL DZTEXT(0,CDUMMQ,3)
cccc      ENDIF
...
cccc      IF (CHTEXT.NE.'-') CALL DZTEXT(1,CDUMMQ,1)
   ...
   71 IF (NEWNBK.EQ.0)                                     GO TO 75
      ...
      IF (CHTEXT.NE.'-') THEN
ccc          WRITE(CQLINE,'(1X,2I7,I6,I5,2(1X,A1),20(1X,A4))')
ccc     W                   NWCUM,NEWNW,NEWND,NEWNBK,NEWRIN
ccc     W,                  (IQUEST(J),J=1,NEWLEV)
ccc     W,                  (NEWID(J), J=1,NEWNID)
          WRITE(CQLINE,'(1X,I5,2(1X,A1),20(1X,A4))')
     W                   NEWNBK,NEWRIN
     W,                  (IQUEST(J),J=1,NEWLEV)
     W,                  (NEWID(J), J=1,NEWNID)
          nline = nline + 1
          outchr(nline) = cnl//cqline
cccc          CALL DZTEXT(0,CDUMMQ,1)
      ENDIF
      ...
      IF (NWCUM.GT.MAXALL) THEN
          ...
          nline = nline + 1
          outchr(nline) = cnl//cqline
cccc          CALL DZTEXT(MSHO1Q,CDUMMQ,0)
          IQUEST(1) = 1
                                                           GO TO 999
      ENDIF
   ...
   74 continue
          nline = nline + 1
          outchr(nline) = cnl//cqline
cccc   74 CALL DZTEXT(MSUR1Q,CDUMMQ,0)
 ...
 9991 IF (CHTEXT.NE.'-') THEN
          WRITE(CQline,'('' The structure supported by'',
     +    '' bank '',A4)') IDSTR
          nline = nline + 1
          outchr(nline) = cnl//cqline
          WRITE(CQline,'('' Begins at '',I10,'' in store '',2A4)')
     +    LSTART,NQPNAM(KQT+1),NQPNAM(KQT+2)
          nline = nline + 1
          outchr(nline) = cnl//cqline
          WRITE(CQline,'('' Occupies '',I10,'' words in '',I6,
     &      '' banks '')') NWCUM,NBCUM
          nline = nline + 1
          outchr(nline) = cnl//cqline
cccc          CALL DZTEXT(0,CDUMMQ,2)
      ENDIF
      IQUEST(11) = IDSTR
      IQUEST(12) = LSTART
