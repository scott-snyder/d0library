...
C----------------------------------------------------------------------
C-
C-   Purpose and Methods : Check that all the items in the list do not form
C-   a range.
C-
C-   Inputs  : none
...
C
      GOOD = .TRUE.
      DO COUNT = 1, LIST_TOP
        IF ((LIST(COUNT,ITEM_T_INDEX) .EQ. PARSE_LOWER_BOUNDARY) 
     &    .OR. (LIST(COUNT,ITEM_T_INDEX) .EQ. 
     &    PARSE_UPPER_BOUNDARY)) THEN
          GOOD = .FALSE.
          GOTO 999
        ENDIF
      END DO
