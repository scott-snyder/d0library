...
C       GET pass number
c
        IF(LIST(COUNT,OBJ_INDEX).EQ.KEY_PASS_ONE_OF)THEN
          IF(LIST(COUNT,ITEM_T_INDEX).EQ.PARSE_ASSERTED)THEN
            L15C_PASS(CRATE,TERM) = LIST(COUNT,ITEM_INDEX)
C            WRITE(*,*)CRATE,TERM,L15C_PASS(CRATE,TERM)
          ELSE
