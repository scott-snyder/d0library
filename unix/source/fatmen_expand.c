...
int fatmen_expand(char *file, int nout, char *outbuf)

/*
 *-
 *-   Purpose and Methods: Expand a fatmen generic name containing wildcards.
 *-
...
/* Remote execute. */
  ...
  if (sock < 0) {
    fprintf(stderr, "fatmen_expand: rexec failed");
    return FAILURE;
  }
...
/* Read output from remote command. */
  ...
  if(remote == NULL) {
    fprintf(stderr, "fatmen_expand: fdopen failed");
    return FAILURE;
  }
  ...
  /* See if this line contains a generic name.  Add to outbuf. */
    ...
    if(gen != NULL && strstr(line, "Generic filename")) {
      pt = gen;
