...
C----------------------------------------------------------------------
C-
C-   Name: smg_paste_virtual_display
C-
C-   Purpose: Paste a virtual display to the specified pasteboard at the
...
C-   Fortran usage:
C-
C-      ok = smg$paste_virtual_display( dpid, pbid, prow, pcol, top_dpid)
C-
C-   Arguments:
C-
...

{
  ...
  smg_static_data.function_name = "smg$paste_virtual_display";
...
/* Error if already pasted. */

  if(display->pasted)
    return smg_error("display alread pasted", 0);
...
/* Update the terminal.  This means updating the specified display and all
   succeeding displays. */
