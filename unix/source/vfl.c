...
C-             filename is kept.  The directory portion of the pathname
C-             is ignored when looking for the first filename.
C-
C- Usage:
C-
C-     vfl [-d] file(s)
...
/* Determine input source (arg list or standard input). */
  ...
  /* Expand this file specification by calling lib_find_file. */
 
    do {
      lib_find_file(ifile, rfile, &context);
      if(*rfile != '\0') {
 
    /* Got a file.  Find the file portion of the filename. */
