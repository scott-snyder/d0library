      SUBROUTINE BKVTMS(LSUP,ND,LBANK,IFL)
C----------------------------------------------------------------------
C-
C-   Purpose and Methods : Create Vertex Chamber Times bank 
C-
C-   Inputs  : LSUP = Address of the supporting bank
C-             ND = length of the created bank
C-   Output  : LBANK = address of the created bank
C-   Control : IFL = 0, Book VTMS with FTMH as supporting bank
C-             IFL = 1, Book VTMS as next bank in linear structure
C-
...
C
      ...
      ENDIF
C
      CALL MZFORM('VTMS','30I -F',NIO)  ! Data Type
      IF (IFL.EQ.0) THEN
        CALL MZBOOK(IDVSTP,LBANK,LSUP,-IZVTMS,'VTMS',NL,NS,ND,NIO,0)
      ELSE
        CALL MZBOOK(IDVSTP,LBANK,LSUP,0,'VTMS',NL,NS,ND,NIO,0)
      ENDIF
C
C ****  Get date and time 
C
