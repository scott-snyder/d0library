      SUBROUTINE VPDINI(CRUN, MAX_VTXCRT, DB_OPENED, OK)
C----------------------------------------------------------------------
C-
C-   Purpose and Methods : Read VTX pedestals from database.
C-                         Called by VDBINI.
C-
...
C-   Updated   8-APR-1992   Peter M. Grudberg  Take out pads (avoid problems
C-                                             with T0 detector)
...
C
      ...
      INTEGER LINKH,LSRCP
      INTEGER LVPCH
      INTEGER LVPDL, GZVPDL, LVPDZ, GZVPDZ, POINT
...
C
      ...
      ENDIF
C
C ****  Loop over all crates.  Check if pedestal run has changed since last run
C ****  processed - if so, update!
C
      ACCESSED = 0
      IF (DB_OPENED) CALL DBCLB_PATH('PEDESTAL', 'VTX', PATH)
        ...
        ELSE
          IF (.NOT.DB_OPENED) THEN
            ...
            IF (.NOT.OK) THEN
              ...
              GO TO 999
            END IF
            CALL DBCLB_PATH('PEDESTAL','VTX',PATH)
            DB_OPENED = .TRUE.
          END IF
        END IF
C
...
C Get Crate ped banks.
        ...
        IF ( IQUEST(1) .NE. 0 ) THEN
          IF (BYPASS_DBL3_ERROR) THEN
            CALL ERRDB(' Error in VPDINI ')
            GO TO 999
          ELSE
            CALL ERRMSG('VTRAKS','VPDINI',
     &        'failed to fetch from DBL3','F')
        ...
        LVPCH = LDATA
        IF ( LVPCH .EQ. 0 ) THEN
          CALL ERRDB(' VPDINI - Error in finding requested data ')
          OK = .FALSE.
          GO TO 999
        ENDIF
...
C
        LINKH = LC(LVPDH - IZVPCH)
        IF ( LINKH .NE. 0 ) THEN
          CALL EZDROP('SCPH')
          CALL MZDROP(IXSTP,LINKH,' ')
        ENDIF
C
        CALL ZSHUNT(IXSTP, LVPCH, LVPDH, -IZVPCH, 0)
        LVPCH = LC(LVPDH - IZVPCH)
        PEDRUN = IC(LVPCH+6)
C
        CALL CPATHG(SPTH)
    ...
    5     FORMAT('CRATE_',I3.3,A1)
          ...
          CALL EZNAME('SCPH', LVPCH, IZCRCP)
          CALL EZNAME(SRCPNAM, LVPCH, 4)
          CALL EZPICK('SCPH')
...
C
          DO CHNL = IMIN, IMAX
            CALL VGTPDS(CHNL, VALUE, 1)
            CALL NUM_TO_ADDR(CHNL, ID, 1)
            IF ( ID .GE. 0 ) THEN
              CALL VCODER(ID, TYPE, LAY, SEC, WIR, STR, END, UB, 1)
              IF ( UB .EQ. 0 .AND. TYPE .EQ. 0) THEN
                LVPDL = GZVPDL(LAY)
                IF ( IC(LVPDL+1) .NE. LORUN(CRT)
     &                .OR. IC(LVPDL+2) .NE. HIRUN(CRT) ) THEN
                  IC(LVPDL + 1) = LORUN(CRT)
                  IC(LVPDL + 2) = HIRUN(CRT)
                ENDIF
                POINT = LVPDL +
     &              (SEC*IC(LVPDL+4)+2*WIR+END)*IC(LVPDL+3) + 5
                C(POINT + 1)=VALUE(1)
                C(POINT + 2)=VALUE(2)
              ENDIF
            ENDIF
...
C
      IF ( ACCESSED .GT. 0 ) THEN
        WRITE(MSG,'(A,I10.10)') 
     &    ' VTX PEDS read from DBL3 online path, crate access = ', 
     &    ACCESSED
        CALL INTMSG(MSG)
      ENDIF
C
