      SUBROUTINE BKVGNS(LSUP,ND,LBANK,IFL)
C----------------------------------------------------------------------
C-
C-   Purpose and Methods : Create Vertex Chamber Gains bank 
C-
C-   Inputs  : LSUP = Address of the supporting bank
C-             ND = length of the created bank
C-   Output  : LBANK = address of the created bank
C-   Control : IFL = 0, Book VGNS with FGNH as supporting bank
...
C
      ...
      ENDIF
C
      CALL MZFORM('VGNS','30I -F',NIO)
      IF (IFL.EQ.0) THEN
        CALL MZBOOK(IDVSTP,LBANK,LSUP,-IZVGNS,'VGNS',NL,NS,ND,NIO,0)
      ELSE
        CALL MZBOOK(IDVSTP,LBANK,LSUP,0,'VGNS',NL,NS,ND,NIO,0)
      ENDIF
C
C ****  Get date and time 
C
