       FUNCTION L2NAME_PASSED_EXCL(L2FILTNAME)
C----------------------------------------------------------------------
C-
...
C-             Exact name match required unlike L2NAME_PASSED
C-             
C-   Controls: NONE
C-
C-   Created   5-DEC-1992   Norman A. Graf
C-   Updated  15-JAN-1996   Bob Hirosky  require exact name match
C-
C----------------------------------------------------------------------
      IMPLICIT NONE
...
C
        CALL GET_L2_BIT_NUMBER_EXCL(L2FILTNAME,BITNUM,FOUND)
        IF(FOUND) L2NAME_PASSED_EXCL = L2BIT_PASSED(BITNUM)
C
  999 RETURN
      END
