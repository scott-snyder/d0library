...
C
        ...
        CALL HBNAME_i(1, 'HEAD',RUNNUM,QCD_EVT_INFO_TAG)
        IF(DO_LUM) CALL HBNAME(1, 'HEAD',LUM,QCD_EVT_INFO2_TAG)
        IF(MC_DATA) CALL HBNAME(1, 'MC',WEIGHT,QCD_MC_INFO_TAG)
        CALL HBNAME_l(1, 'JUTL',QCDFILT,QCD_JUTL_HEAD1_TAG)
        IF(VERTWD.EQ.2)
     &    CALL HBNAME_i(1, 'JUTL2',NZ,QCD_JUTL_HEAD2_TAG)
        IF(GLOBWD.GE.1) CALL HBNAME(1, 'GLOB1',ETOT,QCD_GLOB1_TAG)
        IF(GLOBWD.GE.2) CALL HBNAME_i(1, 'GLOB2',NCDCTK,QCD_GLOB2_TAG)
        CALL HBNAME_i(1, 'JETS',NJETS,QCD_JETS1_TAG)
        IF(DO_FULL_JET) THEN
          CALL HBNAME(1, 'JETS',EXJ(1),QCD_JETS2_TAG)
          IF(ESCALE) THEN
            CALL HBNAME(1, 'JETS',ETUN(1),QCD_JETS3_TAG)
            IF(UPATH.EQ.'RECO') THEN
              CALL HBNAME(1, 'JETS',DEMFR(1),QCD_JETS5_TAG)
            END IF
          END IF
          IF(DO_ELC.AND.UPATH.EQ.'RECO') THEN
            CALL HBNAME(1, 'JETS',DJNEP(1),QCD_JETS6_TAG)
          END IF
        END IF
        IF(JTCSON) CALL HBNAME(1, 'JETS',ETR1(1),QCD_JETS4_TAG)
        IF(PNUTWD.GE.1) CALL HBNAME(1, 'NUT1',NUT_ET(1),QCD_NUT1_TAG)
        IF(PNUTWD.GE.2) CALL HBNAME(1, 'NUT2',NUT_E(1),QCD_NUT2_TAG)
        IF(MUONWD.GE.1) CALL HBNAME_i(1, 'MUON',NMUTOT,QCD_MUON1_TAG)
        IF(MUONWD.GE.2) CALL HBNAME(1, 'MUON',MU_EX(1),QCD_MUON2_TAG)
        IF(DO_ELC) THEN
          CALL HBNAME_i(1, 'PELC',NELEC,QCD_ELC1_TAG)
          CALL HBNAME(1, 'PELC',ELC_EM_FRAC(1),QCD_ELC2_TAG)
          CALL HBNAME_i(1, 'PELC',ELC_NCELLS(1),QCD_ELC3_TAG)
          CALL HBNAME(1, 'PELC',ELC_EMV_PRBXY(1),QCD_ELC4_TAG)
        END IF
        IF(DO_PHO) CALL HBNAME_i(1, 'PPHO',NPHOT,QCD_PHO_TAG)
        IF(DO_L1L2) THEN
          IF(L1WD.GT.10) THEN
            L1TYPE=L1WD-10
          ELSE
          ...
          IF(L1TYPE.EQ.1.OR.L1TYPE.EQ.3) THEN
            CALL HBNAME_i(1, 'L1TT',NTT,QCD_JUTL_JETS1_TAG)
          END IF
          IF(L1TYPE.EQ.2.OR.L1TYPE.EQ.3) THEN
            CALL HBNAME_i(1, 'L1LJ',NLJ,QCD_JUTL_JETS2_TAG)
          END IF
          IF(L1WD.LT.10) THEN
            CALL HBNAME_i(1, 'L2',NL2,QCD_JUTL_JETS3_TAG)
          END IF
        END IF
        IF(DO_AIDA) CALL HBNAME_i(1, 'AIDA',NAIDA,QCD_AIDA_TAG)
