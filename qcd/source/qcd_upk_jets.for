...
C
        ...
        CALL EZGET('DO_ZSP_CORRECTION', DO_ZCOR, IER )
        IF ( IER.LT.0) CALL ERRMSG('QCD_UPK_JETS','CAFIX_RCP',
     &    ' DO_ZSP_CORRECTION not found ','F')
        CALL EZGET('DO_UND_CORRECTION', DO_UDVCOR, IER )
        IF ( IER.LT.0) CALL ERRMSG('QCD_UPK_JETS','CAFIX_RCP',
     &    ' DO_UND_CORRECTION not found ','F')
        CALL EZGET('DO_CONE_CORRECTION', DO_OUTCOR, IER )
        IF ( IER.LT.0) CALL ERRMSG('QCD_UPK_JETS','CAFIX_RCP',
...
C
        CALL EZGET('NTEMPLATE', NTEMPLATE,IER)
        IF ( IER.LT.0) THEN
     ...
     &      'NO VARIABLE FOUND','W')
        ...
        END IF
C
        CALL EZGET('UTEMPLATE', UTEMPLATE,IER)
        IF ( IER.LT.0) THEN
...
C
      ENDIF
C
      CALL VZERO(TRANS,MAX_JET*2)
      CALL VZERO(INDJ,MAX_JET)
C
C Initialize
C
      NJETS=0
...
C
      IF(JTCSDO) THEN
        CALL VZERO(INDJET,MAX_JET)
        CALL VZERO(INDJTC,MAX_JET)
