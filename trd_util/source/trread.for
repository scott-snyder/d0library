...
C ****  Define the limits of the TRD RMIN, ZMIN, RMAX, ZMAX
      ...
      ZMAX = ZMIN
      CALL EZPICK('TRD_RCP')
      CALL EZGET('TYPPRO',TYPPRO,IER)
      CALL EZGET('OPT',IOPTRD,IER)
      CALL EZGET('THMIN',THMIN,IER)
      CALL EZGET('THMAX',THMAX,IER)
      CALL EZGET('SWTLYR',JSWITC(1),IER)
      ...
      ELSE
        ...
        EPICOR(3)=1.
      ENDIF
      CALL EZGET('CLUSTER_RECONS',ITRANC(7),IER)
      CALL EZGET('CALIBRATION',ITRANC(8),IER)
      CALL EZGET('ALIGNMENT',ITRANC(9),IER)
      CALL EZGET('VOL0_CORRECTION',ITRANC(10),IER)
      CALL EZGET('ONE_ANOD_CELL',ITRANC(12),IER)
      CALL EZGET('CONVERTED_GAMMAS',ITRANC(13),IER)
...
*****   If no correction by Epion, no efficiency calculation
   ...
   10   TVALCA(IBC) = 'NO '
      ENDIF
*****
      CALL EZGET('FADC_DRAW',ITRANI(1),IER)
      CALL EZGET('TRK_FADC_DRAW',ITRANI(2),IER)
      CALL EZGET('ALL_FADC_DRAW',ITRANI(3),IER)
      IF (ITRANI(3).LT.ITRANI(2)) ITRANI(3) = ITRANI(2)
      CALL EZGET('CLUSTER_THRESHOLD',ITRANI(4),IER)
*****
      CALL EZGET('TEXAMIN',TEXAMIN,IER)
      CALL EZGET('TTRAKIN',TTRAKIN,IER)
      CALL EZGET('TRDPEDES',TRDPEDES,IER)
      CALL EZGET('TRDGAIN',TRDGAIN,IER)
      CALL EZRSET
      LOUT=TRUNIT()
