...
C      EQUIVALENCE (HSTBOK,IHSTBK),(SWTARR,ISWARR),(SWTGEN,SWHISG)
      ...
      IF(FIRST)THEN
        ...
        DO I=1,4
          ...
          END IF
        END DO
        CALL EZGET_l('EFF_CALCUL',EFFC,IER)
        IF (EFFC) THEN
          CALL EZGET_l('EFFC_ETOT',EFFC_ETOT,IER)
          CALL EZGET_l('EFFC_ETRUNC',EFFC_ETRUNC,IER)
          CALL EZGET_l('EFFC_E_L',EFFC_ETOT,IER)
          CALL EZGET_l('EFFC_E_CL_L',EFFC_ETOT,IER)
        ELSE
          EFFC_ETOT=.FALSE.
          EFFC_ETRUNC=.FALSE.
          EFFC_E_L=.FALSE.
...
C        CALL EZGET('PIONS_HISTOS',SWITCH_HISTO(1,1),IER)
        IF(DO_HISTO(1))THEN
          IF(DO_HISTO(2))THEN
            CALL VZERO_i(IWS,14)
            CALL EZGET_iarr('PIONS_HISTOS'    ,IWS ,IER)
            DO I=1,14
              IF(IWS(I).NE.0)THEN
...
C        CALL EZGET('ELECTRONS_HISTOS',SWITCH_HISTO(1,2),IER)
          IF(DO_HISTO(3))THEN
            CALL VZERO_i(IWS,14)
            CALL EZGET_iarr('ELECTRONS_HISTOS',IWS,IER)
            DO I=1,14
              IF(IWS(I).NE.0)THEN
...
C        CALL EZGET('GENERAL_TRD_HISTOS',SWHISG(1),IER)
          CALL VZERO_i(IWS,14)
          IF(DO_HISTO(4))THEN
            CALL EZGET_i('GENERAL_TRD_HISTOS',IWS(1),IER)
            DO I=1,12
              IF(IWS(I).NE.0)THEN
