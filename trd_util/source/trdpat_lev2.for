      SUBROUTINE TRDPAT_LEV2(NTRIP)
C----------------------------------------------------------------------
C-
C-   Purpose and Methods : Try to associate 3 TRD layers
C-
C-   Inputs  : Table of coded wires in common block TRWCOD
C-   Outputs :nb. of triplets
C-   Controls:
C-
C-   Created  13-MAY-1991   A. Zylberstejn
C-   Updated  22-SEP-1993   A. Zylberstejn  Modified for lev2 and 512 cells
C-
C----------------------------------------------------------------------
      IMPLICIT NONE
      ...
      INTEGER II,IS,ICI,ICS,ISTART,IUCOMP,IW1,IW2,IW3,TCHNB
...
C----------------------------------------------------------------------
      LOUT=TRUNIT()
C      DOPRINT=SWTDBG.EQ.1 .AND. TNEVDG.GT.0 .AND. LOUT.NE.0
C  Put coded wires in a continuous array
      ...
      DO 10 ICH =  1,  2
        ...
        END DO
   10 CONTINUE
C  special treatment for layer 3
      ICH=3
      NW(ICH)=0
      IF(NWIRE_PER_LAYER(ICH).EQ.512)THEN
        DO I=1,NWIRE_PER_LAYER(ICH)-1,2
          IF(TWCOD(TCHNB(I,ICH)) .OR. TWCOD(TCHNB(I+1,ICH)))THEN
            NTOT=NTOT+1
            NW(ICH)=NW(ICH)+1
            IWS(NTOT)=(I-1)/2+1
            IAUX(NTOT)=0
...
C  Set IAUX to 1 for wire and adjacent ones
        ...
        IF(I.NE.256 .AND. IWS(I+1).EQ.IW1+1)IAUX(I+1)=1
        IF(ID2.NE.  1.AND.IWS(ID2+NW(1)-1).EQ.IW1-1)IAUX(ID2+NW(1)-1)
     &      =1
        IF(ID2.NE.256.AND.IWS(ID2+NW(1)+1).EQ.IW1+1)IAUX(ID2+NW(1)+1)
     &      =1
        IF(ID3.NE.  1 .AND.  IWS(ID3+NW(1)+NW(2)-1).EQ.IW1-1)
     &                               IAUX(ID3+NW(1)+NW(2)-1)=1
        IF(ID3.NE.256 .AND.  IWS(ID3+NW(1)+NW(2)+1).EQ.IW1+1)
     &                               IAUX(ID3+NW(1)+NW(2)+1)=1
   40 CONTINUE
