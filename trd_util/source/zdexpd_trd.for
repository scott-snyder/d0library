      SUBROUTINE ZDEXPD_TRD(DCDTYP, LABEL, DATAS)
C----------------------------------------------------------------------
C-
...
C----------------------------------------------------------------------
      ...
      INTEGER DCDTYP, LABEL, DATAS(*), MXLABL,SUPLAB
      ...
      INTEGER ILR,IPIN1,LCHA1,IWR,TCHNB
      PARAMETER ( HIGHOF = 16 , LOWOFF = 0 )
      INTEGER IDTYPE,LABINF,LABSUP,UBIT
...
C
      IF (FIRST) THEN
        ...
        CALL TCODER(LABINF,0,0,UBIT,2)
        CALL TCODER(LABSUP,5,255,UBIT,2)
        SUPLAB=0
      ENDIF
C
...
C
   20     IPIN   = IPORIG + LONG
          ...
          IF(JCDD.NE.4)SUPLAB =MAX0(ILAB,SUPLAB)
          IF(ILAB.EQ.20031)THEN
          END IF
          ...
          LONG = LONG - LCHA
          IF(JCDD.EQ.4)THEN
            CALL TCODER(ILAB,ILR,IWR,UBIT,1)
            ILR=ILR+1
            IWR=IWR+1
            IPIN1 = IPOINT( ILAB,1 ) + LCDDN( JCDD)
            LCHA1 = IBITS( IQ(IPIN1), LOWOFF, 16 ) / 4 - 1
            IPIN1 = IPIN1 - 1
            LCLU = IBITS( IQ(IPIN1), LOWOFF, 16 ) - 2
            IDTYPE = IBITS(ILAB, 13, 2)
            IF (VERSION(IDTYPE+1) .NE. 0) LCLU = LCLU - 2
            TWCOD(TCHNB(IWR,ILR))=LCLU.NE.0
          END IF
          GOTO 20
C
...
C
 ...
 1100     FORMAT
     &      (10X,'At offset ',I10,' found label=',I10,' length=',I10)
        ENDIF
      ENDIF
      GOTO 12
C
