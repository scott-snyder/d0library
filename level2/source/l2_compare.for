...
C
     ...
     &    'L2_COMPARE.RCP does not have L2_COMPARE bank!','F')
      ENDIF
      CALL EZGET('DO_FILT',DOFILT,IER)
      CALL EZGET('DO_TRGR',DOTRGR,IER)
      IF (DOTRGR) THEN
        CALL EZGET('L1_MATRIX',MATRIX,IER)
      ENDIF
      CALL EZGET('DO_FRES',DOFRES,IER)
      IF (DOFRES) THEN
        CALL EZGET('DO_L2JETS',DOL2JETS,IER)
        CALL EZGET('DO_L2EM',DOL2EM,IER)
      ENDIF
      CALL EZGET('DO_PNUT',DOPNUT,IER)
      CALL EZGET('DO_L0VT',DOL0VT,IER)
      CALL EZGET('DO_RMAS',DORMAS,IER)
      CALL EZGET('DO_LETA',DOLETA,IER)
      CALL EZGET('DO_ACOL',DOACOL,IER)
C
      CALL EZGET('PRTEMP',PRTEMP,IER)
      CALL EZGET('WRT_ALL',WRT_ALL,IER) 
      CALL EZGET('WRT_BAD',WRT_BAD,IER) 
      CALL EZGET('WRT_MU' ,WRT_MU,IER) 
C
      CALL EZGET('PRTLEV',PRTLEV,IER)
      ACTION = 'TERM'
