...
C-   Modified 12-JUL-1991   Yi-Cheng Liu
C-                          ( for FDC )
C-   Modified  4-OCT-1993   Yi-Cheng Liu , add in protection 
C-                          against bad HITADR,HITLEN read              
C-
C----------------------------------------------------------------------
      IMPLICIT NONE
...
C
      ...
      INTEGER MASK8,MASK16,NPULSE,IER
...
C      
      IF (UNIT.EQ.0) THEN
C------ for theta chamber ---------------------------------------------        
        IF (WIRE.EQ.8.OR. WIRE.EQ.9) THEN
          THR1 = DL_THR1
          THR2 = DL_THR2
          MAX_PULSE = 5
        ENDIF
...
C
        NPULSE = NPULSE + 1
        HITLST(NPULSE,0,WIRE) = 10.*(SUMX/SUM + IFIRST + 0.5)     ! Drift time
        HITLST(NPULSE,1,WIRE) = EVTDAT(ILAST) - IPED              ! Peak Height
        HITLST(NPULSE,2,WIRE) = AREA                              ! Area
