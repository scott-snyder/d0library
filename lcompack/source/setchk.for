...
C&      INTEGER LIB$GET_FOREIGN             ! RUN-TIME LIBRARY MODULES
C&      INTEGER STATUS                      ! RUN-TIME LIB. CALL RETURN STATUS
C&ENDIF
      INTEGER  IERR
C----------------------------------------------------------------------
...
C
     ...
     *         COMPACKCLI_TABLE,,,)
        ...
        ENDIF
C&ELSE
C&        STATUS = LIB$GET_FOREIGN(COMAND,%VAL(0),ISIZ,%VAL(0))
...
C
        IF(PARSOK) THEN
          ...
          CALL GCONFI       !Check if /CONFIRM_END was present
