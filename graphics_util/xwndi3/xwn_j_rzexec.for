...
C  ZOOM(ZOOM) IN TO CURSOR WINDOW
 1110 DDD=.2*(UMAX-UMIN)
      CALL JLOCAT(1,1,0,IB1,VX1,VY1)
      CALL JPECHO(1,2,1,VX1,VY1)
      CALL JCONVW(VX1,VY1,UMINZ,VMINZ,ZMINZ)
      CALL JCOLOR(7)                    ! WHITE
      CALL JMOVE(UMINZ-DDD,VMINZ)
      CALL JDRAW(UMINZ+DDD,VMINZ)
      CALL JMOVE(UMINZ,VMINZ-DDD)
      CALL JDRAW(UMINZ,VMINZ+DDD)
...
C  UNZOOM/UNPAN
 1120 MODWIN=.FALSE.
      GO TO 2000
C  PAN TO NEW CENTER
 1130 CALL JLOCAT(1,1,0,IB1,VX1,VY1)
...
C  HARDCOPY
 1190 GO TO 2010
C  EXIT!
 1200 CALL DELETE_DISPLAY(VD_IDB)
      RETURN
C
C  NEW PICTURE
 2000 CONTINUE
...
C  OUTPUT SETTINGS FOR DEBUG PURPOSES
 2010 CONTINUE
C      IF(IPSETI.EQ.1)TYPE 1221,ITEMI,UVEC,VVEC,NVEC,
C     &  VVIEW,VPROJ,VDIST
C      IF(IPSETI.EQ.1)WRITE(10,1221)ITEMI,UVEC,VVEC,NVEC,
C     &  VVIEW,VPROJ,VDIST
C 1221 FORMAT(' *****RZP3D TASK:',I,/,
C     &  ' U:',3F,/,' V:',3F,/,' N:',3F,/,' VIEWPOINT:',3F,/,
C     &  ' EYEPOINT :',3F,/,' VDIST:',F)
C      CALL J_DMPSEG(10)
C
      RETURN
C
C  ENTRY FOR PIXIE TO TELL RZP3D WHICH SEGMENT CONTAINS THE
