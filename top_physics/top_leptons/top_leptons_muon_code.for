...
C
        IF(IER.EQ.0) CALL EZGET('MU_DO_T0FLOAT_CUT',DO_T0FLOAT_CUT,IER)
        IF(IER.EQ.0) CALL EZGET('MU_MAX_T0FLOAT',T0FLOAT_MU_MAX,IER)
C
        IF(IER.EQ.0) CALL EZGET('MU_DO_OPP_MIP_CUT',DO_OPP_MIP_CUT,IER)
...
C
        IF(IER.EQ.0) 
     1    CALL EZGET('MU_DO_HITPLN_CUTS',DO_MUHITPLN_CUTS,IER)
        IF(IER.EQ.0) CALL EZGET('MU_CF_WAMA_MIN',CF_WAM_MINA,IER)
        IF(IER.EQ.0) CALL EZGET('MU_CF_WAMB_MIN',CF_WAM_MINB,IER)
        IF(IER.EQ.0) CALL EZGET('MU_CF_WAMC_MIN',CF_WAM_MINC,IER)
        IF(IER.EQ.0) 
     1    CALL EZGET('MU_CF_WAMTOT_MIN',CF_WAM_MINTOT,IER)
        IF(IER.EQ.0) CALL EZGET('MU_EF_WAMA_MIN',EF_WAM_MINA,IER)
        IF(IER.EQ.0) CALL EZGET('MU_EF_WAMB_MIN',EF_WAM_MINB,IER)
        IF(IER.EQ.0) CALL EZGET('MU_EF_WAMC_MIN',EF_WAM_MINC,IER)
        IF(IER.EQ.0) 
     1    CALL EZGET('MU_EF_WAMTOT_MIN',EF_WAM_MINTOT,IER)     
        IF(IER.EQ.0) CALL EZGET('MU_DO_EF_LVL2_CUT',DO_EF_LVL2_CUT,IER)
C
        IF(IER.EQ.0)
     1     CALL EZGET('MU_DO_REJECT_A_STUB',DO_REJECT_ASTUB,IER)
C
        IF(IER.EQ.0) CALL EZGET('MU_DO_IFW4_CUT',DO_MUIFW4_CUT,IER)
        IF(IER.EQ.0) CALL EZGET('MU_IFW4_CUT',IFW4_CUT,IER)
C
        IF(IER.EQ.0) 
     1       CALL EZGET('MU_DO_BEND_QUAL_CUT',DO_BEND_QUAL_CUT,IER)
         IF(IER.EQ.0) 
     1       CALL EZGET('MU_MAX_BEND_QUAL',BEND_VIEW_QUAL,IER)
        IF(IER.EQ.0) 
     1      CALL EZGET('MU_DO_NBEND_QUAL_CUT',DO_NBEND_QUAL_CUT,IER)
        IF(IER.EQ.0) 
...
C
     ...
     1      CALL EZGET('MU_MAX_ZTRAK_DPHI',MAX_ZTRAK_DPHI,IER)
        IF(IER.EQ.0)
     1      CALL EZGET('MU_MIN_CD_MULT',MIN_NO_ZTRAKS,IER)
        IF(IER.EQ.0)
     1      CALL EZGET('MU_MAX_CD_MULT',MAX_NO_ZTRAKS,IER)
C
