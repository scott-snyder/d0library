...
C
        CALL EZGETA_i('MUON_MASK_1',0,0,0,N,IER)
        IF(N.LT.32) WRITE(LUN,1000) N
        IF(IER.EQ.0) CALL EZGETA_iarr('MUON_MASK_1',1,N,1,MUON_MASK_1,
     &       IER)
...
C
        CALL EZGETA_i('MUON_MASK_2',0,0,0,N,IER)
        IF(N.LT.32) WRITE(LUN,1001) N
        IF(IER.EQ.0) CALL EZGETA_iarr('MUON_MASK_2',1,N,1,MUON_MASK_2
     &       ,IER)
...
C *** to true.
C
        IF(IER.EQ.0) CALL EZGET_l('FINDTOP_CAND',FIND_TOP,IER)
        IF(FIND_TOP) THEN
          IF(IER.EQ.0) CALL EZGET_l('FINDTOP_MUMU',FIND_TOP_MUMU,IER)
          IF(FIND_TOP_MUMU) THEN
            ITEST=ITEST+1
          ENDIF
          IF(IER.EQ.0) CALL EZGET_l('FINDTOP_MUJET',FIND_TOP_MUJT,IER)
          IF(IER.EQ.0) CALL EZGET_i('LJ_NOMUON_MIN',MUJET_NOMU_MIN,IER)
...
C
        IF(IER.EQ.0) CALL EZGET_l('FINDZ_CAND',FIND_Z,IER)
        IF(FIND_Z) THEN
          IF(IER.EQ.0) CALL EZGET_l('FINDZ_MUMU',FIND_Z_MUMU,IER)
          IF(FIND_Z_MUMU) THEN
            ITEST=ITEST+1
          ENDIF
        ENDIF
...
C
      ...
      IF(IER.EQ.0) CALL EZGETA_iarr('MAX_IFW4',1,N,1,
     1  MAX_IFW4_TIGHT,IER)
      IF(IER.EQ.0) CALL EZGETA_i('MAX_IFW4_LOOSE',0,0,0,N,IER)
      IF(IER.EQ.0) CALL EZGETA_iarr('MAX_IFW4_LOOSE',1,N,1,
...
C
     ...
     1  CALL EZGETA('MAX_ZTRAK_DPHI',1,N,1,MAX_ZTRAK_DPHI,IER)
      ...
      IF(IER.EQ.0)
     1  CALL EZGETA_iarr('MIN_ZTRAK_MULT',1,N,1,MIN_ZTRAK_MULT,IER)
      IF(IER.EQ.0) CALL EZGETA_i('MAX_ZTRAK_MULT',0,0,0,N,IER)
      IF(IER.EQ.0)
     1  CALL EZGETA_iarr('MAX_ZTRAK_MULT',1,N,1,MAX_ZTRAK_MULT,IER)
C
