...
C
      ...
      IF (IER.EQ.0) CALL EZGET('LIST_IDCUTS',DO_LIST_IDCUTS,IER)
      ...
      IF(DO_HISTOGRAMS) THEN
        IF (IER.EQ.0) CALL EZGET('PLOT_ELECTRON_HIST',
     &    EL_DIAG_HISTOGRAMS,IER)
        IF (IER.EQ.0) CALL EZGET('PLOT_MUON_HIST',
     &    MU_DIAG_HISTOGRAMS,IER)
        IF (IER.EQ.0) CALL EZGET('PLOT_PHOTON_HIST',
     &    PH_DIAG_HISTOGRAMS,IER)
      ENDIF
      IF (IER.EQ.0) CALL EZGET('FILL_NTUPLES',DO_NTUPLES,IER)
      IF(DO_NTUPLES) THEN
        IF (IER.EQ.0) CALL EZGET('ELPH_NTUPLE',DO_ELPH_NTUPLE,IER)
        IF (IER.EQ.0) CALL EZGET('MU_NTUPLE',DO_MU_NTUPLE,IER)
      ENDIF
      ...
      ENDIF
      IF (IER.EQ.0) CALL EZGET('DO_DUPLICATE_CUT',DO_EVT_CHECK,IER)
C
...
C
      ...
      IF(IER.EQ.0) CALL EZGET('DO_ISOL_CONE_SUB',DO_ISOLCONE_SUB,
     1  IER)
...
C
      ...
      IF(DO_FINDW) THEN
        IF (IER.EQ.0) CALL EZGET('FINDW_ENU',DO_FIND_WE,IER)
...
C
      ...
      IF(DO_FINDZ) THEN
        IF (IER.EQ.0) CALL EZGET('FINDZ_EE',DO_FIND_ZEE,IER)
...
C
      ...
      IF(DO_FINDWWG) THEN
        IF (IER.EQ.0) CALL EZGET('FIND_WE_GAM',DO_FIND_WWG_WE,IER)
        IF (IER.EQ.0) CALL EZGET('FIND_WMU_GAM',DO_FIND_WWG_WMU,IER)
     ...
     1     CALL EZGET('WWG_MIN_MT_CLUSTER',WWG_MT_CLUS_MIN,IER)
        ...
        IF (IER.EQ.0) CALL EZGET('WWG_ISOL_CUT',DO_ISOL_CUT_WGAM,IER)
...
C
      ...
      IF(DO_FINDTOP) THEN
        IF (IER.EQ.0) CALL EZGET('FINDTOP_EE',DO_FINDTOP_EE,IER)
...
C
          IF(IER.EQ.0) CALL EZGET('I_BREM_FLAG',COBREM_FLAG,IER)
          IF(IER.EQ.0) CALL EZGET('MU_ELPH_DRMIN',MIN_DR_MUELPH,IER)
          IF(IER.EQ.0) CALL EZGET('DO_MT_WAB_CUT',DO_WAB_CUT,IER)
          IF(IER.EQ.0) CALL EZGET('I_BREM_FLAG2',WABREM_FLAG,IER)
...
C
          IF(IER.EQ.0) CALL EZGET('DO_DPHI12_CUT',DO_DPHI12_CUT,IER)
...
C
        IF(DO_FINDTOP_EJET.OR.DO_FINDTOP_MUJET) THEN
          ...
          IF(IER.EQ.0) CALL EZGET('LJ_NOMUON_MIN',LJ_NOMUON_MIN,IER)
          IF(IER.EQ.0) CALL EZGET('LJ_NOMUON_MAX',LJ_NOMUON_MAX,IER)
          IF(IER.EQ.0) CALL EZGET('LJ_DO_ISOL_CUT',LJ_DO_ISOL_CUT,IER)
          IF(IER.EQ.0) CALL EZGET('LJ_NOMU_ISO_MIN',LJ_NOMU_ISO_MIN,IER)
          IF(IER.EQ.0) CALL EZGET('LJ_NOMU_ISO_MAX',LJ_NOMU_ISO_MAX,IER)
          IF(IER.EQ.0) CALL EZGET('LJ_MUON2_PTMIN',LJ_MUON2_PTMIN,IER)
          IF(IER.EQ.0) CALL EZGET('LJ_NOMU_NONISO_MIN',
     &                            LJ_NOMU_NONISO_MIN,IER)
          IF(IER.EQ.0) CALL EZGET('LJ_NOMU_NONISO_MAX',
     &                            LJ_NOMU_NONISO_MAX,IER)
          ...
          IF(IER.EQ.0) CALL EZGET('LJ_JET_MULT_MIN',LJ_NOJT_MIN,IER)
     ...
     &      IER)
          ...
          IF(IER.EQ.0) CALL EZGET('DO_ISOL_CONE_SUB',LJ_DO_ISOLCONE_SUB,
     1      IER)
        ENDIF
      ENDIF
C
...
C
      ...
      IF(IER.EQ.0) CALL EZGET('FINDQCD_EMU',DO_FINDQCD_EMU,IER)
...
C
      ...
      IF(IER.EQ.0) CALL EZGET('FINDWPAIR_EE',DO_FINDWPAIR_EE,IER)
      IF(IER.EQ.0) CALL EZGET('FINDWPAIR_EMU',DO_FINDWPAIR_EMU,IER)
      IF(IER.EQ.0) CALL EZGET('FINDWPAIR_MUMU',DO_FINDWPAIR_MUMU,IER)
      IF(IER.EQ.0) CALL EZGET('FINDWPAIR_MUJET',DO_FINDWPAIR_MUJET,IER)
...
C
      IF(DO_FINDWPAIR) THEN
        ...
        IF(IER.EQ.0) CALL EZGET('WP_JET_MULT_MAX',WP_NOJT_MAX,IER)
      ENDIF
C
C
C *** Cuts
...
C
      IF(IER.EQ.0) CALL EZGET('DO_VERTEX_CUTS',DO_VERTEX_CUTS,IER)
...
C
      IF(IER.EQ.0) CALL EZGET('DO_TRIGGER_SELECT',DO_TRIG_TEST,IER)
      IF(DO_TRIG_TEST) THEN
        IF(IER.EQ.0)
     1    CALL EZGET('DO_TRIGFAIL_SELECT',DO_TRIGFAIL_ONLY,IER)
C###
...
C####
     ...
     1  DO_MICRO_BLANK_CUT,IER)
      IF(IER.EQ.0) CALL EZGET('DO_MRBS_LOSS_CUT',
     1  DO_MRBS_LOSS_CUT,IER)
C
C ***      Bad Run Test Information
C
      CALL EZGET('DO_RUN_RANGE_SELECT',DO_RUN_RANGE_SELECT,IER)
      IF(DO_RUN_RANGE_SELECT) THEN
        IF(IER.EQ.0) CALL EZGET('FIRST_RUN',FIRST_RUN,IER)
        IF(IER.EQ.0) CALL EZGET('LAST_RUN',LAST_RUN,IER)
      ENDIF
      IF(IER.EQ.0) CALL EZGET('DO_GOOD_RUN_SELECT',
     1  DO_GOOD_RUN_SELECT,IER)
C
C *** Jet Algorithm 
C
      IF (IER.EQ.0) CALL EZGET('JETS_ALGORITHM',JET_ALG,IER)
C
C *** Jet Energy Corrections
C
      IF (IER.EQ.0) CALL EZGET('JETS_CORR',CORR_JETS,IER)
      IF (IER.EQ.0) CALL EZGET('JETS_ISYS',JET_ISYS,IER)
      IF (IER.EQ.0) CALL EZGET('DO_ZSP_CORR',DO_ZSP_CORR,IER)
...
C ***    .... for RECO testing
C
      IF (IER.EQ.0) CALL EZGET('DO_RECO_DIAGNOSTICS',DO_RECO_STUDY,IER)
C
C ***    .... for TOP_LEPTONS
C
      IF (IER.EQ.0) CALL EZGET('DO_DIAGNOSTIC_DUMPS',DO_TOPLEP_STUDY,
     &  IER)
      IF(DO_TOPLEP_STUDY) THEN
        IF (IER.EQ.0) CALL EZGET('NO_EVT_DIAGNOSTICS',NO_DIAG_EVT,IER)
      ENDIF
      CALL EZRSET
