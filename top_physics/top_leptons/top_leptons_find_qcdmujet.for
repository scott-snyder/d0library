      SUBROUTINE TOP_LEPTONS_FIND_QCDMUJET(IFGOOD,NOMU,NOEL,NOPH,NOJT,
     1  MET_VEC)
C----------------------------------------------------------------------
C-
...
C-   Modified  2-Apr-1994   Same as Find_Mujet but without muon isolation
C-                          for B2B cuts and diff RCP variables for muon
C-                          and jet multiplicity requirements
C-
C----------------------------------------------------------------------
      IMPLICIT NONE
C
...
C
        CALL EZGET('MJQCD_MUON1_PTMIN',MUON1_PTMIN,IER)
        IF (IER.EQ.0) CALL EZGET('MJQCD_NOMUON_MIN',NOMUON_MIN,IER)
        IF (IER.EQ.0) CALL EZGET('MJQCD_NOMUON_MAX',NOMUON_MAX,IER)
        IF (IER.EQ.0) CALL EZGET('MJQCD_DO_ISOL_CUT',DO_ISOL_CUT,IER)
        IF (IER.EQ.0) CALL EZGET('MJQCD_NOMU_ISO_MIN',NOMU_ISO_MIN,IER)
        IF (IER.EQ.0) CALL EZGET('MJQCD_NOMU_ISO_MAX',NOMU_ISO_MAX,IER)
        IF (IER.EQ.0) CALL EZGET('MJQCD_MUON2_PTMIN',MUON2_PTMIN,IER)
        IF (IER.EQ.0) CALL EZGET('MJQCD_NOMU_NONISO_MIN',
     1    NOMU_NONISO_MIN,IER)
        IF (IER.EQ.0) CALL EZGET('MJQCD_NOMU_NONISO_MAX',
     1    NOMU_NONISO_MAX,IER)
        ...
        IF (IER.EQ.0) CALL EZGET('MJQCD_JET_MULT_MIN',NOJT_MIN,IER)
        IF (IER.EQ.0) CALL EZGET('MJQCD_JET1_PTMIN',JET_PTMIN(1),IER)
        IF (IER.EQ.0) CALL EZGET('MJQCD_JET2_PTMIN',JET_PTMIN(2),IER)
        IF (IER.EQ.0) CALL EZGET('MJQCD_JET3_PTMIN',JET_PTMIN(3),IER)
        IF (IER.EQ.0) CALL EZGET('MJQCD_JET4_PTMIN',JET_PTMIN(4),IER)
        IF (IER.EQ.0) CALL EZGET('MJQCD_JET5_PTMIN',JET_PTMIN(5),IER)
        IF (IER.EQ.0) CALL EZGET('MJQCD_JET6_PTMIN',JET_PTMIN(6),IER)
        IF (IER.EQ.0) CALL EZGET('MJQCD_JET_MULT_MAX',NOJT_MAX,IER)
        IF (IER.EQ.0) CALL EZGET('MJQCD_PTMAX_NEXTJET',PTMAX_NEXTJET,
     1    IER)
...
C
      IF (I_MU.GE.1) THEN    !Isolated muons required for standard MuJet
        MU_Phi = Q(LPMUO_VEC(1) + 17)
