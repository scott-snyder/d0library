      SUBROUTINE TOP_LEPTONS_FIND_EMU(IFGOOD,NOMU,NOEL,NOPH,
     1  NOJT,MET_VEC)
C----------------------------------------------------------------------
C-
C-   Purpose and Methods : event search routine for top->e mu decays
C-
C-   Inputs  : 
...
C-   Modified  7-Jul-1993   em, jet and missing energy scale corrections
C-                          added
C-
C----------------------------------------------------------------------
      IMPLICIT NONE
...
C
      EXTERNAL TOP_LEPTONS_GOOD_MUON,TOP_LEPTONS_GOOD_ELECTRON
...
C
      INTEGER NOMU,NOEL,NOPH,NOJT,NOJT_MIN,IER,MODE
...
C
      REAL PI,TWOPI,CONV,TEMP,TOP_LEPTONS_UTIL_CALC_DR
      ...
      REAL TOP_LEPTONS_UTIL_MASS4,TWO_BODY_MASS
...
C
     ...
     1    IER)
        IF (IER.EQ.0) CALL EZGET_i('I_BREM_FLAG',I_BREM,IER)
        IF (IER.EQ.0) CALL EZGET_i('I_BREM_FLAG2',I_BREM2,IER)
