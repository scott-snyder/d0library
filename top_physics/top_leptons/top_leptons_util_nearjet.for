      SUBROUTINE TOP_LEPTONS_UTIL_NEARJET(ETA,PHI,DR_MIN,
...
C-   Modified  9-Apr-1994   Protected against calls in which the dPhi
C-                          calculation is incorrectly handled
C-
C----------------------------------------------------------------------
      IMPLICIT NONE
C
...
C
      LOGICAL TOP_LEPTONS_GOOD_JET,ISKIP_DR,ISKIP_DPHI
C
...
C
C *** If dPhi loop is disabled then force recalulation of dPhi(min)
C
            IF(ISKIP_DPHI) THEN
              DPHI=ABS(Q(LJETS+8)-PHI)
              IF(DPHI.GT.PI) DPHI=TWOPI-DPHI
