...
C
      CALL EZGETA_i('ELECTRON_MASK',0,0,0,NOEL_BITS,IER)
      IF(NOEL_BITS.LT.32) THEN
        WRITE(LUN,1000) NOEL_BITS
        IER=-1
      ELSE
        IF(IER.EQ.0) 
     1       CALL EZGETA_iarr('ELECTRON_MASK',1,NOEL_BITS,1
     &       ,ELECTRON_MASK,IER)
      ENDIF
C
      CALL EZGETA_i('PHOTON_MASK',0,0,0,NOPH_BITS,IER)
      IF(NOPH_BITS.LT.32) THEN
        WRITE(LUN,1010) NOPH_BITS
        IER=-1
      ELSE
        IF(IER.EQ.0)
     1     CALL EZGETA_iarr('PHOTON_MASK',1,NOPH_BITS,1,PHOTON_MASK,IER)
      ENDIF
C
      CALL EZRSET
...
C
      CALL EZGETA_i('COARSE_CHISQUARED',0,0,0,N,IER)
      IF(IER.EQ.0) 
     1  CALL EZGETA('COARSE_CHISQUARED',1,N,1,COARSE_HMATRIX,IER)
      IF(IER.EQ.0) CALL EZGETA_i('FINE_CHISQUARED',0,0,0,N,IER)
...
C
     ...
     &  IER)
      ...
      IF(IER.EQ.0) CALL EZGET('ISOLATION_CONES',DR_ISOL_CONES(1),IER)
      IF(IER.EQ.0) CALL EZGET_rarr('ISOLATION_CONE_ENERGY_CUT',
     &  ISOL_CONE_ECUT,IER)
      IF(IER.EQ.0) CALL EZGET_rarr('ISOLATION_CONE_ET_CUT'
     &     ,ISOL_CONE_ETCUT,IER)
      IF(IER.EQ.0) CALL EZGET_rarr('ISOLATION_CONE_TOT_ENERGY_CUT',
     &  ISOL_CONE_TOT_ECUT,IER)
      IF(IER.EQ.0) CALL EZGET_rarr('ISOLATION_CONE_TOT_ET_CUT',
     1  ISOL_CONE_TOT_ETCUT,IER)
      IF(IER.EQ.0) CALL EZGET('CC_CRACK_WIDTH',CC_CRACK,IER)
      IF(IER.EQ.0) CALL EZGET_i('MINIMUM_NUMBER_OF_CELLS',MIN_CELLS,IER)
      IF(IER.EQ.0) CALL EZGET_l('MATCH_LEVEL2_ELECTRON',LVL2_MATCH,IER)
      IF(IER.EQ.0) CALL EZGET('L1ET_THRESHOLD',L1_ET_THRESH,IER)
      IF(IER.EQ.0) CALL EZGET('L2ET_THRESHOLD',L2_ET_THRESH,IER)
      IF(IER.EQ.0) CALL EZGET_rarr('DISTANCE_CC_CUT',CC_DIST_CUT,IER)
      IF(IER.EQ.0) CALL EZGET_rarr('OFFSET_DISTANCE_CC',CC_OFFSET_DIST
     &     ,IER)
      IF(IER.EQ.0) CALL EZGET_rarr('ERR_DISTANCE_CC',CC_ERR_DIST,IER)
      IF(IER.EQ.0) CALL EZGET_rarr('DISTANCE_EC_CUT',EC_DIST_CUT,IER)
      IF(IER.EQ.0) CALL EZGET_rarr('OFFSET_DISTANCE_EC',EC_OFFSET_DIST
     &     ,IER)
      IF(IER.EQ.0) CALL EZGET_rarr('ERR_DISTANCE_EC',EC_ERR_DIST,IER)
      IF(IER.EQ.0) CALL EZGET('SIG_TRKMATCH_CUT',SIG_TRK_MATCH,IER)
      IF(IER.EQ.0) CALL EZGET('TRACK_CONE_SIZE',TRK_CONE_SIZE,IER)
      IF(IER.EQ.0) CALL EZGET_i('NZTRAKS_CUT',MAX_ZTRAKS,IER)
      IF(IER.EQ.0) CALL EZGET_rarr('CDC_MIP_CUT',CDC_MIP_CUT,IER)
      IF(IER.EQ.0) CALL EZGET_rarr('FDC_MIP_CUT',FDC_MIP_CUT,IER)
      IF(IER.EQ.0) CALL EZGET_rarr('VTX_MIP_CUT',VTX_MIP_CUT,IER)
     ...
     &  TRD_TRUNCATED_MEAN,IER)
      IF(IER.EQ.0) CALL EZGET_rarr('IMPACT_PARAMETER_XY',IMPACT_XY,IER)
