...
C
        ...
        CALL EZGET_l('DO_TRIGGER_SELECT',DO_TRIG_TEST,IER)
        IF(DO_TRIG_TEST) THEN
          IF(IER.EQ.0) 
     1      CALL EZGET_l('DO_TRIGFAIL_SELECT',DO_TRIGFAIL_SELECT,IER)
C##
          IF(IER.EQ.0) CALL EZGET('DO_BIT_SELECT',DO_BIT_TEST,IER)
          IF(DO_BIT_TEST)THEN
C##
            IF(IER.EQ.0) CALL EZGETA('L1_BITS',0,0,0,NO_L1_SELECTED,IER)
            IF(IER.EQ.0) CALL EZGET_i('L1_BITS',L1_BITS_SELECTED,IER)
            IF(IER.EQ.0)
     1         CALL EZGETA('L2_FILTERS',0,0,0,NO_L2_SELECTED,IER)
            IF(IER.EQ.0) CALL EZGET_i('L2_FILTERS',L2_BITS_SELECTED,IER)
C###
          ENDIF
