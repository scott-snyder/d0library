...
C----------------------------------------------------------------------
      IF( first ) THEN
        ...
        CALL EZGET('STRUCTURE_FUNCTION_TYPE',ISTRUC,IER)
C
        CALL EZGET('TEST_MODEL',TESTQ,IER)
        CALL EZGET('TEST_MODEL_IN_SITU',TESTQ1,IER)
        CALL EZGET('NUMBER_TESTS',NTEST,IER)
        CALL EZGET('TOP_TEST_MASS',TMAST,IER)
        CALL EZGET('TEST_SHAT',SHT,IER)
