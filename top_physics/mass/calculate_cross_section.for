...
C----------------------------------------------------------------------
      IF( first ) THEN
        ...
        CALL EZGET_d('ROOTS_TEV',ROOTS_TEV,IER)
        CALL EZGET('QCD_LAMBDA',ALAM,IER)
        CALL EZGET_d('ALPHA_STRONG',ALPHA_S,IER)
        CALL EZGET_d('CONV_PB',CONV_PB,IER)
        CALL EZGET_i('NGAUSS',NG3,IER)
        CALL EZGET_i('NSUB_DIV',NI3,IER)
        CALL EZGET_l('USE_FITTED_CROSS_SECTION',USE_FITTED,IER)
        CALL EZGET_l('READ_CROSS_SECTION',READ_CROSS,IER)
        IF ( READ_CROSS ) THEN
