...
C
      CALL EZGET('TOP_LIMIT_ID',ID,IER)
...
C
      ...
      IF (IER.EQ.0) CALL EZGET('CHANNEL_SWITCH',CH_SW,IER)
      IF (IER.EQ.0) CALL EZGET('OBSERVED_SIGNAL',N,IER)
      IF (IER.EQ.0) CALL EZGET('EXPECTED_BG',BG,IER)
      IF (IER.EQ.0) CALL EZGET('BG_ERROR',DBG,IER)
      IF (IER.EQ.0) CALL EZGET('LUMINOSITY',LUM,IER)
      IF (IER.EQ.0) CALL EZGET('LUMINOSITY_ERROR',DLUM,IER)
      IF (IER.EQ.0) CALL EZGET('BRANCHING_RATIO',BR,IER)
      IF (IER.EQ.0) CALL EZGET('EFFICIENCY',EFF,IER)
      IF (IER.EQ.0) CALL EZGET('EFFICIENCY_ERROR',DEFF,IER)
      IF (IER.EQ.0) CALL EZ_GET_CHARS('CHANNEL_NAMES',NCHAN,NAMES,IER)
      IF (IER.EQ.0) CALL EZGET('TOP_MASS',TOP_MASS,IER)
      IF (IER.EQ.0) CALL EZGET('CONFIDENCE_LEVEL',CL,IER)
      IF (IER.EQ.0) CALL EZGET('ERROR',ERROR,IER)
      IF (IER.EQ.0) CALL EZGET('GAMMA',GAMMA,IER)
      IF (IER.EQ.0) CALL EZGET('NGEN',NGEN,IER)
      IF (IER.NE.0) GOTO 900
      EPS = ERROR
      CALL EZRSET
C
