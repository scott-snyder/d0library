...
C
      ...
      IF(FIRST) THEN
        ...
        ELSE
          ...
          MET_ERR_MULT=RDUMMY
          IF(IER.EQ.0) CALL EZGET_l('USE_MC_KAPPA_ERR',LDUMMY,IER)
     ...
     &      ADUMMY,IER)
          ...
          DELTA_CHI=RDUMMY
          IF(IER.EQ.0) CALL EZGET_l('COMPUTE_MET_ERRORS',DOMETERRS,IER)
          IF(IER.EQ.0) CALL EZGET_l('COMPUTE_MUON_ERRORS',DOERRORS,IER)
          IF(IER.EQ.0) CALL EZGET_i('MET_CONSTRAINT',MET_CONSTRAINT,IER)
          ERRTXT='RCP parameter error'
          IF(IER.NE.0)
     >         CALL ERRMSG('Z_FIT_MUMU_RCPPAR','Z_FIT_MUMU',ERRTXT,'F')
C
          CALL EZGET_l('USE_OFF_DIAGONAL',USE_OFF_DIAGONAL,IER) 
          IF(IER.NE.0) THEN
