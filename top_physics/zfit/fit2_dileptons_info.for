...
C
      ...
      IF(FIRST) THEN
        ...
        IF(IER.EQ.0)CALL EZGET('DO_HV_CORRECTION',DO_HV_CORRECTION,
     &    IER)
        CALL EZGET('DO_ELECTRONS',DO_ELECTRONS,IER)
        IF(DO_ELECTRONS)THEN
          CALL EZGET('ELECTRON_CRACK_CUT',ELECTRON_CRACK_CUT,IER)
        ENDIF
        CALL EZGET('DO_ELECTRONS',DO_ELECTRONS,IER)
        CALL EZGET('DO_MUONS',DO_MUONS,IER)
        IF(IER.EQ.0)CALL EZGET('DO_ONE_LOOSE_LEPTON',
     &    DO_ONE_LOOSE_LEPTON,IER)
        IF(IER.EQ.0)CALL EZGET('DO_TIGHT_LEPTONS',
     &    DO_TIGHT_LEPTONS,IER)
        IF(IER.EQ.0)CALL EZGET('MB_MET_ERROR',
     &    MB_MET_ERROR,IER)
        IF(IER.EQ.0)CALL EZGET('OLD_VERSION',
     &    OLD_VERSION,IER)
        IF(IER.EQ.0)CALL EZGET('DO_JET_CORRECTION',
     &    DO_JET_CORRECTION,IER)
        IF(IER.EQ.0)CALL EZGET('DO_EM_CORRECTION',
     &    DO_EM_CORRECTION,IER)
        IF(IER.EQ.0)CALL EZGET('DO_MUON_CORRECTION',
     &    DO_MUON_CORRECTION,IER)
        IF(IER.EQ.0)CALL EZGET('DO_SOFT_CORRECTION',
     &    DO_SOFT_CORRECTION,IER)
        IF(IER.EQ.0)CALL EZGET('DO_ENERGY_CORRECTIONS',
     &    DO_ENERGY_CORRECTIONS,IER)
        CALL EZGET('DO_CLEAN_UP',DO_CLEAN_UP,IER)
...
C
     ...
     &        'Cannot find bank BAD_RUN_RCP','W')
          ...
          IF(NBAD_RUNS.LE.200) THEN
            IF(IER.EQ.0) CALL EZGET('BAD_RUN_NOS',BAD_RUNS,IER)
          ELSE
