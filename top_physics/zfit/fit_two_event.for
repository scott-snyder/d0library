...
C
      ...
      DIMENSION BLEP(2,16),BLEP_ERR(2,3,3),BMET(2),BMET_ERR(2,2)
...
C
      ...
      INTEGER SKIP_EVTS(100),NSKIP_EVTS,LUNIT
      INTEGER RUN,EVENT
C
     ...
     &ZMASS(2) !ZMASS RANGE
      ...
      REAL LEP(2,16),LEP_ERR(2,3,3),MET(2),MET_ERR(2,2)
...
C
        ...
        ZWID_OBS=OBS_ZWIDTH
        CALL EZGET_l('FIX_ZMASS',FIX_ZMASS,IER)
        CALL EZGET_l('OFF_DIAG',OFF_DIAG,IER)
...
C
        CALL EZGET_l('CALCULATE_ERRORS',CALCULATE_ERRORS,IER)
        CALL EZGET('SCALE_MISSET_ERRORS',METERR_SCALE,IER)
        IF(IER.NE.0)METERR_SCALE=1.
        CALL EZGET_i('MINUIT_PRINT_LEVEL',PRINT_LEV,IER)
        CALL EZGET_l('BW_CONSTRAINT',BW_CONSTR,IER)
        IF(IER.NE.0)BW_CONSTR=.TRUE.
        CALL EZGET_l('WRITE_FIT2_FILE',WRITE_FIT2_FILE,IER)
        CALL EZGET_l('WRITE_NTUPLE',WRITE_NTUPLE,IER)
C
        CALL EZGETA_i('SKIP_EVENTS',0,100,1,NSKIP_EVTS,IER)
...
C
   ...
   10 CONTINUE
C
C ****  Process the DST file
C ****  GET DI-LEPTONS FROM AN EVENT
C
C
      ISKIP=1
       RUN=RUNNO()
       EVENT=EVONUM()
C
      CALL FIT2_DILEPTONS_INFO(IRUN,IEVENT,LEP,LEP_ERR,MET,MET_ERR,OK)
      NEV=NEV+1
      IF(.NOT.OK)THEN
        WRITE(LUNIT,*)'RUN=',IRUN,' EVENT=',IEVENT,
     &  '  NO TWO GOOD LEPTONS'
        GOTO 999
      ENDIF
C      IF(IEVENT.EQ.1003)GOTO 999
      IF(NSKIP_EVTS.GE.1)THEN
        DO I=1,NSKIP_EVTS
          IF(IEVENT.EQ.SKIP_EVTS(I))THEN
            WRITE(6,*)'SKIPPING EVENT:',IEVENT
            WRITE(LUNIT,*)'SKIPPING EVENT:',IEVENT
            GOTO 999
          ENDIF
        ENDDO
      ENDIF
...
C
      ...
      ENDIF
C
      MASS=2.*(BLEP(1,4)*BLEP(2,4) - (BLEP(1,1)*BLEP(2,1)+
     &BLEP(1,2)*BLEP(2,2)+BLEP(1,3)*BLEP(2,3)))
      IF ( MASS.GT.0 ) THEN
        MASS=SQRT(MASS)
      ELSE
        MASS=0.
     ...
     &   '*******************************************************'
      WRITE(LUNIT,*)
     &   '*******************************************************'
      WRITE(LUNIT,*)' IRUN= ',IRUN,' IEVENT=',IEVENT
      WRITE(LUNIT,*)
     &   '*******************************************************'
...
C
      ...
      DO I=1,NPARM
        ...
        IF(FIX_ZMASS)THEN
          ARGLIS(1)=6.
          CALL MNEXCM(FCN,'FIX',ARGLIS(1),1,IER,0)
        ENDIF
        IF(IER .NE. 0)THEN
...
C
      ARGLIS(1)=PRINT_LEV
      CALL MNEXCM(FCN,'SET PRINT',ARGLIS,1,IER,0)
C
      CALL MNEXCM(FCN,'MINI',0.d0,0,IER,0)
C
C        CALL MNEXCM(FCN,'MINOS',3.,1,IER,0)
C
...
C
      ...
      ENDIF
      ARGLIS(1)=10
      CALL MNEXCM(FCN,'END',0.d0,0,IER,0)
      CALL MNEXCM(FCN,'CALL FCN',ARGLIS,1,IER,0)
C
...
C
      IF(WRITE_NTUPLE)THEN
        CALL FIT2_NTUPLE(IER)
      ENDIF
C
  900 CONTINUE
  999 RETURN
